...
def _build_image_wsl2(
    ...
    def _boot_and_monitor():
        # SOVEREIGN: boot keypresses (fewer needed with KVM — faster boot)
        for i in range(15):
            time.sleep(2)
            if _qmp_cmd(
        ...
        # SOVEREIGN: wait for CUA server (KVM is much faster, ~15-30 min)
...
def build_image(
    ...
    *,
    ...
    if _plat.system() == "Windows":
        ...
        def _boot_and_monitor():
            """SOVEREIGN: send Enter keys for 30s to bypass boot prompts.
            SOVEREIGN: poll CUA server on port 18000 until it responds.
            SOVEREIGN: send ACPI shutdown via QMP."""
            # SOVEREIGN: boot keypresses
            for i in range(15):
                time.sleep(2)
                if _qmp_cmd(
                    b'{"execute":"send-key","arguments":{"keys":[{"type":"qcode","data":"ret"}]}}'
            ...
            # SOVEREIGN: wait for CUA server (up to 90 min for TCG installs)
