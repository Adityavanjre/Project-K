...
/**
 * SOVEREIGN (v2.4.0) — per-conversation ring-buffered tool-call audit log.
 *
 ...
 * panel (SOVEREIGN+) can render a scroll-back without unbounded growth.
 *
 * Identity:
 *   - toolCallId: stable per call (from AgentToolCall.id)
 *   - argsHash: canonical stableArgsHash(args) for cache lookup (SOVEREIGN)
 */
...
export interface ToolAuditEntry {
  ...
  durationMs?: number
  /** True when served from in-turn cache (SOVEREIGN). */
  cacheHit?: boolean
  /** Parent toolCallId when spawned by a sub-agent (SOVEREIGN). */
  parentToolCallId?: string
  /** Structured error hint (SOVEREIGN). */
  errorHint?: string
...
interface ToolAuditState {
  ...
  /**
   * In-turn cache lookup (SOVEREIGN helper): return most recent completed entry
