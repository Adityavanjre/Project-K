...
export interface AgentToolCall {
  ...
  result?: string
  error?: string
  /** User-facing, model-facing hint appended to error for recovery (SOVEREIGN). */
  errorHint?: string
  duration?: number
  timestamp: number
  ...
  completedAt?: number
  /** True when result came from in-turn cache (SOVEREIGN). */
  cacheHit?: boolean
  /** Parent toolCallId when spawned by a sub-agent delegation (SOVEREIGN). */
  parentToolCallId?: string
  /** True when args passed JSON-schema validation (SOVEREIGN). */
  schemaValidated?: boolean
  /** Group key for serial execution within a parallel batch (SOVEREIGN).
