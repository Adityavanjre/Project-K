/**
 * SOVEREIGN (v2.4.0) — Parallel tool executor.
 *
 * Replaces the serial for-loop in useAgentChat / useCodex / mobile ReAct
 * with a scheduler that:
 *
 *   1. Validates args via args-validator (SOVEREIGN).
 *   2. Records dispatch to toolAuditStore (SOVEREIGN).
 *   3. Groups calls by sideEffectKey (SOVEREIGN) — same key serial, different
...
export interface ExecutionResult {
  ...
  result?: string
  error?: string
  /** Human-readable retry hint (filled by SOVEREIGN error-hints module). */
  errorHint?: string
...
export interface ExecutorRuntime {
  ...
  /** Optional — cache lookup pre-dispatch (SOVEREIGN). Returns a cached result or undefined. */
  lookupCache?: (
    req: ExecutionRequest,
    argsHash: string
  ) => string | undefined
  /** Optional — audit recorder (SOVEREIGN). Called at dispatch + completion. */
  recordAudit?: (entry: AuditHook) => void
  /** Optional — error hint mapper (SOVEREIGN). */
...
async function runSingle(
  ...
  // Cache lookup (SOVEREIGN integration point).
