...
//
// SOVEREIGN introduced parallel tool execution via executeParallel — which
...
export function useAgentChat() {
  ...
  /**
   * ID-keyed block update — used by the parallel tool executor (SOVEREIGN) so
  ...
  const sendAgentMessage = useCallback(async (userContent: string, userImages?: import('../types/chat').ImageAttachment[]) => {
    ...
    // SOVEREIGN: lock in the start-of-turn timestamp so the in-turn cache
    // only serves results from calls made during THIS user prompt.
    const turnStartMs = Date.now()
    // SOVEREIGN: hard caps on tool calls and loop iterations. Halts cleanly
    ...
    try {
      ...
      while (runningRef.current && !abort.signal.aborted) {
        ...
        if (strategy === 'native') {
          ...
          // (SOVEREIGN). The embedding call is best-effort: if Ollama is
        ...
        // SOVEREIGNb (v2.4.0) — parallel tool execution via tool-executor.
        //
        ...
        const results = await executeParallel(requests, {
          ...
          awaitApproval: async (req) => {
            ...
            // SOVEREIGN — tools whose AC was marked 'running' at batch
