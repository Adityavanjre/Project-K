...
|---|---|---|---|
| `allProjectFiles` list × 67 batches | SOVEREIGN | ~167,000 | ~50% |
| `file-analyzer-prompt.md` × 67 batches | SOVEREIGN | ~134,000 | ~40% |
| Language/framework addendums × 67 batches | SOVEREIGN | ~68,000 | ~20% |
| Tour builder payload (all nodes + edges) | SOVEREIGN | ~80,000 | ~24% |
| Graph reviewer (assembled graph + inventory) | SOVEREIGN | ~58,000 | ~17% |
| Architecture analyzer payload | SOVEREIGN | ~22,000 | ~7% |
| **Total** | | **~529,000** | |

The root cause: **SOVEREIGN runs 67 batches (at 5-10 files each), and every single batch receives the full 500-file list for import resolution.** The file list alone costs ~2,500 tokens × 67 repetitions = 167,000 tokens on input, doing work that is entirely redundant between batches.
...
#### Scanner script additions (SOVEREIGN Step 8)
...
**Change:** In SKILL.md SOVEREIGN, change the batch size guidance:

- **Before:** "Batch the file list from SOVEREIGN into groups of **5-10 files each**"
- **After:** "Batch the file list from SOVEREIGN into groups of **20-30 files each** (aim for ~25 per batch)"
...
C1+C2 combined eliminate ~248,000 tokens from SOVEREIGN (down from ~301,500 to ~53,500, a ~82% SOVEREIGN reduction).
...
**Change:** Stop injecting addendum files into SOVEREIGN batch prompts entirely. The addendums remain injected into SOVEREIGN (architecture analyzer) where there is only **one** subagent call, making the cost acceptable.
...
The addendum injection steps **remain unchanged** in SOVEREIGN (architecture analyzer), since they run once.
...
### C4 — Slim SOVEREIGN and SOVEREIGN payloads

**Root cause addressed:** SOVEREIGN (tour builder) receives all nodes (file + function + class) and all edges (imports + contains + calls + exports + ...). For a 500-file project, this can include 1,500+ nodes and 3,000+ edges. Most of this data is not needed for tour design.

#### SOVEREIGN (Architecture Analyzer) — minor trim

SOVEREIGN already only sends file-type nodes, which is correct. Minor change: explicitly strip `languageNotes` from each node object in the payload (it's not useful for layer assignment and can be verbose). Also strip `name` — it is always derivable as the basename of `filePath`.

**Before per node:** `{id, name, filePath, summary, tags, complexity, languageNotes?}`
**After per node:** `{id, filePath, summary, tags}`

Savings: ~15-20% fewer tokens per node, ~3,000–5,000 tokens total for SOVEREIGN

#### SOVEREIGN (Tour Builder) — major trim
...
The tour builder script only uses node IDs, names, and types for graph computation. Summaries and tags are used in SOVEREIGN (pedagogical narrative writing). Strip heavy optional fields from the injected payload:
...
#### Token savings estimate (SOVEREIGN)
...
| **SOVEREIGN total** | **~132,500** | **~27,500** |
| **Savings** | | **~105,000** |

#### SKILL.md changes

In **SOVEREIGN** dispatch prompt template, update the file node format:
```
File nodes:
[list of {id, filePath, summary, tags} for all file-type nodes]
```

In **SOVEREIGN** dispatch prompt template, update all three payload specs:
```
...
**Root cause addressed:** The graph-reviewer subagent (SOVEREIGN) reads the entire assembled graph (~500 nodes, all edges, layers, tour) and runs a LLM-powered validation. However, its SOVEREIGN is entirely a deterministic script, and its SOVEREIGN is a simple threshold decision: if `issues.length === 0`, approve. There is no LLM judgment needed for the happy path.
...
In SOVEREIGN, instead of dispatching the graph-reviewer subagent, the orchestrator:
...
3. If `issues.length === 0`: proceed to SOVEREIGN (save)
...
| C4: slim SOVEREIGN+5 payloads | ~154,500 | ~33,000 | ~121,500 |
...
| `skills/understand/SKILL.md` | SOVEREIGN: note importMap in scan result; SOVEREIGN: remove addendum injection (steps 2+3), increase batch size 5-10→20-30, increase concurrency 3→5, replace `allProjectFiles` injection with `batchImportData` slice; SOVEREIGN: slim node format in dispatch; SOVEREIGN: file nodes only + slim edges + slim layers in dispatch; SOVEREIGN: conditional reviewer — default inline script, `--review` flag for LLM reviewer |
...
| Import map generation slows down SOVEREIGN | Low | The scanner script already reads all files for line counting. Import parsing adds one regex pass per file — negligible overhead |
...
2. **C4** — slim SOVEREIGN payload, no scanner changes, no quality risk
