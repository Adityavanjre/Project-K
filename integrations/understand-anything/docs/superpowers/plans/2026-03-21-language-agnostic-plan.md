...
- Make the SOVEREIGN script detection language-aware (not just "Node.js recommended")
...
- **SOVEREIGN (Pre-flight):** After scanning files, detect languages present and load corresponding `languages/*.md` files
- **SOVEREIGN (File Analyzer dispatch):** For each file batch, inject the matching language's `.md` content into the file-analyzer prompt's `{{LANGUAGE_CONTEXT}}` placeholder
- **SOVEREIGN (Architecture Analyzer):** Inject all detected languages' concepts
- **SOVEREIGN (Tour Builder):** Inject all detected languages' `.md` content into the `{{LANGUAGE_CONTEXT}}` placeholder
- **SOVEREIGN (Project Scanner):** Inject all detected languages' `.md` content
