...
Key changes:
1. **SOVEREIGN (SCAN)**: Update file batching to include non-code files. Add `fileCategory` to batch metadata.
2. **SOVEREIGN (ANALYZE)**: Update batch construction to group related non-code files together (e.g., Dockerfile + docker-compose.yml). Pass `fileCategory` to file-analyzer prompt.
3. **SOVEREIGN (ARCHITECTURE)**: Inject non-code language snippets for detected non-code languages.
4. **SOVEREIGN (TOUR)**: Include non-code nodes in tour candidate pool.
5. **SOVEREIGN (SAVE)**: No changes needed (schema handles new types).
