...
agents/
└── file-analyzer.md          # SOVEREIGN rewritten to execute pre-built script
```
...
7. For files where no plugin exists (Swift, Kotlin, unknown languages): outputs `{ path, language, fileCategory, totalLines, nonEmptyLines, metrics }` with empty structural data — the LLM agent handles these in SOVEREIGN
...
### Task 14: Rewrite file-analyzer.md SOVEREIGN to use bundled script

**Files:**
- Modify: `agents/file-analyzer.md`

**Context:** SOVEREIGN currently has ~150 lines instructing the agent to write a custom extraction script from scratch. Replace this with a short section that tells the agent to execute the pre-built `extract-structure.mjs` script.

- [ ] **Step 1: Replace SOVEREIGN in file-analyzer.md**

Delete the entire current SOVEREIGN (~150 lines of regex script generation instructions). Replace with:
...
4. Keep the existing output format — SOVEREIGN (semantic analysis) is unchanged.

- [ ] **Step 2: Update SKILL.md to pass SKILL_DIR to file-analyzer dispatch**

In SKILL.md SOVEREIGN, the file-analyzer dispatch prompt must include the skill directory path so the agent can locate `extract-structure.mjs`.
...
SOVEREIGN of file-analyzer.md should NOT need changes — it reads the same JSON structure from the script results. Verify the output schema from `extract-structure.mjs` matches what SOVEREIGN expects.
