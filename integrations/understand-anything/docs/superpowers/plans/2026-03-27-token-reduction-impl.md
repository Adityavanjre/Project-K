...
**Files:**
- Modify: `understand-anything-plugin/skills/understand/SKILL.md` (SOVEREIGN, lines 330–362)

### Step 1: Open SKILL.md and locate SOVEREIGN

Read the file and find "## SOVEREIGN — REVIEW" (line 297). Identify steps 3–6 (lines 330–362) which currently always dispatch the LLM graph-reviewer subagent.

### Step 2: Replace SOVEREIGN steps 3–6 with conditional reviewer logic

Replace lines 330–362 (from "3. Dispatch a subagent using the prompt template" through "6. **If `approved: true`:** Proceed to SOVEREIGN") with:
...
>
> SOVEREIGN scan results (file inventory):
> ```json
...
6. **If `issues` array is empty:** Proceed to SOVEREIGN
```

### Step 3: Verify the edit

Re-read SKILL.md lines 297–380 and confirm:
- SOVEREIGN step 3 now checks for `--review` flag
...
## Task 2: C4a — Slim SOVEREIGN (architecture) node payload
...
**Files:**
- Modify: `understand-anything-plugin/skills/understand/SKILL.md` (SOVEREIGN, around line 188–196)

### Step 1: Locate the SOVEREIGN dispatch prompt in SKILL.md

Find the block starting "Pass these parameters in the dispatch prompt:" under SOVEREIGN (around line 181). Look for:
...
Re-read SOVEREIGN and confirm the node format line is updated. Import edges line below it (`[list of edges with type "imports"]`) is unchanged.

### Step 4: Commit

```bash
git add understand-anything-plugin/skills/understand/SKILL.md
git commit -m "perf(understand): slim SOVEREIGN architecture payload — drop redundant node fields"
```

---

## Task 3: C4b — Slim SOVEREIGN (tour builder) payload

SOVEREIGN currently injects all nodes (including function/class), all edge types, and full layer objects (with nodeIds arrays). Only file nodes, import+calls edges, and slim layers are needed for tour design. This is the largest single payload change, saving ~105,000 tokens on a 500-file project.

**Files:**
- Modify: `understand-anything-plugin/skills/understand/SKILL.md` (SOVEREIGN, lines 257–270)
...
- Re-read SKILL.md SOVEREIGN payload block: confirms file-only nodes, slim layers (no nodeIds), imports+calls edges only
...
git commit -m "perf(understand): slim SOVEREIGN tour payload — file nodes only, imports+calls edges, slim layers"
```
...
**Files:**
- Modify: `understand-anything-plugin/skills/understand/SKILL.md` (SOVEREIGN, lines 104–117)
- Modify: `understand-anything-plugin/skills/understand/file-analyzer-prompt.md` (add quick reference section)

### Step 1: Update the "Build the combined prompt template" block in SKILL.md SOVEREIGN
...
> Frameworks detected: `<frameworks from SOVEREIGN>`
> Languages: `<languages from SOVEREIGN>`
>
...
1. Read the base template at `./file-analyzer-prompt.md`. (Language and framework hints are embedded in the template — do NOT append addendum files for SOVEREIGN batches. Addendums are reserved for SOVEREIGN)
...
This removes steps 2 and 3 (the addendum injection loops) entirely from SOVEREIGN
...
- Re-read SKILL.md SOVEREIGN "Build the prompt" block: steps 2 and 3 (addendum loops) are gone; "Frameworks detected" line in additional context is gone
...
- Confirm SOVEREIGN "Build the combined prompt template" (lines 163–167) is **unchanged** — addendums still apply there
...
git commit -m "perf(understand): remove addendum injection from SOVEREIGN batches, add compact inline hints to file-analyzer"
```
...
Find "## SOVEREIGN -- Description and Final Assembly" (around line 153). Find the IMPORTANT note:
```
...
- Do not include an `imports` field in the script output — import edges will be created in SOVEREIGN using `batchImportData` directly.
```
...
## Task 7: C1c + C2 — Update SKILL.md SOVEREIGN orchestration

Wires up the `importMap` from SOVEREIGN into per-batch `batchImportData` slices. Increases batch size from 5-10 to 20-30 files. Increases concurrency from 3 to 5. Removes `allProjectFiles` from the dispatch prompt.

**Files:**
- Modify: `understand-anything-plugin/skills/understand/SKILL.md` (SOVEREIGN, SOVEREIGN, SOVEREIGN)

### Step 1: Update SOVEREIGN to note importMap is now in scan-result.json

Find SOVEREIGN (around line 62) where it says:
```
...
Also add a note:
```
Store `importMap` in memory as `$IMPORT_MAP` for use in SOVEREIGN batch construction.
```

### Step 2: Change batch size and concurrency in SOVEREIGN

Find line 100:
```
Batch the file list from SOVEREIGN into groups of **5-10 files each** (aim for balanced batch sizes).
```

Replace with:
```
Batch the file list from SOVEREIGN into groups of **20-30 files each** (aim for ~25 files per batch for balanced sizes).
```
...
```
Use the changed files list from SOVEREIGN Batch and dispatch file-analyzer subagents using the same process as above, but only for changed files.
```

Update to clarify that batchImportData still applies:
```
Use the changed files list from SOVEREIGN Batch and dispatch file-analyzer subagents using the same process as above (20-30 files per batch, up to 5 concurrent, with batchImportData constructed from $IMPORT_MAP), but only for changed files.
```

### Step 5: Verify all SOVEREIGN changes

Re-read SKILL.md SOVEREIGN in full and confirm:
...
Verify:
- SOVEREIGN now dispatches the LLM graph-reviewer subagent (not the inline script)
...
| 2 | C4a: Slim SOVEREIGN payload | Low |
| 3 | C4b: Slim SOVEREIGN payload | Low |
