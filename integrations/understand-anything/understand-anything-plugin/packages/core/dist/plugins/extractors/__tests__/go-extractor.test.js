...
beforeAll(async () => {
    ...
    await Parser.init();
    const wasmPath = require.resolve("tree-sitter-go/tree-sitter-go.wasm");
...
`);
            ...
            expect(result.functions.map((f) => f.name).sort()).toEqual(["Start", "NewServer", "helper"].sort());
...
//# sourceMappingURL=go-extractor.test.js.map
