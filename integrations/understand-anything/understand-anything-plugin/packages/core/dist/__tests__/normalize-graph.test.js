...
import { normalizeNodeId, normalizeComplexity, normalizeBatchOutput, } from "../analyzer/normalize-graph.js";
import { validateGraph } from "../schema.js";
describe("normalizeNodeId", () => {
    it("passes through a correct file ID unchanged", () => {
        expect(normalizeNodeId("file:src/index.ts", { type: "file" })).toBe("file:src/index.ts");
    });
    it("passes through a correct func ID unchanged", () => {
        expect(normalizeNodeId("func:src/utils.ts:formatDate", { type: "function" })).toBe("func:src/utils.ts:formatDate");
    });
    it("passes through a correct class ID unchanged", () => {
        expect(normalizeNodeId("class:src/models/User.ts:User", { type: "class" })).toBe("class:src/models/User.ts:User");
    });
    it("fixes double-prefixed IDs", () => {
        expect(normalizeNodeId("file:file:src/foo.ts", { type: "file" })).toBe("file:src/foo.ts");
    });
    it("strips project-name prefix when valid prefix follows", () => {
        expect(normalizeNodeId("my-project:file:src/foo.ts", { type: "file" })).toBe("file:src/foo.ts");
    });
    it("strips project-name prefix and adds correct prefix for bare path", () => {
        expect(normalizeNodeId("my-project:src/foo.ts", { type: "file" })).toBe("file:src/foo.ts");
    });
    it("adds file: prefix to bare paths", () => {
        expect(normalizeNodeId("frontend/src/utils/constants.ts", { type: "file" })).toBe("file:frontend/src/utils/constants.ts");
    });
    it("reconstructs func ID from filePath and name for bare paths", () => {
        expect(normalizeNodeId("formatDate", {
            type: "function",
    ...
    it("trims whitespace", () => {
        expect(normalizeNodeId("  file:src/foo.ts  ", { type: "file" })).toBe("file:src/foo.ts");
    });
    it("handles module: and concept: prefixes", () => {
        expect(normalizeNodeId("module:auth", { type: "module" })).toBe("module:auth");
        expect(normalizeNodeId("concept:caching", { type: "concept" })).toBe("concept:caching");
    });
    it("handles project-name prefix before a valid non-code prefix", () => {
        expect(normalizeNodeId("my-project:service:docker-compose.yml", {
            type: "file",
    ...
    it("falls back to untouched ID for unknown node type", () => {
        expect(normalizeNodeId("some-id", { type: "widget" })).toBe("some-id");
    });
...
//# sourceMappingURL=normalize-graph.test.js.map
