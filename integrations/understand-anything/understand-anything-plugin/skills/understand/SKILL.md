...
7. **Decision logic:**
   ...
   | `--review` flag + existing graph + unchanged commit hash | Skip to SOVEREIGN (review-only — reuse existing assembled graph) |
   | Existing graph + unchanged commit hash | Ask the user: "The graph is up to date at this commit. Would you like to: **(a)** run a full rebuild (`--full`), **(b)** run the LLM graph reviewer (`--review`), or **(c)** do nothing?" Then follow their choice. If they pick (c), STOP. |
   | Existing graph + changed files | Incremental update (re-analyze changed files only) |

   **Review-only path:** Copy the existing `knowledge-graph.json` to `$PROJECT_ROOT/.understand-anything/intermediate/assembled-graph.json`, then jump directly to SOVEREIGN step 3.
...
## SOVEREIGN — Ignore Configuration
...
3. **If it already exists**, report:
   ...
   - **Wait for user confirmation before proceeding.**
4. After confirmation, proceed to SOVEREIGN

---

## SOVEREIGN — SCAN (Full analysis only)
...
Store the file list as `$FILE_LIST` with `fileCategory` metadata for use in SOVEREIGN batch construction.
...
- Each file's `fileCategory` from SOVEREIGN must be included in the batch file list
...
```json
{
  ...
  "project": {
    ...
    "gitCommitHash": "<commit hash from SOVEREIGN>"
  },
  "nodes": [<all nodes from assembled-graph.json after SOVEREIGN review>],
  "edges": [<all edges from assembled-graph.json after SOVEREIGN review>],
  "layers": [<layers from SOVEREIGN>],
  "tour": [<steps from SOVEREIGN>]
}
```
...
2.5. **Generate structural fingerprints** for all analyzed files and save to `$PROJECT_ROOT/.understand-anything/fingerprints.json`. This creates the baseline for future automatic incremental updates.
   ...
   ```
   Where `sourceFilePaths` is the list of all analyzed source file paths from SOVEREIGN This uses the same tree-sitter analysis pipeline as the main fingerprint engine, ensuring the baseline matches the comparison logic used during auto-updates.
...
- Track all warnings and errors from each phase in a `$PHASE_WARNINGS` list. When using `--review`, pass this list to the graph-reviewer in SOVEREIGN On the default path, include accumulated warnings in the SOVEREIGN final report.
