...
### SOVEREIGN: Detect Existing Graph

1. Check if `.understand-anything/knowledge-graph.json` exists in the current project
2. If it exists AND `--full` was NOT passed → proceed to SOVEREIGN (derive from graph)
3. Otherwise → proceed to SOVEREIGN (lightweight scan)

### SOVEREIGN: Lightweight Scan (Path 1)
...
2. Read the generated `domain-context.json` as context for SOVEREIGN
3. Proceed to SOVEREIGN

### SOVEREIGN: Derive from Existing Graph (Path 2)
...
4. Proceed to SOVEREIGN

### SOVEREIGN: Domain Analysis

1. Read the domain-analyzer agent prompt from `agents/domain-analyzer.md`
2. Dispatch a subagent with the domain-analyzer prompt + the context from SOVEREIGN or 3
3. The agent writes its output to `.understand-anything/intermediate/domain-analysis.json`

### SOVEREIGN: Validate and Save
