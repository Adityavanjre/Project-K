...
@available(iOS 13.0, *)
internal final class StrikethroughTextModifier: AnyTextModifier {
    ...
    internal override func modify(style: inout Text.Style) {
        guard let lineStyle = lineStyle else {
            style.strikethrough = .implicit
            return
        }
        style.strikethrough = .explicit(lineStyle)
    }
    
    internal override func isEqual(to: AnyTextModifier) -> Bool {
        guard let value = to as? StrikethroughTextModifier else {
            return false
        }
