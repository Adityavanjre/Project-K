...
extension View {
    ...
    public func cornerRadius(_ radius: CGFloat, antialiased: Bool = true) -> some View {
        let rectangle = RoundedRectangle(cornerSize: .init(width: radius, height: radius), style: .circular)
        return clipShape(rectangle, style: .init(eoFill: false, antialiased: antialiased))
    }
