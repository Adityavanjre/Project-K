...
@available(iOS 13.0, *)
private struct ContainerRelativeFrameModifier: MultiViewModifier, PrimitiveViewModifier {
    
    fileprivate var axes: Axis.Set

    fileprivate var alignment: Alignment

    fileprivate var updateLength: ((CGFloat, Axis) -> CGFloat)?
    
    fileprivate static func _makeView(modifier: _GraphValue<ContainerRelativeFrameModifier>, inputs: _ViewInputs, body: @escaping (_Graph, _ViewInputs) -> _ViewOutputs) -> _ViewOutputs {
        let frameLayoutModifier: _GraphValue<_FrameLayout> = .init(MakeLayout(modifier: modifier.value, containerSize: inputs.scrollableContainerSize))
        return _FrameLayout._makeView(modifier: frameLayoutModifier, inputs: inputs) { graph, internalInputs in
            body(_Graph(), inputs)
        }
    }
    
    fileprivate struct MakeLayout: Rule, AsyncAttribute {
        
        typealias Value = _FrameLayout
        
        @Attribute
        fileprivate var modifier: ContainerRelativeFrameModifier
        
        @OptionalAttribute
        fileprivate var containerSize: ViewSize?
        
        fileprivate init(modifier: Attribute<ContainerRelativeFrameModifier>,
        ...
        fileprivate var value: Value {
            ...
            let axes = modifier.axes
            var width: CGFloat? = nil
            var height: CGFloat? = nil
            let containerSize: CGSize? = self.containerSize?.value
            ...
            if let body = modifier.updateLength {
                if let length = width {
                    width = body(length, .horizontal)
                }
                
                if let length = height {
                    height = body(length, .vertical)
                }
            }
            
            return _FrameLayout(width: width , height: height, alignment: modifier.alignment)
        }
    }
...
extension View {
    ...
    ///   - A NavigationStack
    ///   - A tab of a TabView
    ///   - A scrollable view like ScrollView
    ///
    ...
    public func containerRelativeFrame(_ axes: Axis.Set, alignment: Alignment = .center) -> some View {
        modifier(ContainerRelativeFrameModifier(axes: axes, alignment: alignment))
    }
    ...
    public func containerRelativeFrame(_ axes: Axis.Set, count: Int, span: Int = 1, spacing: CGFloat, alignment: Alignment = .center) -> some View {
        modifier(ContainerRelativeFrameModifier(axes: axes, alignment: alignment, updateLength: { length, axis in
            guard axes.contains(.init(axis: axis)) else {
                return length
            }
            let count = CGFloat(count)
            let span = CGFloat(span)
            let availableWidth = (length - (spacing * (count - 1)))
            let columnWidth = (availableWidth / count)
            let itemWidth = (columnWidth * span) + ((span - 1) * spacing)
            return itemWidth
        }))
    ...
    public func containerRelativeFrame(_ axes: Axis.Set, alignment: Alignment = .center, _ length: @escaping (CGFloat, Axis) -> CGFloat) -> some View  {
        modifier(ContainerRelativeFrameModifier(axes: axes, alignment: alignment, updateLength: length))
    }
