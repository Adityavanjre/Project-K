...
@available(iOS 13.0, *)
internal struct SpatialEvent: SpatialEventType, PressableEventValue, Equatable {
    ...
    internal init(phase: EventPhase, timestamp: Time, binding: EventBinding?, globalLocation: CGPoint, location: CGPoint, radius: CGFloat) {
        self.phase = phase
    ...
    internal init?(_ event: EventType) {
        if let spatialEvent = event as? SpatialEventType {
            self.init(spatialEvent)
        } else {
            self.init(phase: .failed, timestamp: .zero, binding: event.binding, globalLocation: .zero, location: .zero, radius: .zero)
        }
    }
    
    internal init(_ spatialEvent: SpatialEventType) {
        phase = spatialEvent.phase
        timestamp = spatialEvent.timestamp
        binding = spatialEvent.binding
