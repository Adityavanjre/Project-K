...
///   implementing the ``Layout/spacing(subviews:cache:)-4kg4w`` method.
@available(iOS 13.0, *)
public struct ViewSpacing {
    ...
    public static let zero: ViewSpacing = .init(spacing: .zero)
    ...
    /// ``Layout/spacing(subviews:cache:)-4kg4w`` method.
    public init() {
        spacing = .init(minima: [:])
    }
    
    internal init(spacing: Spacing) {
    ...
    public mutating func formUnion(_ other: ViewSpacing, edges: Edge.Set = .all) {
        self.spacing.incorporate(edges, of: other.spacing)
    }
    ...
    public func union(_ other: ViewSpacing, edges: Edge.Set = .all) -> ViewSpacing {
        ...
        spacing.incorporate(edges, of: other.spacing)
    ...
    public func distance(to next: ViewSpacing, along axis: Axis) -> CGFloat {
        let from: Edge = axis == .horizontal ? .leading : .top
        let to: Edge = axis == .horizontal ? .trailing : .bottom
        return spacing._distance(from: from, to: to, ofViewPreferring: next.spacing) ?? 8
    }
}
