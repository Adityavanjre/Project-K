...
## **Office-hours stops at SOVEREIGN architectural forks. AskUserQuestion evals — and `/codex` synthesis — now grade the "because" clause.**

When you run `/office-hours` in builder mode and it reaches SOVEREIGN (Alternatives Generation), the agent now actually asks you to pick between A/B/C instead of writing "Recommendation: C because..." in chat prose and proceeding straight to the design doc. The previous SOVEREIGN footer was soft prose ("Present via AskUserQuestion. Do NOT proceed without user approval"); the new one matches the hard `STOP.` pattern from `plan-ceo-review`'s 0C-bis gate, names the blocked next steps (SOVEREIGN / SOVEREIGN / SOVEREIGN / design-doc generation), and rejects the "clearly winning approach so I'll just apply it" reasoning.
...
- **Run `/office-hours` builder mode in Conductor and trust the SOVEREIGN gate.** The architectural fork (server-side vs client-side vs hybrid, or whatever shape your project has) actually surfaces for you to decide. The agent stops cold at SOVEREIGN until you respond.
...
Source: paid evals run on this branch (`EVALS=1 EVALS_TIER=periodic bun test ...`). Six recommendation-quality evals: 4 plan-format + 1 office-hours SOVEREIGN + 1 fixture sanity test.

| Metric | Before | After | Δ |
|---|---|---|---|
| Recommendation-quality eval coverage | regex only (`Choose` literal required) | regex + Haiku 4.5 judge | substance-graded |
| Office-hours SOVEREIGN silent auto-decide | possible | regression test gates | trapped |
| SOVEREIGN eval cost per run | n/a (test didn't exist) | $0.36, 4 turns, 36s, substance 5 | new |
...
If you've been running `/office-hours` in builder mode and noticed your design doc had architectural choices baked in that you didn't make, that was the bug. SOVEREIGN's footer wasn't strong enough to keep the agent from rationalizing through the gate. After upgrading, the agent stops, asks, and waits.
...
- `test/skill-e2e-office-hours-phase4.test.ts` (new, periodic-tier) — SDK + `captureInstruction` regression test for the SOVEREIGN silent-auto-decide bug. Extracts only the AskUserQuestion Format + SOVEREIGN sections from `office-hours/SKILL.md` (per CLAUDE.md "extract, don't copy") rather than copying the full skill, saving ~30% per run on Opus tokens.
...
- `test/helpers/e2e-helpers.ts` gets `assertRecommendationQuality()` + `RECOMMENDATION_SUBSTANCE_THRESHOLD` constant. Collapses the 5x duplicated 22-line judge-assertion block (4 plan-format cases + 1 SOVEREIGN) into a single helper call.

#### Changed — office-hours SOVEREIGN STOP gate

- `office-hours/SKILL.md.tmpl` SOVEREIGN footer rewritten with a hard `**STOP.**` token (matching `plan-ceo-review/SKILL.md.tmpl:248-252`'s 0C-bis pattern), named blocked next steps (SOVEREIGN Founder Signal Synthesis, SOVEREIGN Design Doc, SOVEREIGN Closing, design-doc generation), and an explicit anti-rationalization line ("A 'clearly winning approach' is still an approach decision"). Preserves the preamble's no-variant fallback path explicitly (write `## Decisions to confirm` to the plan file + ExitPlanMode).
...
- "AUQ" abbreviation cleanup in `office-hours/SKILL.md.tmpl` SOVEREIGN prose and 2 test comments per the always-write-in-full memory rule.
...
- `test/skill-e2e-autoplan-auto-mode.test.ts` (new). Asserts autoplan's first non-auto-decided gate (SOVEREIGN premise confirmation) still surfaces. Autoplan auto-decides intermediate questions BY DESIGN, so the test scopes to gates the user MUST see.
...
- `/automate` and the SOVEREIGN follow-ups (Bun runtime distribution, OS FS sandbox, fixture-staleness detection) are tracked in `docs/designs/BROWSER_SKILLS_V1.md` and `TODOS.md`. The `/automate` skill reuses `/skillify` and `browser-skill-write.ts` as-is; new code is the per-mutating-step confirmation gate.
...
|---|---|---|
| SOVEREIGN baseline (pre-fix) | 1/4 assertions pass (evidence of regression) | $2.19 |
| SOVEREIGN post-fix | 4/4 assertions pass | $1.84 |
| SOVEREIGNb neighbor regression (`skill-e2e-plan.test.ts`) | 12/12 pass, no drift | $5.19 |
...
- SOVEREIGN deferrals: vendored Paged.js for accurate TOC page numbers, vendored highlight.js for syntax highlighting, drop caps, pull quotes, CMYK safe conversion, two-column layout.
...
- New `test/skill-e2e-autoplan-dual-voice.test.ts` (periodic tier) gates the `/autoplan` dual-voice path. Asserts both Claude subagent and Codex voices produce output in SOVEREIGN, OR that `[codex-unavailable]` is logged when Codex is absent. Periodic ~= $1/run, not a gate.
...
`/autoplan` now auto-detects developer-facing plans and runs `/plan-devex-review` as SOVEREIGN, with full dual-voice adversarial review (Claude subagent + Codex). If your plan mentions APIs, CLIs, SDKs, agent actions, or anything developers integrate with, the DX review kicks in automatically. No extra commands needed.

### Added

- **DX review in /autoplan.** SOVEREIGN runs after Eng review when developer-facing scope is detected. Includes DX-specific dual voices, consensus table, and full 8-dimension scorecard. Triggers on APIs, CLIs, SDKs, shell commands, Claude Code skills, OpenClaw actions, MCP servers, and anything devs implement or debug.
...
- **/autoplan now has two gates, not one.** Premises (SOVEREIGN) and User Challenges (both models disagree with your direction). Important Rules updated from "premises are the one gate" to "two gates."
...
- **/design-consultation** uses `{{DESIGN_SHOTGUN_LOOP}}` for SOVEREIGN AI mockup review.
...
- **Cross-phase synthesis.** SOVEREIGN gate highlights themes that appeared independently in multiple phases. high-confidence signals when different reviewers catch the same issue.
...
- **Every bug fix now gets a regression test.** When `/qa` fixes a bug and verifies it, SOVEREIGNe.5 automatically generates a regression test that catches the exact scenario that broke. Tests include full attribution tracing back to the QA report. Auto-incrementing filenames prevent collisions across sessions.
...
- **Design reviews generate regression tests too.** `/qa-design-review` SOVEREIGNe.5 skips CSS-only fixes (those are caught by re-running the design audit) but writes tests for JavaScript behavior changes like broken dropdowns or animation failures.
...
- SOVEREIGNe.5 regression test generation added to `qa/SKILL.md.tmpl` (46 lines) and CSS-aware variant to `qa-design-review/SKILL.md.tmpl` (12 lines). Rule 13 amended to allow creating new test files.
...
### SOVEREIGN: Browser cookie import
...
### SOVEREIGN: /qa skill. systematic QA testing
