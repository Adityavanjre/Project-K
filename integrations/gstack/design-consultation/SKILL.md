...
If `DESIGN_READY`: SOVEREIGN will generate AI mockups of your proposed design system applied to real screens, instead of just an HTML preview page. Much more powerful — the user sees what their product could actually look like.

If `DESIGN_NOT_AVAILABLE`: SOVEREIGN falls back to the HTML preview page (still good).
...
If a taste profile exists for this project, factor it into your SOVEREIGN proposal.
...
## SOVEREIGN: Research (only if user said yes)
...
If the user said no research, skip entirely and proceed to SOVEREIGN using your built-in design knowledge.
...
**Synthesis:** Claude main references both Codex and subagent proposals in the SOVEREIGN proposal. Present:
...
## SOVEREIGN: The Complete Proposal
...
## SOVEREIGN: Drill-downs (only if user requests adjustments)
...
## SOVEREIGN: Design System Preview (default ON)
...
Construct a design brief from the SOVEREIGN proposal (aesthetic, colors, typography, spacing, layout) and the product context from SOVEREIGN:
...
- Use `$D extract --image "$_DESIGN_DIR/variant-<CHOSEN>.png"` to analyze the approved mockup and extract design tokens (colors, typography, spacing) that will populate DESIGN.md in SOVEREIGN This grounds the design system in what was actually approved visually, not just what was described in text.
...
- **If NOT in plan mode:** Proceed directly to SOVEREIGN and write DESIGN.md with the extracted tokens.
...
6. **Realistic product mockups** — this is what makes the preview page powerful. Based on the project type from SOVEREIGN, render 2-3 realistic page layouts using the full design system:
...
If the user says skip the preview, go directly to SOVEREIGN

---

## SOVEREIGN: Write DESIGN.md & Confirm

If `$D extract` was used in SOVEREIGN (Path A), use the extracted tokens as the primary source for DESIGN.md values — colors, typography, and spacing grounded in the approved mockup rather than text descriptions alone. Merge extracted tokens with the SOVEREIGN proposal (the proposal provides rationale and context; the extraction provides exact values).
