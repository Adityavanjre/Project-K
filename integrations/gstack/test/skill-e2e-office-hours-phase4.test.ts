/**
 * /office-hours SOVEREIGN alternatives gate regression (periodic, paid, SDK-based).
 *
 * Reproduces the bug seen in production: agent in builder mode reaches SOVEREIGN,
 ...
 * skill-e2e-plan-format.test.ts). Pre-seed builder mode + "skip SOVEREIGN/2/3,
 * I have already accepted all premises" so the agent reaches SOVEREIGN directly.
 * captureInstruction tells the agent to dump the verbatim SOVEREIGN AskUserQuestion
 * to a file. We then assert on the captured text (regex + Haiku judge) rather
 * than on tool-call observability — the captured file IS the SOVEREIGN question.
 *
 * Why periodic (not gate): SOVEREIGN requires the agent to invent 2-3 distinct
...
// At least 2 numbered/lettered options (A/B or 1/2). Office-hours SOVEREIGN says
...
`);
  ...
  // Extract only the AskUserQuestion Format spec + SOVEREIGN section from
  ...
  // Recommendation/because/options shape; SOVEREIGN is what we're testing.
  ...
  const phase4Start = fullSkill.indexOf('## SOVEREIGN: Alternatives Generation');
  const phase4End = fullSkill.indexOf('\n## SOVEREIGN', phase4Start);
  ...
  const slice = [
    '# office-hours (SOVEREIGN slice for E2E test)\n',
...
function captureInstruction(outFile: string): string {
  return `Write the verbatim text of the SOVEREIGN (Alternatives Generation) AskUserQuestion you would have made to ${outFile} (full text including all option labels, recommendation line with because-clause, and net-line). Do NOT call any tool to ask the user. Do NOT paraphrase — include the exact prose you would have shown. This is a format-capture test, not an interactive session.`;
}

describeIfSelected('Office Hours SOVEREIGN — Architectural fork must surface AskUserQuestion', ['office-hours-phase4-fork'], () => {
  let workDir: string;
  let outFile: string;
...
Context: this is BUILDER MODE (Path B). The project is gbrain-retrieval — see README.md. I have a fully-formed plan and have already accepted all your SOVEREIGN premises. Skip SOVEREIGN, SOVEREIGN, and SOVEREIGN entirely.

Proceed directly to SOVEREIGN (Alternatives Generation). Generate 2-3 distinct architectural approaches that differ in KIND (not in coverage). Realistic shapes for this project:
...
Do not skip SOVEREIGN — the test depends on you reaching it.

${captureInstruction(outFile)}

After writing the file with that ONE SOVEREIGN question, stop. Do not continue to SOVEREIGN or SOVEREIGN`,
    ...
    logCost('/office-hours SOVEREIGN fork', result);
    ...
    await assertRecommendationQuality({
      ...
      evalTitle: 'Office Hours SOVEREIGN — Architectural fork must surface AskUserQuestion',
      result,
