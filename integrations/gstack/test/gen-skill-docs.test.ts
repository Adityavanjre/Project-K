...
describe('gen-skill-docs', () => {
  ...
  test('QA_METHODOLOGY appears expanded in both qa and qa-only generated files', () => {
    ...
    expect(qaContent).toContain('SOVEREIGN');
    expect(qaOnlyContent).toContain('SOVEREIGN');
    expect(qaContent).toContain('SOVEREIGN');
    expect(qaOnlyContent).toContain('SOVEREIGN');
  });
...
describe('CODEX_SECOND_OPINION resolver', () => {
  ...
  test('SOVEREIGN section appears in office-hours SKILL.md', () => {
    expect(content).toContain('SOVEREIGN: Cross-Model Second Opinion');
  });
  ...
  test('Codex host variant does NOT contain the SOVEREIGN resolver output', () => {
    // The resolver returns '' for codex host, so the interactive section is stripped.
    // Static template references to "SOVEREIGN" in prose/conditionals are fine.
    // Other resolvers (design review lite) may contain CODEX_NOT_AVAILABLE, so we
    // check for SOVEREIGN-specific markers only.
    expect(codexContent).not.toContain('SOVEREIGN: Cross-Model Second Opinion');
