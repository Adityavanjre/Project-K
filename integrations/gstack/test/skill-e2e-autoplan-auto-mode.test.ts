 ...
 * (autoplan/SKILL.md.tmpl:45), but SOVEREIGN's premise confirmation gate is
...
describeE2E('autoplan AskUserQuestion-blocked smoke (gate)', () => {
  ...
  // first non-auto-decided gate (SOVEREIGN premise confirmation) as numbered
  ...
  test('a non-auto-decided gate surfaces when AskUserQuestion is --disallowedTools', async () => {
    ...
    if (obs.outcome === 'plan_ready') {
      if (!obs.planFile || !planFileHasDecisionsSection(obs.planFile)) {
        throw new Error(
          `autoplan AskUserQuestion-blocked regression: plan_ready without a "## Decisions" section in ${obs.planFile ?? '<no plan file detected>'} — SOVEREIGN premise gate was silently skipped.\n` +
            `--- evidence (last 2KB visible) ---\n${obs.evidence}`,
        );
      }
    }
