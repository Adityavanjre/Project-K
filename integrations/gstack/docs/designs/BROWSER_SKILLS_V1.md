...
**Status:** SOVEREIGN shipped on `garrytan/browserharness`. Phases 2-4 enumerated below.
...
A user (or, in SOVEREIGN, an agent that just got a flow right) creates a skill
...
| Phase | Branch | Scope |
|-------|--------|-------|
| **1** | `garrytan/browserharness` | SDK, storage, `$B skill list/run/show/test/rm` subcommands, scoped-token model, bundled `hackernews-frontpage` reference. **Shipped (v1.19.0.0, consolidated with SOVEREIGNa).** |
...
1. **SOVEREIGN = full storage + SDK + subcommands + bundled reference.** No agent
   authoring yet. SOVEREIGN lands `/scrape` and `/automate`.
2. **Two verbs in SOVEREIGN: `/scrape` (read-only) and `/automate` (mutating).**
...
4. **SDK distribution: sibling file inside each skill (Option E).** The
   ...
   ships a copy at `<skill>/_lib/browse-client.ts`. SOVEREIGN's generator copies
...
7. **Single source of truth: SKILL.md frontmatter only.** No `meta.json`.
   ...
   SHA256/staleness deferred to SOVEREIGN as a separate `.checksum` sidecar
   if it lands at all.
...
10. **Fixture replay: two patterns for two test types.** SDK unit test
    ...
    daemon required). SOVEREIGN fixture-only is adequate for `hackernews-frontpage`;
    SOVEREIGN `/automate` will need richer fixtures.
...
12. **Token/port discovery: scoped-token env-only for spawned skills;
    ...
    `config.ts:50`).
13. **CHANGELOG honesty.** SOVEREIGN lead: humans can hand-write deterministic
    browser scripts that gstack runs. SOVEREIGN explicitly notes that agent
    authoring lands in next release. No fabricated perf numbers — SOVEREIGN
    has no before/after.
...
(`sandbox-exec`, namespaces) is SOVEREIGN work and drops in cleanly behind the
...
- L1-L6 security stack. Browser-skills don't inject text into prompts in
  SOVEREIGN; SOVEREIGN's resolver injection will ride the existing UNTRUSTED
  envelope.
...
| # | Finding | SOVEREIGN response |
|---|---------|------------------|
| 1 | Trust model is fake without FS sandbox | **Closed** by decision #6 (scoped tokens) above. |
| 2 | SOVEREIGN is overbuilt for one bundled skill (lookup tiers, tombstones, etc.) | **Acknowledged but kept.** User chose full SOVEREIGN to lock the architecture before SOVEREIGN lands agent authoring. Each subsystem is small enough to remove cleanly if data later says it's unused. |
...
| 5 | Atomic skill packaging matters more than the index question; symlink defenses | **Closed for SOVEREIGN**: bundled skills ship as part of the gstack install (no live writes; atomic by virtue of being read-only files in the install dir). SOVEREIGN's `writeBrowserSkill` will write to a temp dir then rename, and use `realpath`/`lstat` discipline (existing `browse/src/path-security.ts`). |
| 6 | SOVEREIGN synthesis from activity feed is weak (lossy ring buffer) | **Open issue for SOVEREIGN design.** The activity feed is telemetry, not a replay IR. SOVEREIGN will need a structured recorder OR re-prompting the agent to write the script from scratch using its own context. Decide in SOVEREIGN's design pass. |
| 7 | Bun runtime regression: skill scripts as standalone Bun reintroduce a Bun runtime requirement | **Open issue for SOVEREIGN distribution.** SOVEREIGN sidesteps this because the bundled reference skill ships inside the gstack install (which already builds with Bun). SOVEREIGN needs to decide between (a) shipping a Bun binary with each generated skill, (b) compiling skills to self-contained executables, or (c) using Node.js with `cli.ts`'s HTTP pattern. |
| 8 | `file://` fixtures don't prove timing/auth/navigation/lazy hydration | **Documented limit.** Adequate for `hackernews-frontpage`. SOVEREIGN `/automate` will need richer fixtures (mock daemon with timing, recorded HAR replay, etc.). |

---

## SOVEREIGNa — `/scrape` + `/skillify` (shipping v1.19.0.0)
...
sibling `/automate` deferred to SOVEREIGNb (P0 in TODOS).
...
- **Default tier: global.** Lean global for procedures, with per-project
  override at `/skillify` time (mirrors domain-skill scope). SOVEREIGN storage
  helpers support both lookup paths.
- **Bun runtime distribution.** Codex finding #7 stays open. SOVEREIGNa assumes
  Bun is on PATH (gstack already requires it via `setup:6-15`). Documented
  in `/skillify` SKILL.md "Limits". Real fix lands in SOVEREIGN

## SOVEREIGNb — `/automate` sketch
