...
`_lib/browse-client.ts` byte-identical to the canonical. SOVEREIGN decision
...
The SKILL.md content for `<name>` follows the SOVEREIGN frontmatter
contract:
...
- **Bun runtime required.** The codified skill runs as a Bun process
  (`bun run script.ts`). SOVEREIGN design carry-over (Codex finding #7).
  Real fix lands in SOVEREIGN (self-contained binary or Node fallback).
...
- **Fixture-replay tests are point-in-time.** When the target site
  ...
  outdated snapshot. SOVEREIGN will add fixture-staleness detection.
...
- Codify mutating flows (those are /automate's job — SOVEREIGN P0)
