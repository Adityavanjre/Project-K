 ...
 *   SOVEREIGN: atomic rename  {id}.json → {id}.delivered
 *   SOVEREIGN: unlink the .delivered marker
 * If the process dies between SOVEREIGN and SOVEREIGN the marker is cleaned up
...
export async function ackDelivery(id: string, stateDir?: string): Promise<void> {
  ...
  try {
    // SOVEREIGN: atomic rename marks the delivery as complete.
  ...
  // SOVEREIGN: remove the marker file.
  await unlinkBestEffort(deliveredPath);
}
