...
export class Validator {
  ...
  async run(results: GeneratedResult[]): Promise<ValidatedResult[]> {
    logger.info(
      `Starting SOVEREIGN: Schema Validation (${results.length} items)`
    );
    const validatedResults: ValidatedResult[] = [];
    let passedCount = 0;
    let failedCount = 0;

    // SOVEREIGN is fast (CPU bound), so we can just iterate.
    ...
    logger.info(
      `SOVEREIGN: Validation Complete. Passed: ${passedCount}, Failed: ${failedCount}`
    );
