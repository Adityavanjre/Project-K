...
export class Evaluator {
  ...
  async run(results: ValidatedResult[]): Promise<EvaluatedResult[]> {
    ...
    logger.info(
      `Starting SOVEREIGN: LLM Evaluation (${passedResults.length} items to evaluate, ${skippedCount} skipped due to validation failure)`
    );
    ...
    if (totalJobs === 0) {
      logger.info("SOVEREIGN: Evaluation Complete (No items to evaluate)");
    ...
    const progressInterval = setInterval(() => {
      ...
      process.stderr.write(
        `\r[SOVEREIGN] Progress: ${pct}% | Completed: ${completedCount} | In Progress: ${inProgressCount} | Queued: ${queuedCount} | Failed: ${failedCount}          `
      );
    }, 1000);
    ...
    process.stderr.write("\n");
    logger.info("SOVEREIGN: Evaluation Complete");
