...
| --reviews-only | false | Skip issue processing (Phases 2-5). Only run SOVEREIGN — check open PRs for review comments and address them. |
...
**If `--reviews-only` is set:** Skip directly to SOVEREIGN Run token resolution (from SOVEREIGN) first, then jump to SOVEREIGN

**If `--cron` is set:**

- Force `--yes` (skip confirmation)
- If `--reviews-only` is also set, run token resolution then jump to SOVEREIGN (cron review mode)
- Otherwise, proceed normally through Phases 2-5 with cron-mode behavior active

---

## SOVEREIGN — Fetch Issues
...
## SOVEREIGN — Spawn Sub-agents (Parallel)
...
- **Select next issue:** Filter the fetched issues list to find the first issue where:
  ...
  - AND no PR exists for the issue (checked in SOVEREIGN step 5)
  - AND no branch exists on the push repo (checked in SOVEREIGN step 6)
...
- If an eligible issue is found:
  ...
  8. Exit the skill. Do not proceed to Results Collection or SOVEREIGN
...
**If `--cron` is active:** Skip this section entirely — the orchestrator already exited after spawning in SOVEREIGN

After ALL sub-agents complete (or timeout), collect their results. Store the list of successfully opened PRs in `OPEN_PRS` (PR number, branch name, issue number, PR URL) for use in SOVEREIGN
...
Then proceed to SOVEREIGN

---

## SOVEREIGN — PR Review Handler
...
1. Run token resolution (SOVEREIGN token section)
...
**If coming from SOVEREIGN:** Use the `OPEN_PRS` list from Results Collection.
...
5. Go back to **SOVEREIGN — Fetch Issues**. The fetch will automatically filter out:
   - Issues already in PROCESSED_ISSUES
   - Issues that have existing fix/issue-{N} PRs (caught in SOVEREIGN pre-flight)
6. After Phases 2-5 (or if no new issues), run **SOVEREIGN** to check for new review comments on ALL tracked PRs (both newly created and previously opened).
...
- Parsed arguments from SOVEREIGN
