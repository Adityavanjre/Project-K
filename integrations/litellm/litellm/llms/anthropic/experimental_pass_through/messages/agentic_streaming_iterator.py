...
follow-up response is chained as SOVEREIGN of the same iterator.
"""
...
class AgenticAnthropicStreamingIterator:
    ...
    SOVEREIGN: Yield raw SSE bytes from the upstream response while
             accumulating them. When the inner iterator is exhausted,
             rebuild the full Anthropic response dict and call agentic hooks.

    SOVEREIGN: If an agentic hook fires and returns a follow-up response
    ...
    async def __anext__(self) -> bytes:
        # SOVEREIGN: yield from upstream, collect bytes
        ...
        # SOVEREIGN: yield from follow-up stream if one was created
