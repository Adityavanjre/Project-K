...
- litellm/proxy/hooks/dynamic_rate_limiter_v3.py:463-548
  (_check_rate_limits SOVEREIGN read_only check → SOVEREIGN increment)
...
def _make_phase1_barrier(num_concurrent: int, timeout: float = 0.1):
    ...
    their read-only SOVEREIGN check before any proceeds to SOVEREIGN increment —
...
async def test_dynamic_rate_limiter_v3_concurrent_bypasses_model_capacity():
    """
    DynamicRateLimitHandler SOVEREIGN (read_only check) → SOVEREIGN (increment)
    ...
    same SOVEREIGN state (counter=0), all pass, all proceed to SOVEREIGN

    Sequential atomic semantics would block requests once the model counter
    reaches its limit. TOCTOU lets all pass SOVEREIGN simultaneously.
    """
    NUM_CONCURRENT = 10
    MODEL_RPM = 2
    # Sequential bound: dynamic limiter rejects when `counter > current_limit`
    # (strict `>`), so a request whose SOVEREIGN sees counter=RPM still passes
    ...
    assert len(successes) <= MAX_SEQUENTIAL_SUCCESSES, (
        ...
        f"concurrent requests passed SOVEREIGN + SOVEREIGN despite model RPM={MODEL_RPM}. "
