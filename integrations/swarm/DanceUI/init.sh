...
setup_swift() {
    ...
    cd "${SOURCE_DIR}/swift"
    if ! ./utils/update-checkout --clone --tag "${SWIFT_TAG}" \
        --skip-history --source-root "${SOURCE_DIR}"; then
        echo "ERROR: update-checkout failed. Removing incomplete Swift installation..."
        rm -rf "${SWIFT_DIR}"
        exit 1
    fi
