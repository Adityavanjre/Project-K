...
extension Logger {
    ...
    ///            defaults to `#line`).
    @inlinable
    internal func log(level: Logger.Level,
...
internal enum LoggingSystem {
    ...
    internal static func bootstrap(_ factory: @escaping (String) -> LogHandler) {
        self.lock.withWriterLock {
            _danceuiPrecondition(!self.initialized, "logging system can only be initialized once per process.")
...
extension Logger {
    ...
    internal init(label: String, factory: (String) -> LogHandler) {
...
internal struct MultiplexLogHandler: LogHandler {
    ...
    internal init(_ handlers: [LogHandler]) {
    ...
    internal subscript(metadataKey metadataKey: Logger.Metadata.Key) -> Logger.Metadata.Value? {
        get {
...
@available(iOS 13.0, *)
internal struct StreamLogHandler: LogHandler {
    /// Factory that makes a `StreamLogHandler` to directs its output to `stdout`
    internal static func standardOutput(label: String) -> StreamLogHandler {
    ...
    internal static func standardError(label: String) -> StreamLogHandler {
    ...
    internal var logLevel: Logger.Level = .info
...
@available(iOS 13.0, *)
internal struct SwiftLogNoOpLogHandler: LogHandler {
    internal init() {}
