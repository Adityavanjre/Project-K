...
///
/// ![A diagram that shows a grey box on the left, the DanceUI icon on the
...
///
/// For this example, DanceUI shows a ``ProgressView`` first, and then the
/// image scaled to fit in the specified frame:
///
/// ![A diagram that shows a progress view on the left, the DanceUI icon on the
...
///
@available(iOS 13.0, *)
public struct AsyncImage<Content> : View, PrimitiveView where Content : View {
    

    internal var url: URL?

    internal let alternativeURLs: [URL]

    internal var scale: CGFloat

    internal var transaction: Transaction

    internal var content: (AsyncImagePhase, AsyncImageEventContext?) -> Content

    internal var progress: AsyncImageProgress?
    
    /// Loads and displays an image from the specified URL.
    ///
    /// Until the image loads, DanceUI displays a default placeholder. When
    /// the load operation completes successfully, DanceUI updates the
    /// view to show the loaded image. If the operation fails, DanceUI
    ...
    ///
    /// Until the image loads, DanceUI displays the placeholder view that
    /// you specify. When the load operation completes successfully, DanceUI
    ...
    ///     }
    ///
    /// If the load operation fails, DanceUI continues to display the
...
@available(iOS 13.0, *)
internal struct LoadingError : Error {
...
/// the appearance of the view using a `content` closure. DanceUI calls the
