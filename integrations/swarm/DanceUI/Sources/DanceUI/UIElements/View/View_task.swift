...
extension View {
    ...
    /// before DanceUI removes the view or the view changes identity, DanceUI
    /// cancels the task.
    ///
    ...
    ///   - action: A closure that DanceUI calls as an asynchronous task
    ///     before the view appears. DanceUI will automatically cancel the task
    ...
    /// chooses a different server, DanceUI cancels the task and creates a new
    ...
    ///     [userInitiated](https://developer.apple.com/documentation/swift/taskpriority/userinitiated).
    ///   - action: A closure that DanceUI calls as an asynchronous task
    ///     before the view appears. DanceUI can automatically cancel the task
    ///     after the view disappears before the action completes. If the
    ///     `id` value changes, DanceUI cancels and restarts the task.
    ///
    /// - Returns: A view that runs the specified action asynchronously before
...
public struct _TaskModifier: ViewModifier {
    ...
    nonisolated public static func _viewListCount(
        ...
        body: @escaping (_ViewListCountInputs) -> Int? // FIXME: ViewModifier _viewListCount should not be @escaping
    ) -> Int? {
    ...
    private struct InnerModifier: ViewModifier {
        ...
        func body(content: Content) -> some View {
            ...
            .onChange(of: base.value) {
                guard let taskState,
                      taskState.value != base.value else {
                    return
                }
                taskState.task.cancel()
                let action = base.action
