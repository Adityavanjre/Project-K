...
extension Sequence where Element: Comparable {
    ...
    @inlinable
    internal func sorted(stable: Bool) -> [Element] {
        return sorted(stable: stable, by: <)
    }
}
...
extension Sequence {
    ...
    /// - Complexity: O(*n* log *n*), where *n* is the length of the sequence.
    @inlinable
    internal func sorted(stable: Bool,
        by areInIncreasingOrder:
        (Element, Element) throws -> Bool
    ) rethrows -> [Element] {
        var result = ContiguousArray(self)
        try result.sort(stable: stable, by: areInIncreasingOrder)
        return Array(result)
    }
}
...
where Self: RandomAccessCollection, Element: Comparable {
    ...
    @inlinable
    internal mutating func sort(stable: Bool) {
        sort(stable: stable, by: <)
    }
}
...
extension UnsafeMutableBufferPointer {
    ...
    @inlinable
    internal mutating func _stableSortImpl(
        by areInIncreasingOrder: (Element, Element) throws -> Bool
    ) rethrows {
        ...
        _ = try Array<Element>(unsafeUninitializedCapacity: count / 2) {
            buffer, _ in
