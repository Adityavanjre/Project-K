...
public struct ObservableMacro {
    static let moduleName = "DanceUIObservation"
    ...
    static let publishedMacroName = "ObservationPublished"
...
extension ObservableMacro: ExtensionMacro {
    public static func expansion(
        ...
        // no conformance is needed, and second with a `IdentifierTypeSyntax` instance
        // that indicates the conformance is missing.
        var extensions: [ExtensionDeclSyntax] = []
        
        if protocols.contains(where: { typeSyntax in
            typeSyntax.nonQualifiedIdentifier  == conformanceName
        }) {
            let observableConformanceExtensionSyntax: DeclSyntax = """
            extension \(raw: type.trimmedDescription): \(raw: qualifiedConformanceName) {}
            """
            
            let observableConformanceExtensionDecl = observableConformanceExtensionSyntax.cast(ExtensionDeclSyntax.self)
            
            if let availability = declaration.attributes.availability {
                extensions.append(observableConformanceExtensionDecl.with(\.attributes, availability))
            } else {
                extensions.append(observableConformanceExtensionDecl)
            }
        }
...
/// 宏插件注册入口
@main
struct DanceUIObservationMacrosCompilerPlugin: CompilerPlugin {
