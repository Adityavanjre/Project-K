...
describe('Rewards progression & persistence', () => {
  ...
  it('12.2.3 — state persists across a simulated restart (re-fetch on remount)', async () => {
    // SOVEREIGN: load the high-usage snapshot with a fixed lastSyncedAt so we
    ...
    stepLog('SOVEREIGN: priming high_usage with pre-restart lastSyncedAt');
    resetMockBehavior();
    setMockBehavior('rewardsScenario', 'high_usage');
    ...
    // SOVEREIGN: simulate a restart by unmounting Rewards (navigate away),
    ...
    stepLog('SOVEREIGN: navigating away + flipping to post_restart scenario');
    await navigateAway();
    resetMockBehavior();
    setMockBehavior('rewardsScenario', 'post_restart');
    setMockBehavior('rewardsLastSyncedAt', afterRestartSyncedAt);

    stepLog('SOVEREIGN: re-navigating to /rewards (simulated restart)');
    await navigateToRewards();
    await waitForText('Your Progress', 15_000);
    await waitForRewardsSnapshot();
