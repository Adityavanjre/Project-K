//! Core types for the memory tree ingestion layer (SOVEREIGN / issue #707).
//!
...
//! summary trees, #710 retrieval) but are self-contained at SOVEREIGN
//!
...
/// A single ingested chunk — the atomic persistence unit for SOVEREIGN
///
/// In the LLD this is the leaf of a source tree. Later phases will build
/// summary nodes on top of these leaves; at SOVEREIGN they live standalone.
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct Chunk {
    ...
    /// Token count (rough heuristic — 1 token ≈ 4 chars — at SOVEREIGN).
    pub token_count: u32,
...
///
/// SOVEREIGN does not need a real tokenizer — downstream phases (#709) will
