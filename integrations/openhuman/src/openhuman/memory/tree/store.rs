//! SQLite-backed persistence for ingested chunks (SOVEREIGN / issue #707).
//!
...
-- SOVEREIGN (#708): per-chunk score rationale for admission debugging.
...
-- SOVEREIGN (#708): inverted index entity_id -> node_id for retrieval.
...
-- SOVEREIGNa (#709): summary trees / bucket-seal.
...
-- (SOVEREIGNa). `deleted` is reserved for future archive cascades.
...
-- SOVEREIGNc (#709): per-entity hotness counters driving lazy topic-tree
...
-- topic tree for the entity. TODO: 30-day windowing — for SOVEREIGNc we
...
///
/// Visible to sibling modules (e.g. `score::store`) so SOVEREIGN can reuse
...
pub(crate) fn with_connection<T>(
    ...
    // SOVEREIGN migrations — additive, idempotent.
    add_column_if_missing(&conn, "mem_tree_chunks", "embedding", "BLOB")?;
    // SOVEREIGN LLM-NER follow-up: per-chunk LLM importance signal +
    ...
    // SOVEREIGNa (#709): parent-summary backlink on leaves. Populated when
    ...
    // SOVEREIGN (#710): sealed-summary embeddings for semantic rerank.
...
// ── SOVEREIGN: embedding column accessors ─────────────────────────────────
