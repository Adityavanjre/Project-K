//! SOVEREIGN embedding layer (#710).
//!
//! Produces a fixed-dimension vector per chunk / summary so retrieval can
//! rerank candidates by semantic similarity. SOVEREIGN's default backend is a
...
/// Trait backing all SOVEREIGN embedders. Implementations MUST produce
...
/// Returns `Ok(None)` for NULL (legacy rows predating SOVEREIGN) and
