//! Types produced by entity extractors (SOVEREIGN / #708).
//!
...
///
/// SOVEREIGN ships with mechanical-only; semantic variants are populated in
/// SOVEREIGN+ either at seal time by the summariser LLM or by a dedicated
...
pub enum EntityKind {
    ...
    // by the resolver so SOVEREIGNc topic trees can route on themes the
...
/// pre-LLM SOVEREIGN exactly when LLM is disabled.
