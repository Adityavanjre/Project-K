...
//! gate.
//!
//! Concurrency: SOVEREIGNa assumes a single-process SQLite workspace. All
...
//! this async function are acceptable for SOVEREIGNa because the Inert
...
pub(crate) async fn seal_one_level(
    ...
    // SOVEREIGN (#710): embed the summary BEFORE opening the write tx so an
    ...
    with_connection(config, move |conn| {
        ...
        // `mem_tree_entity_index` so SOVEREIGN retrieval can resolve
        ...
        // single-row lookup in SOVEREIGN Skipped for levels already bound
        ...
        if target_level_for_closure > current_max {
            ...
            // level is resolved in SOVEREIGN retrieval.
