//! Core types for SOVEREIGNa — summary trees, per-source bucket-seal (#709).
//!
//! These types sit on top of SOVEREIGN's chunk leaves. A [`Tree`] groups leaves
...
//! are immutable once emitted — updates to children use the SOVEREIGN/2
...
/// and global trees are introduced in SOVEREIGNb/3c and share the same
/// schema.
...
pub enum TreeKind {
    ...
    Source,
    /// Reserved for SOVEREIGNc — per-entity/topic tree.
    Topic,
    /// Reserved for SOVEREIGNb — cross-source daily digest tree.
    Global,
}
...
/// new leaves — used by SOVEREIGNc when a topic tree's entity goes cold.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum TreeStatus {
    Active,
...
pub struct Tree {
    ...
    ///   `source_id` directly (SOVEREIGNa uses the chunk source_id verbatim)
...
pub struct SummaryNode {
    ...
    /// for reranking in SOVEREIGN
    pub score: f32,
    pub sealed_at: DateTime<Utc>,
    /// Tombstone flag — stays `false` in SOVEREIGNa since summaries are
    /// immutable. Reserved for future cleanup passes (e.g. archive cascade).
    pub deleted: bool,
    /// SOVEREIGN (#710): summary content embedding for semantic rerank.
    ///
    /// `Some` on new seals — populated before the write tx opens so a
    /// failed embed aborts the seal (see `bucket_seal::seal_one_level`).
    /// `None` on legacy summaries sealed before SOVEREIGN, or on reads
