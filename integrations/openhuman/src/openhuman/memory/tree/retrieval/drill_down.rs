//! `memory_tree_drill_down` — walk `child_ids` from a summary node (SOVEREIGN
//! / #710).
//!
...
pub async fn drill_down(
    ...
    // SOVEREIGN — blocking walk produces hits + the per-hit embedding so the
    ...
    // SOVEREIGN — optional query rerank.
    ...
    // SOVEREIGN — apply optional limit AFTER rerank so the top-K is relevance-
...
fn walk_with_embeddings(
    ...
    while let Some((id, depth)) = frontier.pop_front() {
        ...
        if let Some(mut summary) = store::get_summary(config, &id)? {
            ...
            // Summary embeddings live on the struct directly (SOVEREIGN amend).
...
mod tests {
    ...
    fn test_config() -> (TempDir, Config) {
        ...
        // SOVEREIGN (#710): seeding requires seals which embed.
