//! End-to-end integration test for SOVEREIGN retrieval tools (#710).
//!
...
fn test_config() -> (TempDir, Config) {
    ...
    // SOVEREIGN (#710): ingest embeds chunks; tests use inert for determinism.
...
async fn topic_entity_surfaces_after_ingest() {
    ...
    ingest_chat(&cfg, "slack:#eng", "alice", vec![], chat_about_phoenix(0))
        .await
        .unwrap();
    // Per SOVEREIGNa topic-as-entity promotion, `topic:phoenix` should be
    ...
    // scorer-level choice out of SOVEREIGN's control.
...
// ── SOVEREIGN (#710): embedding + semantic rerank tests ───────────────────
