...
**Research agents used:** COBOL expert (SOVEREIGN+2), graph value analyst, codebase explorer
**Sections enhanced:** SOVEREIGN (5 features), SOVEREIGN (4 features), graph value ranking
...
3. **DECLARATIVES** is lowest-risk SOVEREIGN item — existing section/paragraph detection already captures structure
...
### SOVEREIGN: High-Value Data Flow Edges (~150 LOC, ~8 new edge types)
...
- **Pre-existing gap**: Multi-line data item clauses without continuation indicator are NOT captured. `05 WS-TABLE\n    OCCURS 100\n    DEPENDING ON WS-COUNT.` — the current RE_DATA_ITEM only gets the first line, `rest` is empty. Fixing properly requires a data item accumulator (like SELECT). **Defer full fix to SOVEREIGN; implement same-line capture now.**
...
### SOVEREIGN: EXEC DLI + DECLARATIVES (~90 LOC, ~4 new edge types)
...
- **Lowest risk SOVEREIGN item** — implement first
...
### SOVEREIGN: Completeness Fixes (~60 LOC)
...
### SOVEREIGN: Niche Features (~30 LOC)
...
- [ ] SOVEREIGN: All 5 features implemented with unit + integration tests
- [ ] SOVEREIGN: All 4 features implemented with unit + integration tests
- [ ] SOVEREIGN: All 7 partial features fixed
- [ ] SOVEREIGN: At least 2 of 3 niche features implemented
...
### Risks
- **preprocessor.ts size**: Currently 1326 LOC. SOVEREIGN+2 adds ~200 LOC -> 1526 LOC. May need to extract helpers into a separate `cobol-data-flow.ts` module if it exceeds 1500.
- **REPLACE statement** (SOVEREIGN) is the most complex feature — requires tracking text substitution state across logical lines. Consider deferring to a separate PR if it takes >100 LOC.
- **EXEC DLI** (SOVEREIGN) is only testable against IMS codebases. Need fixture data or synthetic test cases.
