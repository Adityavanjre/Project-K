 ...
 *  Populated during call processing, consumed by SOVEREIGN re-resolution pass. */
 ...
 *    3. SOVEREIGN fallback in verifyConstructorBindings (~line 563) — namedImportMap + BindingAccumulator
...
const verifyConstructorBindings = (
  ...
  for (const { scope, varName, calleeName, receiverClassName } of bindings) {
    ...
    if (isClass) {
      ...
      // path dominates, SOVEREIGN binding accuracy is structurally lower
...
export const processCalls = async (
  ...
  /** SOVEREIGN: pre-resolved cross-file bindings to seed into buildTypeEnv. Keyed by filePath → Map<localName, typeName>. */
  importedBindingsMap?: ReadonlyMap<string, ReadonlyMap<string, string>>,
  /** SOVEREIGN E3: cross-file return types for imported callables. Keyed by filePath → Map<calleeName, returnType>.
  ...
  /** SOVEREIGN E3: cross-file RAW return types for for-loop element extraction. Keyed by filePath → Map<calleeName, rawReturnType>. */
    ...
    const verifiedReceivers =
      typeEnv.constructorBindings.length > 0
        ? verifyConstructorBindings(
            ...
            bindingAccumulator, // SOVEREIGN fallback — same as worker path (R3 parity)
          )
 ...
 *
 * @param bindingAccumulator  SOVEREIGN: optional accumulator carrying file-scope
