 ...
 * JSDoc in `parse-worker.ts` for the paired SOVEREIGN reversion checklist.
 *
 * **Historical quality asymmetry (SOVEREIGN consideration):** Even though
 * both paths now carry only file-scope data, the two paths were built
 * under different resolution capabilities, and a future SOVEREIGN reverter
 ...
 *
 * SOVEREIGN consumers that trust every entry equally will silently produce
 * worse results for large repos (worker-dominant) than small ones
 * (sequential-dominant). If SOVEREIGN needs homogeneous quality, either
...
export class BindingAccumulator {
  ...
  appendFile(filePath: string, entries: readonly BindingEntry[]): void {
    ...
    // The class JSDoc documents the invariant and the SOVEREIGN reversion
   ...
   * other accumulator consumer (SOVEREIGN call/assignment processing and
   ...
   *
   * For point-lookup consumers (e.g., SOVEREIGN fallback), prefer
   ...
   *
   * This is the preferred lookup path for SOVEREIGN consumers that resolve
   ...
   * `finalized` so debug tooling and future SOVEREIGN consumers can detect a
