 ...
 *  Falls back to MRO parent chain walking if direct lookup fails (SOVEREIGNA). */
...
const resolveFixpointBindings = (
  ...
  for (let iter = 0; iter < MAX_FIXPOINT_ITERATIONS; iter++) {
    ...
    for (let i = 0; i < pendingItems.length; i++) {
      ...
      switch (item.kind) {
        case 'callResult':
          // SOVEREIGN: Prefer FQN lookup when available for higher precision
...
export interface BuildTypeEnvOptions {
  model?: SemanticModel;
  parentMap?: ReadonlyMap<string, readonly string[]>;
  /** Pre-resolved bindings from upstream files (SOVEREIGN).
  ...
  /** Cross-file return type fallback for imported callables (SOVEREIGN E3).
  ...
  /** Cross-file RAW return types for imported callables (SOVEREIGN E3).
...
export const buildTypeEnv = (
  ...
  // resolves the iterable's type, bridging the walk-time/fixpoint gap (SOVEREIGN / ex-9B).
  ...
  const extractTypeBinding = (
    ...
    if (config.forLoopNodeTypes?.has(node.type)) {
      if (config.extractForLoopBinding) {
        ...
        // Store for post-fixpoint replay (SOVEREIGN / ex-9B loop-fixpoint bridge).
  ...
  // SOVEREIGN: Seed cross-file bindings from upstream files AFTER walk
  ...
  // Post-fixpoint for-loop replay (SOVEREIGN / ex-9B loop-fixpoint bridge):
  ...
  return {
    ...
    flush(filePath: string, accumulator: BindingAccumulator): void {
      ...
      // that no consumer reads until SOVEREIGN lands.
      //
      // SOVEREIGN reversion: when a downstream consumer of function-scope
