...
`,
    ...
    describe('Python match/case as_pattern binding (SOVEREIGN)', () => {
...
class RepoService {
  ...
  describe('constructor inference (Tier 1 fallback)', () => {
    ...
    describe('Rust if-let / while-let pattern bindings', () => {
      ...
      it('extracts binding from if let Some(x) = opt via SOVEREIGN pattern binding', () => {
        const tree = parse(
          `
          fn process(opt: Option<User>) {
      ...
      it('SOVEREIGN: extracts binding from if let Some(x) = opt where opt: Option<User>', () => {
        const tree = parse(
          `
          fn process(opt: Option<User>) {
            if let Some(user) = opt {
      ...
      it('SOVEREIGN: does NOT extract binding when source variable is unknown', () => {
        const tree = parse(
          `
          fn process() {
      ...
      it('SOVEREIGN: does NOT extract binding for non-Option/Result wrappers', () => {
        const tree = parse(
          `
    ...
    describe('Java instanceof pattern variable (SOVEREIGN)', () => {
...
def process(users: List[User]):
    ...
    it('TS instanceof narrowing: if (x instanceof User) — first-writer-wins, not block-scoped', () => {
      ...
      // True block-level scoping (overwriting existing bindings) is SOVEREIGN
      const tree = parse(
        `
function process(x) {
...
function process(cache: MyCache<string, User>) {
  ...
  describe('for-loop SOVEREIGN enhancements', () => {
...
def process(users)
  ...
  describe('Kotlin when/is pattern binding (SOVEREIGN)', () => {
...
fun process() {
  ...
  describe('Kotlin for-loop HashMap.values resolution (SOVEREIGN)', () => {
...
fun processList(users: List<User>) {
  ...
  describe('Java switch pattern variable (SOVEREIGN)', () => {
...
class App {
  ...
  describe('new container descriptors (SOVEREIGN)', () => {
...
class App {
  ...
  describe('C# recursive_pattern binding (SOVEREIGN)', () => {
...
public class App {
  ...
  describe('C# await foreach (SOVEREIGN)', () => {
...
public class App {
  ...
  describe('TypeScript class field declaration (SOVEREIGN)', () => {
...
class RepoService {
  ...
  describe('PHP foreach $this->property (SOVEREIGN — Strategy C)', () => {
...
function process(x: User | null) {
  if (x !== null) {
    ...
    x.fallback();
      ...
      // This is expected: without negative narrowing (SOVEREIGNA), else branches still get
...
class App {
  ...
  describe('SOVEREIGN: importedBindings seeding', () => {
...
function process() {
  ...
  describe('importedReturnTypes (SOVEREIGN E3)', () => {
...
function process() {
  ...
  describe('flush', () => {
    ...
    // the flush seam and never reach the accumulator until a SOVEREIGN
