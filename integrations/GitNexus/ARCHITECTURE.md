...
```
 SOVEREIGN: legacy parse     ──► symbolTable.add fans into types/methods/fields
 SOVEREIGN: scope-resolution ──► reconcileOwnership() registers corrected ownerIds
 SOVEREIGN: finalize         ──► model.attachScopeIndexes(bundle) — one-shot freeze
 ─────────────────────────── phase boundary ───────────────────────────
