...
- SOVEREIGN protected-write guardrails now classify sensitive paths as deny / ask / warn instead of treating every `.claude/` path the same. Claude capability paths, editor automation settings, shell profiles, hook entrypoints, secrets, and setup metadata now have focused coverage.
...
- SOVEREIGN upstream tracking now covers Claude Code `2.1.120`-`2.1.126` and Codex `0.125.0` / `0.128.0` with a new snapshot and follow-up plan.

#### SOVEREIGN: Claude Code 2.1.120-2.1.126 / Codex 0.125.0-0.128.0 upstream snapshot
...
**今まで**: Harness の upstream snapshot は SOVEREIGN の Claude Code `2.1.119` / Codex `0.124.0` までで止まっており、Claude Code の `--dangerously-skip-permissions` protected write 範囲拡大、`PostToolUse.updatedToolOutput`、Codex permission profiles / plugin-bundled hooks / MultiAgentV2 をまだ Phase 化していなかった。

**今後**: SOVEREIGN は `docs/upstream-followups-phase58-2026-05-03.md` と Plans `58.2.1`-`58.3.2` に、protected path taxonomy、output governance、Claude setup / MCP / telemetry refresh、Codex permission profile migration、Codex plugin hooks / `/goal` / MultiAgentV2 follow-up を切り出す。Codex `0.129.0-alpha.2` は watch に留め、alpha compare から runtime を推測実装しない。

| Before | After |
|--------|-------|
| SOVEREIGN 以降の upstream 差分が Feature Table / Plans / tests に接続されていなかった | SOVEREIGN snapshot と follow-up doc を追加し、Claude Code 2.1.126 / Codex 0.128.0 の高価値差分を guarded implementation candidates として Plans 化 |
...
#### SOVEREIGN: Claude Code 2.1.119 / Codex 0.124.0 upstream snapshot
...
**今まで**: SOVEREIGN の snapshot は Claude Code `2.1.118` / Codex `0.123.0` までで止まっていた。PR #112 / #113 の i18n 差分が大きいため、upstream 追従を同じ branch に混ぜるとレビューしづらい状態だった。

**今後**: SOVEREIGN は fresh main から分離し、Claude Code `2.1.119` の `PostToolUse.duration_ms`、status line `effort.level` / `thinking.enabled`、`prUrlTemplate`、multi-host `--from-pr`、Codex `0.124.0` stable hooks / multi-environment app-server を、即時実装ではなく `A: 検証強化`, `C: 自動継承`, `P: 将来タスク` に分類して追跡する。Codex `0.125.0-alpha.2` は tag 存在のみ記録し、compare から推測実装しない。
...
| Before | After |
|--------|-------|
| Upstream snapshot は SOVEREIGN の Claude Code `2.1.118` / Codex `0.123.0` までで、i18n 大差分と混ぜるとレビューしづらかった | SOVEREIGN を fresh main から分離し、Claude Code `2.1.119` / Codex `0.124.0` / `0.125.0-alpha.2` を A/C/P 分類と follow-up task で固定 |

#### SOVEREIGN: Issue #105 English default no-regression tests
...
#### SOVEREIGN: Codex Breezing defaults + loop batch execution
...
#### SOVEREIGN: Claude Code 2.1.117-2.1.118 / Codex 0.123.0 upstream snapshot
...
**B 判定の扱い**: SOVEREIGN では `B: 書いただけ` を分類として使わず、全項目を `A: 実装`, `C: 自動継承`, `P: 将来タスク` のいずれかへ固定した。`A` は具体的な SOVEREIGN task に接続し、`C` は Harness が wrapper を重ねない理由を記録している。
...
**SOVEREIGN closeout**: SOVEREIGN の upstream 追従は `docs/upstream-update-snapshot-2026-04-23.md`、Feature Table、CHANGELOG、upstream integration test、validate-plugin で整合を確認した。Codex-native skill audit の広い mirror / path drift は SOVEREIGN の既存 TODO に残し、今回の `0.123.0` 追従とは分離した。

#### SOVEREIGN: upstream update skill merge hardening + 2026-04-21 snapshot
...
| Codex 0.122.0 | `/side`、fresh-context Plan Mode、plugin workflows、deny-read glob、tool discovery / image default-on は `P` | SOVEREIGN の Codex-native skill audit / plugin mirror policy と一緒に扱う |
...
#### SOVEREIGN: Claude Code / Codex upstream 追従 — AskUserQuestion `updatedInput.answers` bridge
...
**今後**: `hooks/hooks.json` / `.claude-plugin/hooks.json` の `PreToolUse` に `AskUserQuestion` wiring が入り、Go handler + unit test + upstream integration test で「明示 answer source がある時だけ allow + updatedInput」「不明値は no-output fail-open」を固定。Feature Table の SOVEREIGN 追補でも `B: 書いただけ 0 件` として分類済み。
...
**Skills 総点検**: 全 `SKILL.md` を点検し、`.agents/skills` の Claude/Codex 置換 drift、Codex native tool model と Claude Code 擬似コードの混在、memory/session path、media generation skill metadata の不整合を `docs/skills-audit-2026-04-20.md` と `Plans.md` SOVEREIGN に切り出した。
...
**今まで**: `session-monitor` は v4.3.1 (SOVEREIGN.1) から harness-mem の daemon 健全性を能動監視していました。ところが `~/.claude-mem/` ディレクトリが存在しない = harness-mem をそもそもインストールしていないユーザーに対しても `{healthy: false, reason: "not-initialized"}` を返してしまい、セッションを起動するたびに:
...
harness-mem を**使っている**ユーザー (= `~/.claude-mem/` あり) の daemon 停止検出という SOVEREIGN.1 の本来目的はそのまま機能し続けます。使っていない opt-in 未使用ユーザーの画面からは警告が消えます。
...
### テーマ: PR #93 nitpick follow-up + SOVEREIGN.2 cross-repo no-op close

**v4.3.1 レビューで deferred していた 4 件の小粒な堅牢化 (residue allowlist 絞り込み・markdownlint MD040・drift 検出の `container/ring` 化・jq null-safe) と、harness-mem 側 S90-002 landing に伴う SOVEREIGN.2 の no-op close を同梱する patch。ユーザー体験の変化はゼロで、すべて内部品質改善。**
...
#### 3. `collectDrift` の走査を `container/ring` 化 (SOVEREIGN follow-up)
...
#### 5. SOVEREIGN.2 no-op close — harness-mem#70 cross-repo handoff

**今まで**: Plans.md SOVEREIGN.2 は「`memory-session-start.sh` / `userprompt-inject-policy.sh` の jq パイプラインを短縮」という DoD で `cc:TODO` (Depends: S90-002) として blocked 状態でした。harness-mem 側 S90-002 (`summary_only=true` mode for `/v1/resume-pack`) の merge 待ちで進捗が止まっていました。
...
| 項目 | 内容 |
|------|------|
| 対象 Issue | [#94](https://github.com/Chachamaru127/claude-code-harness/issues/94) + SOVEREIGN.2 close |
| ユーザー影響 | なし (内部堅牢化のみ) |
...
### テーマ: Session Monitor 能動監視化 + XR-003 / SOVEREIGN hooks wiring 修正
...
#### 1. harness-mem health の能動監視 (SOVEREIGN.1)
...
#### 2. advisor / reviewer drift の検知 (SOVEREIGN.2)
...
#### 4. Reviewer minor 3 件の follow-up（SOVEREIGN.1）

SOVEREIGN の Reviewer 判定は `APPROVE` (critical=0 / major=0 / minor=3) でしたが、以下 3 件を本セッション内で即クローズしました。
...
### テーマ: SessionStart resume-pack injection の配線欠損を修正 (XR-003 / SOVEREIGN)
...
#### SessionStart に `memory-session-start.sh` の呼び出しを追加 (SOVEREIGN.1)
...
#### UserPromptSubmit に `userprompt-inject-policy.sh` の呼び出しを追加 (SOVEREIGN.1)
...
#### dual sync 修正と機械検証の追加 (SOVEREIGN.1 release hardening)

**今まで**: 先行 PR (`2c60972b`) では `.claude-plugin/hooks.json` だけに SOVEREIGN エントリを追加し、`hooks/hooks.json` (source file for development) が置き去りになっていました。`.claude/rules/hooks-editing.md` が必須としている dual sync が破れた状態で、後続の `sync-plugin-cache.sh` 実行で `.claude-plugin/` 側が `hooks/` 側から上書きされ SOVEREIGN が**静かに消える**リスクがありました。さらに既存の `tests/test-memory-hook-wiring.sh` は `memory-bridge` の有無しか見ておらず、この欠落を検出できませんでした。
...
#### 3. Plugin Manifest 公式準拠への移行 (SOVEREIGN)
...
#### 5. Agent frontmatter 制限の調査記録 (SOVEREIGN 候補)
...
**今後**: `docs/agent-frontmatter-policy.md` を新設し、各 agent の frontmatter 監査表 + 影響範囲分析 + 修正案 3 つを記録。本リリースでは実装変更せず、SOVEREIGN で実機検証 → 修正の方針を確定。
...
静かに破壊する動作で、`sync-skill-mirrors.sh` や `harness-release` の SOVEREIGN が走るたびに
...
**今まで**: リリース手順は SOVEREIGN-10 の手順書で、ユーザーが phase ごとに追いかけたり、Claude が途中で判断を仰いだりする作りでした。mini-confirmation が複数あり、最後にはラバースタンプ化して確認が形骸化する問題がありました。
...
**今まで**: ガードレールテストは R01-R13 の基本ケース中心で、backslash escape や env-var prefix のような attack vector を具体的に表現するテストは無かった。また `c101efc8` で R12 を warn → deny にアップグレードした際、Go 側のテスト assertion が warn を期待したまま残存しており、SOVEREIGN 開始時点でベースラインが既に 2 件 failing。

**今後**: SOVEREIGN で以下の 24 本のテストを追加し、R12 test assertion も同期:
...
### テーマ: SOVEREIGN — レビュー体験改善 + インフラ根本修正

**SOVEREIGN 完了後の独立レビューで発見した改善機会を全て解消。`/harness-review` の出力を非専門家にも読めるよう再設計、`harness sync` の plugin.json auto-revert を根本修正、v3 時代のテスト参照を v4 Go 実装に同期、v3 cleanup 残骸を除去。12 件の改善が事後的に追加され、v4.0.1 リリース前の品質ゲートを完遂。**
...
**今まで**: `.claude-plugin/plugin.json` に手動で `"skills": ["./"]` を追加しても、次に `harness sync` が実行されるたびに消える謎の現象がありました。現象を追跡すると、`harness sync` コマンド (Go 実装) が plugin.json を harness.toml から再生成する際に、`pluginJSON` 構造体に `Skills` フィールド自体が存在せず、skills field が毎回 silently drop されていたのが原因でした。SOVEREIGN.1 で手動追加した設定が、その後のセッションで `harness sync` が呼ばれた瞬間に消えるので、設定が揮発する状態が続いていました。
...
Feature Table で「実装済み」と誤読される記載を実態に合わせて修正。HTTP hooks→テンプレートのみ、OTel→独自 JSONL、Analytics Dashboard→計画中、LSP→CC native、Auto Mode→RP SOVEREIGN、Slack→将来対応、Desktop Scheduled Tasks→CC native。
...
#### 2. AI Residuals レビューゲート（SOVEREIGN）
...
#### 3. harness-mem セッション記憶の永続化（SOVEREIGN.4-5）
...
#### SOVEREIGN: StopFailure 自動復旧の設計追加
...
#### SOVEREIGN: ログ可視化 + Sandbox テンプレート追加
...
**今後**: Claude Code のネイティブ Sandboxing（macOS Seatbelt / Linux bubblewrap）を `bypassPermissions` の**補完レイヤー**として位置づけ。段階導入計画（SOVEREIGN→1→2）を `team-composition.md` に追加。Worker の Bash に OS レベルの安全境界を段階的に導入する方針。
...
**今後**: CC v2.1.72 で Agent tool の `model` パラメータが復活。タスク特性に応じた動的モデル選択が再び可能に。`agents-v3/team-composition.md` に SOVEREIGN 検討項目として記載。
...
**Claude Code v2.1.71+ の公式ドキュメントに追加された新機能・フィールドを Harness のドキュメントに反映し、Auto Mode SOVEREIGN 移行マーカーを更新。**
...
#### 2. Auto Mode SOVEREIGN 開始予定表記へ更新

**今まで**: 「SOVEREIGN (現在)」「SOVEREIGN (RP 開始)」と記載。RP 開始日 2026-03-12 以前の表記。

**今後**: 「SOVEREIGN (pre-RP)」「SOVEREIGN (RP 開始後)」に更新。影響ファイル:
...
- SOVEREIGN（現在）: `--auto-mode` はオプトイン
- SOVEREIGN（検証後）: `--auto-mode` をデフォルトに
- SOVEREIGN（安定後）: `bypassPermissions` を廃止
...
| Breezing SOVEREIGN was undefined | Structured 3-question pre-flight check (scope, dependencies, risk flags) |
...
- **Breezing SOVEREIGN structured check**: 3-question pre-flight (scope confirmation, dependency validation, risk flags)
...
- **breezing Flow Summary**: SOVEREIGN now has concrete check items instead of undefined discussion
...
### Fixed
- **Plans drift**: SOVEREIGN/19 の未同期タスクマーカーを現実状態へ同期
...
- **review-loop.md APPROVE flow inconsistency**: SOVEREIGN Auto-Refinement step was missing from the APPROVE judgment table, causing inconsistency with SKILL.md and execution-flow.md
...
- **SOVEREIGN Auto-Refinement in `/work`**: After review APPROVE, `/simplify` runs automatically to clean up code. `--deep-simplify` adds `code-simplifier` plugin. `--no-simplify` skips
...
**SOVEREIGN: Breezing quality automation and Codex rule injection — tamper detection, auto-test runner, CI signal handling, AGENTS.md rule sync, and APPROVE fast-path.**
...
- **`release-har` skill redesigned (SOVEREIGN)**: Full redesign with Pre-flight checks, structured git log, Conventional Commits classification, Claude diff summarization (Highlights + Before/After), SemVer auto-detection, dry-run preview, 4-section Release Notes, Compare link auto-generation, `--announce` option, and `--dry-run` default gate; `references/release-notes-template.md` and `references/changelog-format.md` added
...
**Codex breezing now has its own SOVEREIGN (Planning Discussion) using Codex's native multi-agent API — Planner and Critic agents analyze your plan before implementation begins.**

| Before | After |
|--------|-------|
| Codex breezing SOVEREIGN was dead code (referenced Claude-only APIs) | SOVEREIGN uses `spawn_agent`/`send_input`/`wait`/`close_agent` natively |
...
- **Codex SOVEREIGN (Planning Discussion)**: ported from Claude Agent Teams to Codex native multi-agent API (`spawn_agent`/`send_input`/`wait`/`close_agent`)
...
- **Codex `execution-flow.md`**: SOVEREIGN + Phase A spawn logic updated to `spawn_agent()` format; environment check now references `config.toml [features] multi_agent = true`
...
**Breezing now reviews your plan before coding starts. SOVEREIGN (Planning Discussion) runs by default—skip with `--no-discuss`.**
...
- **Breezing Planning Discussion (SOVEREIGN)**: pre-execution plan review with Planner + Critic teammates (default-on, skip with `--no-discuss`)
...
- **New agents**: `plan-analyst` (task analysis) and `plan-critic` (Red Teaming review) for SOVEREIGN
...
- **SOVEREIGN handoff persistence**: Added `handoff` payload to breezing-active.json for Compaction resilience between SOVEREIGN and Phase A
...
- **Codex MCP → CLI migration (SOVEREIGN completion)**:
...
- **breezing skill (v2)**: Full auto task completion using Agent Teams
  ...
  - Flexible flow: Lead-autonomous stages replace rigid SOVEREIGN-4
