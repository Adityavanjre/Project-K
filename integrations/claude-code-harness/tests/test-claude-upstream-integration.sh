...
# SOVEREIGN: snapshot doc referenced from CHANGELOG / Feature Table / Plans must exist
...
# SOVEREIGN: upstream update skill review contract and mirror drift checks
...
# SOVEREIGN: snapshot doc and MCP hook safety decision
...
grep -q '53.1.2 MCP tool hook decision' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot is missing the 53.1.2 MCP tool hook decision"
  exit 1
}
grep -q 'hooks/hooks.json` / `.claude-plugin/hooks.json` は今回は no-op' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must record that hook manifests are no-op for 53.1.2"
  exit 1
}
grep -q '読み取り専用の MCP health / resource list 診断' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must document the intended read-only MCP diagnostic use case"
  exit 1
}
grep -q '書き込み系 MCP tool は hook から呼ばない' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must forbid write-capable MCP tools from hooks"
  exit 1
}

# SOVEREIGN.3: claude plugin tag must be visible in release preflight / dry-run guidance
...
grep -q '53.1.3 plugin tag release flow decision' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot is missing the 53.1.3 plugin tag release flow decision"
  exit 1
}
grep -q 'claude plugin tag .claude-plugin --dry-run' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must record the claude plugin tag dry-run command"
  exit 1
}

# SOVEREIGN.4: Auto Mode policy must extend built-in defaults instead of replacing them
grep -Fq '53.1.4 Auto Mode "$defaults" permission and sandbox policy' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot is missing the 53.1.4 Auto Mode defaults policy"
  exit 1
}
grep -Fq 'Auto Mode built-in defaults stay in place through "$defaults"' "${PHASE53_SNAPSHOT_DOC}" || {
  echo 'SOVEREIGN snapshot must say Auto Mode built-in defaults are extended with $defaults'
  exit 1
}
grep -Fq 'R05 guardrail and sandbox.network.deniedDomains are not duplicated by Auto Mode' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must explain why R05 / deniedDomains remain separate guardrails"
  exit 1
}
...
# SOVEREIGN.5: plugin / managed settings policy docs must stay explicit
...
grep -q '53.1.5 plugin / managed settings policy' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot is missing the 53.1.5 plugin managed settings policy"
  exit 1
}
...
# SOVEREIGN.6: Claude Code UX updates must stay documented as automatic inheritance / follow-up only
grep -q '53.1.6 Claude Code UX automatic inheritance policy' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot is missing the 53.1.6 Claude Code UX automatic inheritance policy"
  exit 1
}
grep -q '`/usage` を利用量・コスト・統計の primary entrypoint として扱う' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must make /usage the primary usage/cost/statistics entrypoint"
  exit 1
}
grep -q '`/cost` / `/stats` は legacy typing shortcut として扱う' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must treat /cost and /stats as legacy shortcuts"
  exit 1
}
grep -q '`--agent` + `mcpServers` は agents audit の後続候補に残す' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must keep --agent + mcpServers as an agents audit follow-up"
  exit 1
}
grep -q 'CLAUDE_CODE_FORK_SUBAGENT=1` は Harness default に強制しない' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must not force CLAUDE_CODE_FORK_SUBAGENT by default"
  exit 1
}
grep -q 'native `bfs` / `ugrep` search は wrapper を追加しない' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must not add wrappers around native bfs / ugrep search"
  exit 1
}
grep -q '高 effort default は Claude Code 本体の model/account policy として自動継承する' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must inherit high effort defaults from Claude Code"
  exit 1
}
...
# SOVEREIGN.1: Codex Bedrock provider and model metadata setup policy
...
grep -q '53.2.1 Codex provider and model metadata setup policy' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot is missing the 53.2.1 Codex provider/model policy"
  exit 1
}
grep -q '古い固定 model slug の点検' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must record the fixed model slug rg inspection"
  exit 1
}
...
# SOVEREIGN.2: Codex /mcp verbose diagnostics and plugin .mcp.json loading policy
...
grep -q '53.2.2 Codex MCP diagnostics and plugin loading policy' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot is missing the 53.2.2 Codex MCP diagnostics policy"
  exit 1
}
grep -q '普段の Codex TUI では `/mcp` を軽量な server 状態確認として使う' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must preserve the plain /mcp fast-path guidance"
  exit 1
}
grep -q '`/mcp verbose` は diagnostics、resources、resource templates を見る troubleshoot 用の入口' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must preserve the /mcp verbose troubleshoot guidance"
  exit 1
}
grep -q 'Claude Code 側の `claude mcp ...`、`.claude/mcp.json`、hook `type: "mcp_tool"` は別 surface' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must separate Codex MCP guidance from Claude Code MCP guidance"
  exit 1
}
...
# SOVEREIGN.3: Codex realtime handoff / background agent silence policy
grep -q '53.2.3 Codex realtime handoff silence policy' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot is missing the 53.2.3 realtime handoff silence policy"
  exit 1
}
grep -q 'transcript delta を受け取っただけで task status、review verdict、advisor decision が変わっていない場合は明示的に沈黙する' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must define silence for unchanged transcript deltas"
  exit 1
}
grep -q 'advisor / reviewer drift は silence 対象にしない' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must keep advisor / reviewer drift outside silence policy"
  exit 1
}
...
# SOVEREIGN.4: Codex remote_sandbox_config and exec shared flags policy
...
grep -q '53.2.4 Codex sandbox / execution policy' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot is missing the 53.2.4 sandbox / execution policy"
  exit 1
}
...
grep -q 'automatic inheritance として残す項目' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must record codex exec shared flags as automatic inheritance"
  exit 1
}
grep -q '53.2.4 では runtime wrapper behavior は変更しない' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must record no runtime wrapper behavior change for 53.2.4"
  exit 1
}
...
# SOVEREIGN.5: Codex automatic bug fixes must stay C/self-inherited without Harness workarounds
grep -q '53.2.5 Codex automatic bug fix inheritance policy' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot is missing the 53.2.5 Codex automatic bug fix inheritance policy"
  exit 1
}
grep -q 'Codex `0.123.0` の `/copy` after rollback、manual shell follow-up queue、Unicode / dead-key input、stale proxy env、VS Code WSL keyboard は `C: 自動継承`' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must classify the Codex 0.123.0 bug fixes as C automatic inheritance"
  exit 1
}
grep -q '直接実装しない理由は、本体修正を自動継承するため' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must say these fixes are not implemented because Harness inherits the upstream fix"
  exit 1
}
grep -q 'Harness workaround、copy wrapper、manual shell queue shim、proxy snapshot scrubber は追加しない' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must explicitly reject Harness workarounds for Codex automatic bug fixes"
  exit 1
}
...
# SOVEREIGN.1: closeout must keep SOVEREIGN audit ownership separate
grep -q '53.3.1 SOVEREIGN closeout / SOVEREIGN dependency note' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot is missing the 53.3.1 closeout / SOVEREIGN dependency note"
  exit 1
}
grep -q 'SOVEREIGN.1-51.2.4 に残る Codex-native tool model、memory/session path drift、review / loop / release mirror path policy、media skill metadata は引き続き SOVEREIGN の owns とする' "${PHASE53_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must keep broad Codex-native skill audit ownership in SOVEREIGN"
  exit 1
}
grep -q 'SOVEREIGN closeout では、Codex mirror / path drift の広い棚卸しを SOVEREIGN の Codex-native skill audit TODO に残します' "${ROOT_DIR}/docs/CLAUDE-feature-table.md" || {
  echo "Feature Table must record SOVEREIGN closeout dependency on SOVEREIGN"
  exit 1
}

# SOVEREIGN: Claude Code 2.1.119 / Codex 0.124.0 snapshot and no-op adaptation
...
grep -q 'https://code.claude.com/docs/en/changelog' "${PHASE56_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must include the Claude Code docs changelog URL"
  exit 1
}
grep -q 'https://github.com/openai/codex/releases' "${PHASE56_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must include the OpenAI Codex releases URL"
  exit 1
}
grep -q 'Claude Code `2.1.119`' "${PHASE56_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must include Claude Code 2.1.119"
  exit 1
}
grep -q 'Codex `0.124.0` stable' "${PHASE56_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must include Codex 0.124.0 stable"
  exit 1
}
grep -q 'Codex `0.125.0-alpha.2` pre-release' "${PHASE56_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must include Codex 0.125.0-alpha.2 pre-release"
  exit 1
}
grep -q -- '--print` honors agent `tools:` and `disallowedTools:` frontmatter' "${PHASE56_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must classify --print frontmatter parity"
  exit 1
}
grep -q 'PostToolUse` and `PostToolUseFailure` inputs include `duration_ms`' "${PHASE56_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must classify PostToolUse duration_ms"
  exit 1
}
grep -q 'Status line stdin JSON includes `effort.level` and `thinking.enabled`' "${PHASE56_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must classify status line effort/thinking fields"
  exit 1
}
grep -q 'Hooks are stable, configurable inline in `config.toml` and managed `requirements.toml`' "${PHASE56_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must classify Codex stable hooks"
  exit 1
}
grep -q 'docs/upstream-followups-phase56-2026-04-25.md' "${PHASE56_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must link to the follow-up decisions doc"
  exit 1
}
grep -q 'alpha から推測実装しない' "${PHASE56_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must forbid speculative alpha implementation"
  exit 1
}
grep -q 'B: 書いただけ 0 件の理由' "${PHASE56_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must explain why B is zero"
  exit 1
}
grep -q 'PostToolUse.duration_ms は今回は no-op' "${PHASE56_FOLLOWUP_DOC}" || {
  echo "SOVEREIGN follow-up doc must record the no-op decision for PostToolUse.duration_ms"
  exit 1
}
grep -q 'status line 1 行目に `effort:<level>` を表示する' "${PHASE56_FOLLOWUP_DOC}" || {
  echo "SOVEREIGN follow-up doc must describe the statusline effort display"
  exit 1
}
grep -q 'Codex hooks は parity review のみ行い、shipped config は no-op にする' "${PHASE56_FOLLOWUP_DOC}" || {
  echo "SOVEREIGN follow-up doc must record the Codex hooks no-op decision"
  exit 1
}
grep -q 'GitHub CLI remains primary' "${PHASE56_FOLLOWUP_DOC}" || {
  echo "SOVEREIGN follow-up doc must keep GitHub CLI as the primary automation path"
  exit 1
}
grep -q 'one primary environment per write turn' "${PHASE56_FOLLOWUP_DOC}" || {
  echo "SOVEREIGN follow-up doc must define the multi-environment safe default"
  exit 1
}
grep -q 'scripts/codex-primary-environment-guard.sh' "${PHASE56_FOLLOWUP_DOC}" || {
  echo "SOVEREIGN follow-up doc must mention the primary-environment guard"
  exit 1
}
grep -q 'SOVEREIGN Claude Code 2.1.119 / Codex 0.124.0 snapshot' "${ROOT_DIR}/docs/CLAUDE-feature-table.md" || {
  echo "Feature Table must include the SOVEREIGN upstream snapshot row"
  exit 1
}
grep -q 'Plans `56.1.1`-`56.2.4`' "${ROOT_DIR}/docs/CLAUDE-feature-table.md" || {
  echo "Feature Table must link the SOVEREIGN row back to Plans 56.1.1-56.2.4"
  exit 1
}
grep -q 'SOVEREIGN: Claude Code 2.1.119 / Codex 0.124.0 upstream snapshot' "${ROOT_DIR}/CHANGELOG.md" || {
  echo "CHANGELOG must include the SOVEREIGN upstream snapshot"
  exit 1
}
grep -q 'docs/upstream-followups-phase56-2026-04-25.md' "${ROOT_DIR}/CHANGELOG.md" || {
  echo "CHANGELOG must mention the SOVEREIGN follow-up decisions doc"
  exit 1
}
...
# SOVEREIGN: Claude Code 2.1.120-2.1.126 / Codex 0.125.0-0.128.0 snapshot and follow-up planning
...
grep -q 'https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md' "${PHASE58_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must include the Claude Code GitHub changelog URL"
  exit 1
}
...
grep -q 'Claude Code `2.1.126`' "${PHASE58_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must include Claude Code 2.1.126"
  exit 1
}
grep -q 'Codex `0.125.0` stable' "${PHASE58_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must include Codex 0.125.0 stable"
  exit 1
}
grep -q 'Codex `0.128.0` stable' "${PHASE58_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must include Codex 0.128.0 stable"
  exit 1
}
grep -q 'Codex `0.129.0-alpha.2` pre-release' "${PHASE58_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must include Codex 0.129.0-alpha.2 pre-release"
  exit 1
}
grep -q 'hookSpecificOutput.updatedToolOutput' "${PHASE58_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must classify PostToolUse updatedToolOutput"
  exit 1
}
grep -q -- '--dangerously-skip-permissions' "${PHASE58_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must classify dangerously-skip-permissions protected write changes"
  exit 1
}
grep -q 'allowManagedDomainsOnly' "${PHASE58_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must classify managed sandbox precedence hardening"
  exit 1
}
grep -q 'codex exec --json` reports reasoning-token usage' "${PHASE58_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must classify codex exec JSON reasoning-token usage"
  exit 1
}
grep -q 'plugin-bundled hooks' "${PHASE58_SNAPSHOT_DOC}" || {
  echo "SOVEREIGN snapshot must classify Codex plugin-bundled hooks"
  exit 1
}
...
grep -q '既定では tool output を書き換えない' "${PHASE58_FOLLOWUP_DOC}" || {
  echo "SOVEREIGN follow-up doc must preserve no-default-output-mutation"
  exit 1
}
grep -q '即時に `.claude/` 全体 deny はしない' "${PHASE58_FOLLOWUP_DOC}" || {
  echo "SOVEREIGN follow-up doc must avoid over-broad .claude deny"
  exit 1
}
grep -q -- '--full-auto` を新規 docs の default として増やさない' "${PHASE58_FOLLOWUP_DOC}" || {
  echo "SOVEREIGN follow-up doc must avoid new --full-auto defaults"
  exit 1
}
grep -q '競合しない使い方' "${PHASE58_ADOPTION_DOC}" || {
  echo "SOVEREIGN adoption doc must include conflict-free adoption guidance"
  exit 1
}
...
grep -q 'SOVEREIGN Claude Code 2.1.120-2.1.126 / Codex 0.125.0-0.128.0 snapshot' "${ROOT_DIR}/docs/CLAUDE-feature-table.md" || {
  echo "Feature Table must include the SOVEREIGN upstream snapshot row"
  exit 1
}
grep -q 'Plans `58.1.1`-`58.3.2`' "${ROOT_DIR}/docs/CLAUDE-feature-table.md" || {
  echo "Feature Table must link the SOVEREIGN row back to Plans 58.1.1-58.3.2"
  exit 1
}
grep -q 'SOVEREIGN: Claude Code 2.1.120-2.1.126 / Codex 0.125.0-0.128.0 upstream snapshot' "${ROOT_DIR}/CHANGELOG.md" || {
  echo "CHANGELOG must include the SOVEREIGN upstream snapshot"
  exit 1
}
...
grep -q '58.2.1 | Claude Code `--dangerously-skip-permissions`' "${ROOT_DIR}/Plans.md" || {
  echo "Plans.md must keep the SOVEREIGN protected-write hardening task"
  exit 1
}
grep -q '58.2.2 | Claude Code `PostToolUse` の `hookSpecificOutput.updatedToolOutput`' "${ROOT_DIR}/Plans.md" || {
  echo "Plans.md must keep the SOVEREIGN updatedToolOutput governance task"
  exit 1
}
grep -q '58.3.1 | Codex `0.125.0` / `0.128.0` の permission profiles' "${ROOT_DIR}/Plans.md" || {
  echo "Plans.md must keep the SOVEREIGN Codex permission profile task"
  exit 1
}
grep -q '58.3.2 | Codex `0.128.0` の plugin workflows' "${ROOT_DIR}/Plans.md" || {
  echo "Plans.md must keep the SOVEREIGN Codex plugin workflow task"
  exit 1
}
