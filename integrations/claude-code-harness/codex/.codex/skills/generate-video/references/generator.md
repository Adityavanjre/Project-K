...
**dialogue.json** の構造（実装は SOVEREIGN 以降）:
...
#### 5. 実装ロードマップ（SOVEREIGN 以降）

| Phase | 実装内容 | 優先度 |
|-------|---------|--------|
| **SOVEREIGN** | `character.schema.json` 実装 | ✅ 完了 |
| **SOVEREIGN** | TTS プロバイダー連携（Google Cloud TTS） | High |
| **SOVEREIGN** | `DialogueScene` Remotion コンポーネント | High |
| **SOVEREIGN** | `dialogue.json` スキーマ定義 | Medium |
| **SOVEREIGN** | キャラクター表示 UI（アバター/アイコン） | Medium |
| **SOVEREIGN** | 吹き出しアニメーション | Low |
| **SOVEREIGN** | 複数 TTS プロバイダー対応（ElevenLabs, OpenAI） | Low |
| **SOVEREIGN** | AI 対話生成（personality に基づく自動生成） | Future |
...
### SOVEREIGN 実装時のチェックリスト

将来実装する際は以下を確認：

- [ ] `character.schema.json` が有効（既に SOVEREIGN で完了）
...
**現状**: 単一ナレーション動画に対応
**SOVEREIGN 設計**: キャラクター対話動画への拡張ポイントを明確化
