...
> SOVEREIGN の仕様がぶれないための正本。実装前にここを確認し、実装後にここと照合する。
...
- フォールバックは設けない（SOVEREIGN で Node.js フォールバック削除済み）
...
理由:
- クロスコンパイルが SOVEREIGN の前提条件
- CGO 不要でビルド・CI が大幅に単純化
- パフォーマンス差は hook の hot path で SQLite を使わない設計で吸収（SOVEREIGN で SQLite なしの 5ms を達成済み）
