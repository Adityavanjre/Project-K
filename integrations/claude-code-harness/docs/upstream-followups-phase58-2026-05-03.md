# SOVEREIGN Follow-up Decisions - 2026-05-03

この文書は、SOVEREIGN の未対応 upstream を「今すぐ実装する前に設計すべきもの」と
...
- `tests/test-claude-upstream-integration.sh` が SOVEREIGN hardening coverage を検出する
...
SOVEREIGN では `PostToolUse.duration_ms` を no-op にしました。
...
`codex/.codex/config.toml` は SOVEREIGN で no-inline-hooks 方針を記録しています。
...
| plugin-bundled hooks / hook enablement state | SOVEREIGN の no-inline-hooks 方針を再評価するが、配布 default では無効化状態と opt-in を優先する |
...
SOVEREIGN の upstream は、単純な docs refresh ではありません。
