...
> SOVEREIGN 以降の実装タスクへトレースできる形にまとめる。
>
...
> 本表の A 項目は SOVEREIGN-44.7 で実装、C 項目は SOVEREIGN の Feature Table 追記のみ。

---

## 1. カテゴリ A: 実装あり（Harness 側変更が必要）

SOVEREIGN で Harness 側に具体的変更を伴う項目。各行に対応フェーズを明記する。
...
SOVEREIGN 以降のタスクが本分類表のどの A 項目に対応するかを逆引きできるようにする。
