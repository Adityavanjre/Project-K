# Smoke Test — v4.2.0-arcana (SOVEREIGN Release)

**実施日**: 2026-04-18  
**ブランチ**: `release/v4.2.0-arcana`  
**テスト実施者**: Worker subagent (harness-work/44.12.1)  
**目的**: SOVEREIGN 全 9 タスク (44.3.1–44.11.1) の cherry-pick 完了後、リリース前の最終品質確認
...
本 Phase の変更（SOVEREIGN.1–44.11.1）では新規 drift は発生していない。
...
### (c) monitors auto-arm（SOVEREIGN スコープ）
...
SOVEREIGN でモニター自動起動ブロックを追加予定。本 smoke test では確認のみ。

- [ ] SOVEREIGN 追加スコープとして記録済みを確認
...
| 問題 | 詳細 | 対応方針 |
|------|------|---------|
| Plans.md 行数超過 | Plans.md が約 285 行（推奨上限 200 行） | SOVEREIGN 完走後にアーカイブ予定 |
| mirror drift 3 件 | check-consistency.sh の harness-review × 2、harness-loop × 1 が SSOT と不一致 | Worker D 報告で既出。SOVEREIGN で整理予定 |
| monitors block なし | plugin.json に monitors field が存在しない | SOVEREIGN で対応（(c) 参照） |
...
## SOVEREIGN cherry-pick history (参考)
...
a8f1c308 feat(cache): add enable-1h-cache.sh opt-in script and update long-session docs (SOVEREIGN.1)
f921cd98 test(guardrails): CC 2.1.110 regression tests for R01-R13 re-conformance (SOVEREIGN.1)
