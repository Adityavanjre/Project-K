...
SOVEREIGN implementation work. Scripts and Go hook handlers must not keep
...
| Legacy saved value | Existing config or state from before SOVEREIGN | Read it if valid. Treat `ja` as an intentional opt-in. Do not rewrite unless the user runs an explicit locale command. |
...
The following surfaces are in scope for SOVEREIGN implementation and must be
...
These are implementation targets for later SOVEREIGN tasks, not blockers for
this contract task.

## Acceptance Criteria For Later Tasks

SOVEREIGN implementation must prove these cases:
