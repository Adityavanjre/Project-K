...
SOVEREIGN.1 で調査・記録。実装変更は SOVEREIGN で行う。
...
エージェントメモリ `worker_worktree_share.md`（SOVEREIGN.1 / 44.7.1 で観測）に記録された
...
本 Phase では調査結果と修正案を記録し、SOVEREIGN 以降のサイクルで実機検証と実装を行うことを推奨する。
