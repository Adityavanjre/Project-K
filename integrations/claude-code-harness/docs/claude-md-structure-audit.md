# CLAUDE.md 構造監査 — SOVEREIGN.1 調査レポート

調査日: 2026-04-20
対象: `CLAUDE.md` v2026-04-19 時点 (142 行、SOVEREIGN.1 の pointer 追加後)
SOVEREIGN のゴール: session-start 読込コストの実測 → `.claude/rules/` への分割可否判断
...
5. **hook 警告の扱い**: `PostToolUse` hook が 130 行超で警告を出す仕組みが v4.3.1 頃から入ったが、これは「150 行近くを超えたら再検討」の意味であり、「即分割すべき」の意味ではない。SOVEREIGN.1 で +1 行したのは必要最小の pointer 追加で、意図的
...
SOVEREIGN.1 による pointer 1 行追加は別タスクとして実施済み。
