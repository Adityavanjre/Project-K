...
| **`/simplify` (v2.1.63)** | work | SOVEREIGN Auto-Refinement: 実装後の自動コード洗練 |
...
| **Auto Mode (RP SOVEREIGN)** | breezing, work | CC native 機能。Harness 側は PermissionDenied 追跡のみ。判断ロジック未実装。現行 default は `bypassPermissions` |
...
| **`context: fork` host CLAUDE.md 継承仕様と auto-start 回避パターン (SOVEREIGN)** | harness-review | `context: fork` スキルは isolated context で動作し、host CLAUDE.md の session-start rules に override されて停止する事象を解消。host CLAUDE.md 継承仕様と auto-start 回避パターンを `skill-editing.md` に明文化（Issue #84）。A: 実装あり（SKILL.md Step 0 硬化 + `REVIEW_AUTOSTART` marker 契約） | `A: 実装あり` |
...
**注記**:
SOVEREIGN でも `B: 書いただけ` は `0` 件です。Codex 0.121.0 の大きい項目は、今回の直接実装ではなく「Codex 比較軸」として Plans に残し、Claude Code 側の `AskUserQuestion.updatedInput` と 2.1.113 hardening は settings / Go / tests まで実装して `A` としました。
...
| **Codex 0.122.0 plugin / Plan Mode / permission model** | codex workflow, setup, sandbox | `/side`、fresh-context Plan Mode、plugin workflow、deny-read glob、tool discovery default-on を Codex mirror 改善候補に分類 | `P: Plans 化`。SOVEREIGN の Codex-native skill audit と一緒に扱う |
...
**注記**:
SOVEREIGN でも `B: 書いただけ` は `0` 件です。Claude / Codex 本体が自然に改善する UX は `C` とし、Harness に重ねると二重責務になるものは `P` として後続の Codex-native skill audit / plugin policy に接続しました。直接実装は review findings の再発防止に絞り、skill mirror drift と no-op adaptation を test で固定しています。
...
**注記**:
SOVEREIGN でも `B: 書いただけ` は `0` 件です。Feature Table は入口に留め、公式 URL と version-by-version の判断根拠は `docs/upstream-update-snapshot-2026-04-23.md` に集約しました。`A` は SOVEREIGN の具体 task に接続し、`C` は本体修正の自動継承、`P` は推測実装しない将来判断として扱います。

SOVEREIGN closeout では、Codex mirror / path drift の広い棚卸しを SOVEREIGN の Codex-native skill audit TODO に残します。SOVEREIGN は upstream `0.123.0` 差分の具体反映だけを閉じ、SOVEREIGN.1-51.2.4 の tool model / memory path / mirror path / media metadata 整理を先取りしません。
...
`/work` の SOVEREIGN Auto-Refinement として統合され、実装完了後に自動でコードを簡潔化・整理する。
...
Harness では `breezing` の Worker spawn 時に検討可能だが、現状は Lead が明示的に `run_in_background` を制御しているため、追加適用は SOVEREIGN 以降で検討する。
...
- 現状は Worker/Reviewer とも `model: sonnet` で固定。Lead がタスク特性に応じて動的にモデルを切り替える実装は SOVEREIGN 以降で検討
...
## v2.1.99-v2.1.110 + Opus 4.7 詳細セクション（SOVEREIGN.1）
...
**付加価値**: `A: 実装あり`（hooks/hooks.json PreCompact エントリ、SOVEREIGN で確認済み）
...
**付加価値**: `A: 実装あり`（`scripts/enable-1h-cache.sh`、SOVEREIGN.1 で実装済み）
...
**付加価値**: `A: 実装あり`（`.claude/rules/commit-safety.md`、SOVEREIGN.1 で実装済み）
...
**付加価値**: `A: 実装あり`（`go/internal/guardrail/cc2110_regression_test.go`、SOVEREIGN.1 で実装済み）
...
**付加価値**: `A: 実装あり`（`scripts/reenter-worktree.sh`、SOVEREIGN.1 で実装済み）
...
**付加価値**: `A: 実装あり`（`.claude/rules/opus-4-7-prompt-audit.md`、SOVEREIGN.1 + 44.4.2 で実装済み）
...
**付加価値**: `A: 実装あり`（`agents/reviewer.md`, `agents/advisor.md`, `docs/effort-level-policy.md`、SOVEREIGN.1 で実装済み）
...
**付加価値**: `C: 採用見送り`（`docs/task-budgets-research.md`、SOVEREIGN.1 で調査済み）
...
**付加価値**: `A: 実装あり`（`docs/opus-4-7-vision-usage.md`, `skills/harness-review/references/vision-high-res-flow.md`、SOVEREIGN.1 で実装済み）
...
**付加価値**: `A: 実装あり`（`docs/ultrareview-policy.md`, `skills/harness-review/SKILL.md`、SOVEREIGN.1 で実装済み）
