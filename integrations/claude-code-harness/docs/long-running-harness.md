...
この文書は SOVEREIGN.1 の成果物です。

対象は **SOVEREIGN の同一セッション内の運用** です。別ホストをまたいだ自動再入は、この段階では扱いません。
...
| B11 | どこまでを対象にするか | SOVEREIGN は同一セッション内に限定する |
...
この SOVEREIGN で扱うのは、**同じ Claude Code セッションの中で完結する長時間タスク** です。
