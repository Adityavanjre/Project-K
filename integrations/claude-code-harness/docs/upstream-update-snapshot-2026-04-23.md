...
| Codex 0.123.0 | Code Review skill instructions tightened | Codex-driven review の指示が堅くなる | C | review skill | Harness reviewer は別 surface。必要なら SOVEREIGN.1 で重複を整理 |
...
## 53.3.1 SOVEREIGN closeout / SOVEREIGN dependency note

SOVEREIGN の A/C/P 反映はここで閉じる。

完了確認:

- Feature Table は SOVEREIGN 追補テーブルとして、Claude Code `2.1.117-2.1.118` と Codex `0.123.0` の各項目を `A` / `C` / `P` に分類済み。
- CHANGELOG `[Unreleased]` は SOVEREIGN の user-facing change を snapshot、MCP hook safety、plugin tag、Auto Mode、managed settings、Codex provider / MCP / realtime / sandbox / bug-fix inheritance まで記録済み。
- `tests/test-claude-upstream-integration.sh` は SOVEREIGN snapshot と主要 guidance を grep 固定済み。
- `tests/validate-plugin.sh` は SOVEREIGN closeout 時点で PASS。既存 warning は script executable bit と `IMPLEMENTATION_GUIDE.md` 推奨欠落のみ。

SOVEREIGN との重複整理:

- SOVEREIGN は upstream `0.123.0` の具体差分を取り込む周期であり、Codex-native skill audit 全体の片付けではない。
...
- SOVEREIGN.1-51.2.4 に残る Codex-native tool model、memory/session path drift、review / loop / release mirror path policy、media skill metadata は引き続き SOVEREIGN の owns とする。
- 今後 mirror / path drift を広く直す場合は SOVEREIGN 側で扱い、SOVEREIGN の upstream snapshot 追従と混ぜない。
...
- `A` は必ず SOVEREIGN の具体的な Plans task に接続した。Feature Table に書くだけでは終わらせない。
