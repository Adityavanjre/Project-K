...
SOVEREIGN (v4.1.0) で導入された `deleted-concepts.yaml` + `check-residue.sh` の
運用ルールを定義する。
...
CI (validate-plugin.sh のセクション 9) と release preflight (harness-release の SOVEREIGN)
の両方で自動チェックされているので、**merge 前に 0 件であることが保証される**。

## 付録: 今セッション (v4.0.0 → v4.0.1) の 13 件の v3 残骸事例

SOVEREIGN の動機となった事例。**この機能がなぜ生まれたかのストーリー。**
...
視点が必要。SOVEREIGN はその視点を Harness の検証層に組み込むために生まれた。
...
- `skills/harness-release/SKILL.md` — SOVEREIGN preflight step 2（リリースゲート）
