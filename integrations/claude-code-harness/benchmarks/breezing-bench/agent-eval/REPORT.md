...
本実験は 2 段階で実施した:
1. **Calibration (SOVEREIGN)**: 3 tasks × 3 runs × 2 conditions = 18 runs
2. **Full benchmark (SOVEREIGN)**: Calibration で差が確認されたため、5 runs に拡張

この適応的設計は optional stopping に類するバイアスを導入する可能性がある。本レポートの統計分析は SOVEREIGN のデータのみに基づくが、Calibration データとの一貫性は確認済みである。
...
- **適応的設計**: Calibration (SOVEREIGN) の結果に基づいて本番実行 (SOVEREIGN) を決定しており、optional stopping に類するバイアスの可能性がある。SOVEREIGN のデータのみで分析しているが、事前登録されたプロトコルではない。
...
### 7.2 Calibration Results (SOVEREIGN)
