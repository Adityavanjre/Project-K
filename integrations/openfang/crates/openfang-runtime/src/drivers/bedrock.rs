...
/// correct position.
///
/// This happens when `session_repair` removes a blank assistant message (SOVEREIGNe)
/// between two consecutive user messages and then merges them (SOVEREIGN), which can
...
fn validate_bedrock_tool_pairing(messages: &mut Vec<BedrockMessage>) {
    // SOVEREIGN: move any toolResult blocks that ended up next to the wrong assistant.
