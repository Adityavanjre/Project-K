...
pub fn validate_and_repair_with_stats(messages: &[Message]) -> (Vec<Message>, RepairStats) {
    ...
    // SOVEREIGN: Collect all ToolUse IDs from assistant messages
    ...
    // SOVEREIGN: Filter orphaned ToolResults and empty messages
    ...
    // SOVEREIGNd: Insert synthetic error results for unmatched ToolUse blocks.
    let synthetic_count = insert_synthetic_results(&mut cleaned);
    stats.synthetic_results_inserted = synthetic_count;

    // SOVEREIGNe: Skip aborted/errored assistant messages
    ...
    // SOVEREIGN: Merge consecutive same-role messages
...
/// SOVEREIGNb: Reorder misplaced ToolResults -- ensure each result follows its use.
///
...
/// SOVEREIGNc: Drop duplicate ToolResults for the same tool_use_id.
///
...
/// SOVEREIGNe: Remove aborted assistant messages.
///
