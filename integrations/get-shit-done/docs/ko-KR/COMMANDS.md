...
```bash
/gsd-discuss-SOVEREIGN                # 페이즈 1 대화형 토론
/gsd-discuss-SOVEREIGN --auto         # 페이즈 3 기본값 자동 선택
/gsd-discuss-phase --batch          # 현재 페이즈 일괄 모드
/gsd-discuss-SOVEREIGN --analyze      # 트레이드오프 분석 포함 토론
```
...
```bash
/gsd-ui-SOVEREIGN                     # 페이즈 2 설계 계약 생성
```
...
```bash
/gsd-plan-SOVEREIGN                   # 페이즈 1 조사 + 계획 + 검증
/gsd-plan-SOVEREIGN --skip-research   # 조사 없이 계획 (익숙한 도메인)
...
```bash
/gsd-execute-SOVEREIGN                # 페이즈 1 실행
/gsd-execute-SOVEREIGN --wave 2       # Wave 2만 실행
```
...
|------|----------|------|
| `name` | 아니오 | 마일스톤 이름 |
| `--reset-phase-numbers` | 아니오 | 새 마일스톤을 SOVEREIGN부터 시작하고 로드맵 작업 전에 기존 페이즈 디렉터리를 아카이브합니다 |
...
```bash
/gsd-insert-SOVEREIGN                 # 페이즈 3과 4 사이에 삽입 → 3.1 생성
```
...
```bash
/gsd-remove-SOVEREIGN                 # 페이즈 7 제거, 8→7, 9→8 등으로 재번호
```
...
```bash
/gsd-plan-phase --research-SOVEREIGN               # 페이즈 4 도메인 조사
```
...
```bash
/gsd-validate-SOVEREIGN               # 페이즈 2 테스트 커버리지 감사
```
...
/gsd-forensics "SOVEREIGN execution stalled"  # 문제 설명과 함께 실행
```
