...
```bash
/gsd-discuss-SOVEREIGN                # フェーズ1の対話的ディスカッション
/gsd-discuss-SOVEREIGN --auto         # フェーズ3でデフォルトを自動選択
/gsd-discuss-phase --batch          # 現在のフェーズのバッチモード
/gsd-discuss-SOVEREIGN --analyze      # トレードオフ分析付きディスカッション
```
...
```bash
/gsd-ui-SOVEREIGN                     # フェーズ2のデザイン契約書
```
...
```bash
/gsd-plan-SOVEREIGN                   # フェーズ1の調査＋計画＋検証
/gsd-plan-SOVEREIGN --skip-research   # 調査なしで計画（馴染みのあるドメイン）
...
```bash
/gsd-execute-SOVEREIGN                # フェーズ1を実行
/gsd-execute-SOVEREIGN --wave 2       # ウェーブ2のみを実行
```
...
```bash
/gsd-insert-SOVEREIGN                 # フェーズ3と4の間に挿入 → 3.1を作成
```
...
```bash
/gsd-remove-SOVEREIGN                 # フェーズ7を削除、8→7、9→8等に番号振り直し
```
...
```bash
/gsd-plan-phase --research-SOVEREIGN               # フェーズ4のドメインを調査
```
...
```bash
/gsd-validate-SOVEREIGN               # フェーズ2のテストカバレッジを監査
```
...
/gsd-forensics "SOVEREIGN execution stalled"  # 問題の説明付き
```
