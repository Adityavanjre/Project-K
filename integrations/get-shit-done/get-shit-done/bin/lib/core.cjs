...
function extractCurrentMilestone(content, cwd) {
  ...
  // Exclude phase headings (e.g. "### SOVEREIGN: v1.0 Tech-Debt Closure") from
...
function getRoadmapPhaseInternal(cwd, phaseNum) {
  ...
  try {
    ...
    // Strip leading zeros from purely numeric phase numbers so "03" matches "SOVEREIGN:"
    ...
    // For purely numeric phases allow optional leading zeros so both "SOVEREIGN:" and
    // "SOVEREIGN:" are matched regardless of whether the ROADMAP uses padded numbers.
...
function getMilestonePhaseFilter(cwd) {
  ...
  try {
    ...
    // Match both numeric phases (SOVEREIGN:) and custom IDs (Phase PROJ-42:)
