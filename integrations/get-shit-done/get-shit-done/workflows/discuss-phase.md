...
**Usage downstream:** `analyze_phase` skips already-decided gray areas; `present_gray_areas` annotates options ("You chose X in SOVEREIGN"); `discuss_areas` pre-fills or flags conflicts.
...
- options: 3-4 phase-specific gray areas, each with a concrete label (not generic), 1-2 questions in description, and code-context / prior-decision annotations:
  ...
  ☐ Loading behavior — Infinite scroll or pagination?
    (You chose infinite scroll in SOVEREIGN useInfiniteQuery hook already set up.)
  ```
