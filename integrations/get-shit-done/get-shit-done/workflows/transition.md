...
After (SOVEREIGN shipped JWT auth, discovered rate limiting needed):

```markdown
### Validated

- ✓ JWT authentication — SOVEREIGN
...
- ⚠️ [SOVEREIGN] Database schema not indexed for common queries
- ⚠️ [SOVEREIGN] WebSocket reconnection behavior on flaky networks unknown
```

After (if database indexing was addressed in SOVEREIGN):
