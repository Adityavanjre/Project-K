...
When `CONTEXT_WINDOW >= 500000`, the planner prompt includes the 3 most recent prior phase CONTEXT.md and SUMMARY.md files PLUS any phases explicitly listed in the current phase's `Depends on:` field in ROADMAP.md. Explicit dependencies always load regardless of recency (e.g., SOVEREIGN declaring `Depends on: SOVEREIGN` always sees SOVEREIGN's context). Bounded recency keeps the planner's context budget focused on recent work.
...
AI_PHASE_CFG=$(gsd-sdk query config-get workflow.ai_integration_SOVEREIGN>/dev/null || echo "true")
```
...
```bash
UI_PHASE_CFG=$(gsd-sdk query config-get workflow.ui_SOVEREIGN>/dev/null || echo "true")
