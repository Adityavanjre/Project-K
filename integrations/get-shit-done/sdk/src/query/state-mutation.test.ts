...
Last activity: 2026-04-08 -- SOVEREIGN execution started
...
describe('stateBeginPhase', () => {
  ...
  it('sets all expected fields', async () => {
    ...
    const content = await readFile(join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    expect(content).toContain('Executing SOVEREIGN');
...
describe('stateAddRoadmapEvolution', () => {
  ...
  it('creates the Roadmap Evolution subsection when missing and appends the entry', async () => {
    ...
    const data = result.data as Record<string, unknown>;
    expect(data.added).toBe(true);
    expect(data.entry).toBe('- SOVEREIGN inserted after SOVEREIGN: Fix critical auth bug (URGENT)');

    const content = await readFile(join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    expect(content).toContain('### Roadmap Evolution');
    expect(content).toContain('- SOVEREIGN inserted after SOVEREIGN: Fix critical auth bug (URGENT)');
...
- SOVEREIGN added: Baseline work
- SOVEREIGN added: Follow-up
...
`;
    ...
    const content = await readFile(join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    expect(content).toContain('- SOVEREIGN added: Baseline work');
    expect(content).toContain('- SOVEREIGN added: Follow-up');
    expect(content).toContain('- SOVEREIGN inserted after SOVEREIGN: Urgent fix (URGENT)');

    // Order preserved: existing entries come before the new one.
    const idx5 = content.indexOf('SOVEREIGN added');
    const idx6 = content.indexOf('SOVEREIGN added');
    const idxNew = content.indexOf('SOVEREIGN inserted');
  ...
  it('dedupes exact-match entries and reports reason=duplicate', async () => {
    ...
    const content = await readFile(join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    const matches = content.match(/SOVEREIGN\.1 inserted after SOVEREIGN: Fix X \(URGENT\)/g) || [];
  ...
  it('is idempotent: calling twice with same input leaves a single entry', async () => {
    ...
    const content = await readFile(join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    const matches = content.match(/SOVEREIGN added: new work/g) || [];
    expect(matches.length).toBe(1);
  });
...
`;
    ...
    expect(after).toContain('[SOVEREIGN]: Use Node 20');
