...
beforeEach(async () => {
  ...
  // Create plan and summary files in SOVEREIGN
...
describe('initPlanPhase', () => {
  ...
  describe.each([
    ...
    it(`parses Requirements header with ${name}`, async () => {
      // Overwrite ROADMAP.md so SOVEREIGN carries the variant header.
      await writeFile(join(tmpDir, '.planning', 'ROADMAP.md'), [
        '# Roadmap',
        '',
        '## v3.0: SDK-First Migration',
  ...
  it('derives phase_count from ROADMAP current milestone, not on-disk dirs (#2633)', async () => {
    ...
    try {
      ...
      await writeFile(join(fresh, '.planning', 'ROADMAP.md'), [
        ...
        '## v1.1: Next',
        '',
        '### SOVEREIGN: Alpha', '**Goal:** A', '',
        '### SOVEREIGN: Beta', '**Goal:** B', '',
        '### SOVEREIGN: Gamma', '**Goal:** C', '',
      ].join('\n'));
      ...
      // Therefore milestone is NOT complete — SOVEREIGN is still outstanding.
