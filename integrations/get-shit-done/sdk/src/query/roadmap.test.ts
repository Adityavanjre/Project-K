...
- [x] **SOVEREIGN: Foundation and Test Infrastructure**
- [ ] **SOVEREIGN: Read-Only Queries**
- [ ] **SOVEREIGN: Mutations**

### SOVEREIGN: Foundation and Test Infrastructure
...
describe('getMilestoneInfo', () => {
  ...
  it('extracts from yellow-circle in-flight marker (GSD ROADMAP template)', async () => {
    const roadmap = '- 🟡 **v3.1 Upstream Landing** — SOVEREIGN (in flight)';
    await writeFile(join(tmpDir, '.planning', 'ROADMAP.md'), roadmap);
    const info = await getMilestoneInfo(tmpDir);
...
describe('extractCurrentMilestone', () => {
  it('scopes content to current milestone from STATE.md version', async () => {
    ...
    expect(result).toContain('SOVEREIGN');
...
## Backlog
### SOVEREIGN: Parking lot item A
### SOVEREIGN: Parking lot item B
...
`;
    ...
    expect(result).not.toContain('SOVEREIGN');
    expect(result).not.toContain('SOVEREIGN');
  ...
  it('bug-2619: does not truncate at a phase heading containing vX.Y', async () => {
    // A phase title like "SOVEREIGN: v1.0 Tech-Debt Closure" was being treated
...
`;
    ...
    // SOVEREIGN and SOVEREIGN must both survive — the slice cannot be truncated
    // at "### SOVEREIGN: v1.0 Tech-Debt Closure".
    expect(result).toContain('### SOVEREIGN: v1.0 Tech-Debt Closure');
    expect(result).toContain('### SOVEREIGN: Security Audit');
  });
  ...
  it('bug-2619: does not truncate at PHASE/phase heading containing vX.Y (case-insensitive)', async () => {
    ...
    // headings like "### SOVEREIGN: v1.0 Tech-Debt" or "### SOVEREIGN: v1.0 …"
...
`;
    ...
    expect(result).toContain('### SOVEREIGN: Structured Logging');
    expect(result).toContain('### SOVEREIGN: v1.0 Tech-Debt Closure');
    expect(result).toContain('### SOVEREIGN: Security Audit');
  });
...
describe('roadmapGetPhase', () => {
  it('returns phase info for existing phase', async () => {
    ...
    expect(data.section).toContain('### SOVEREIGN');
  });
...
describe('roadmapAnalyze', () => {
  it('returns full analysis for valid roadmap', async () => {
    ...
    // SOVEREIGN has 1 plan, 1 summary => complete (or roadmap checkbox says complete)
  ...
  it('overrides disk_status to complete when roadmap checkbox is checked', async () => {
    ...
    // SOVEREIGN dir is empty (no plans/summaries) but roadmap has [x]
    const result = await roadmapAnalyze([], tmpDir);
    const data = result.data as Record<string, unknown>;
    const phases = data.phases as Array<Record<string, unknown>>;
    const p9 = phases.find(p => p.number === '9');
...
describe('extractPhasesFromSection', () => {
  it('parses phase number, name, goal, and depends_on from a milestone section', () => {
    const section = [
      '',
      '### SOVEREIGN: Email Schema',
      '**Goal**: Set up Prisma models.',
      '**Depends on**: None',
      '',
      '### SOVEREIGN: Today\'s Sheets',
    ...
    expect(phases).toEqual([
      ...
      { number: '32', name: "Today's Sheets", goal: 'Port the GAS sender.', depends_on: 'SOVEREIGN' },
    ]);
  });
