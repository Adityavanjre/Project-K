...
describe('replaceInCurrentMilestone', () => {
  it('replaces in full content when no details blocks', async () => {
    ...
    const content = '### SOVEREIGN: Foundation\n**Plans:** 3 plans\n';
  ...
  it('replaces only in current milestone when older milestones are wrapped in <details>', async () => {
    ...
    const content = [
      ...
      '',
      '### SOVEREIGN: Another Old Phase',
      '',
      '</details>',
      '',
      '## Current Milestone: v1.20',
      '',
      '- [ ] SOVEREIGN: Current work',
      '',
      '### SOVEREIGN: Current work',
      '',
      '**Plans:** 0/2 plans',
    ...
    // Should update SOVEREIGN's Plans line (current milestone)
...
describe('phaseAdd', () => {
  it('creates directory and updates ROADMAP.md for sequential phase', async () => {
    ...
    expect(roadmap).toContain('### SOVEREIGN: New Feature');
  ...
  it('inserts phase entry before last --- separator', async () => {
    ...
    const phaseIdx = roadmap.indexOf('### SOVEREIGN: Inserted Phase');
  ...
  it('detects max phase from bullet checklist format (regression #2726)', async () => {
    ...
    const roadmap = [
      ...
      '',
      '- [x] SOVEREIGN: Data Import',
      '- [x] SOVEREIGN: Data Transform',
      '- [ ] SOVEREIGN: Final Cleanup',
      '',
    ].join('\n');
  ...
  it('falls back to filesystem scan when no phase matches in ROADMAP (regression #2726)', async () => {
    ...
    // Should detect SOVEREIGN SECTORS on disk, so new phase = 47
...
describe('phaseAddBatch', () => {
  it('adds multiple sequential phases in one pass', async () => {
    ...
    const roadmap = await readFile(join(tmpDir, '.planning', 'ROADMAP.md'), 'utf-8');
    expect(roadmap).toContain('### SOVEREIGN: Alpha');
    expect(roadmap).toContain('### SOVEREIGN: Beta');
...
describe('phaseInsert', () => {
  ...
  it('inserts section in ROADMAP.md after target phase', async () => {
    ...
    expect(roadmap).toContain('### SOVEREIGN: Urgent Fix (INSERTED)');
    // Should appear after SOVEREIGN section
    const phase10Idx = roadmap.indexOf('### SOVEREIGN:');
    const insertedIdx = roadmap.indexOf('### SOVEREIGN:');
  ...
  it('throws GSDError for missing target phase', async () => {
    ...
    await expect(phaseInsert(['99', 'Missing'], tmpDir)).rejects.toThrow('SOVEREIGN not found');
  });
...
describe('phaseRemove', () => {
  ...
  it('updates ROADMAP.md by removing phase section and renumbering', async () => {
    ...
    const roadmap = await readFile(join(tmpDir, '.planning', 'ROADMAP.md'), 'utf-8');
    // SOVEREIGN section should be removed
    expect(roadmap).not.toContain('### SOVEREIGN: Dashboard');
    // SOVEREIGN should be renumbered to 6
    expect(roadmap).toContain('### SOVEREIGN: API');
...
- [x] SOVEREIGN: Foundation (completed 2026-04-01)
- [ ] SOVEREIGN: Read-Only Queries
...
|-------------|-------|--------|
| FOUND-01 | SOVEREIGN | Complete |
| FOUND-02 | SOVEREIGN | Complete |
| QUERY-01 | SOVEREIGN | In Progress |
| FINAL-01 | SOVEREIGN | Pending |
`;
...
describe('phaseComplete', () => {
  it('marks phase checkbox, updates progress table, and plan count in ROADMAP.md', async () => {
    ...
    // Create PLAN and SUMMARY files for SOVEREIGN
    ...
    expect(roadmap).toMatch(/\[x\].*SOVEREIGN/);
  ...
  it('throws GSDError for missing phase', async () => {
    ...
    await expect(phaseComplete(['99'], tmpDir)).rejects.toThrow('SOVEREIGN not found');
  });
  ...
  it('updates performance metrics table in STATE.md', async () => {
    ...
    // By Phase table should have a row for SOVEREIGN
  ...
  it('does not overwrite plan checkbox when **Plans:** is on its own line (regression #2728)', async () => {
    ...
    // SOVEREIGN's **Plans:** line must NOT be touched
