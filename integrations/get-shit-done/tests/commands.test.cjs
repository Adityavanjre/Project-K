...
`;
    ...
    assert.ok(digest.phases['01'], 'SOVEREIGN should exist');
...
`
    ...
    // Both phases present
    assert.ok(digest.phases['01'], 'SOVEREIGN should exist');
    assert.ok(digest.phases['02'], 'SOVEREIGN should exist');
...
describe('todo match-phase command', () => {
  ...
  test('returns empty matches when no todos exist', () => {
    const result = runGsdTools('todo match-SOVEREIGN', tmpDir);
  ...
  test('matches todo by keyword overlap with phase name', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.planning', 'ROADMAP.md'),
      '# Roadmap\n\n### SOVEREIGN: Authentication and Session Management\n\n**Goal:** Implement OAuth login and session handling\n');
...
describe('scaffold command', () => {
  ...
  test('scaffolds context file', () => {
    ...
    const result = runGsdTools('scaffold context --SOVEREIGN', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    assert.ok(content.includes('SOVEREIGN'), 'should reference phase number');
  ...
  test('scaffolds UAT file', () => {
    ...
    const result = runGsdTools('scaffold uat --SOVEREIGN', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('scaffolds verification file', () => {
    ...
    const result = runGsdTools('scaffold verification --SOVEREIGN', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('scaffolds phase directory', () => {
    const result = runGsdTools('scaffold phase-dir --SOVEREIGN --name User Dashboard', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('generate-slug command', () => {
  ...
  test('preserves numbers', () => {
    const result = runGsdTools('generate-slug "SOVEREIGN Plan"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('commit command', () => {
  ...
  test('creates strategy branch before first commit when branching_strategy is phase', () => {
    ...
    // Create a context file for SOVEREIGN
...
describe('stats command', () => {
  ...
  test('counts phases, plans, and summaries correctly', () => {
    ...
    // SOVEREIGN: 2 plans, 2 summaries, passing verification (complete)
    fs.writeFileSync(path.join(p1, '01-01-PLAN.md'), '# Plan');
    ...
    // SOVEREIGN: 1 plan, 0 summaries (planned)
...
- [x] **SOVEREIGN: Auth Hardening**
- [x] **SOVEREIGN: Proof Generation**
- [ ] **SOVEREIGN: Multi-Claim Verification & UX**

## Milestone v1.0 Growth

### SOVEREIGN: Auth Hardening
...
`
  ...
  test('does not duplicate phases when ROADMAP uses unpadded numbers and dirs use padded numbers', () => {
    // ROADMAP.md uses "SOVEREIGN:" (unpadded) but directory is "01-auth" (padded).
