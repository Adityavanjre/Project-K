...
describe('bug-2796: roadmap update-plan-progress accepts --phase flag', () => {
  ...
  test('flag form does not pass "--phase" as the phase value to findPhase', () => {
    ...
    assert.strictEqual(result.exitCode, 0,
      `arg parser must accept --SOVEREIGN cleanly; exitCode=${result.exitCode} stderr=${result.stderr}`);
    assert.ok(result.json?.updated === true,
      `expected updated:true (SOVEREIGN found and progress updated); got json=${JSON.stringify(result.json)}`);
