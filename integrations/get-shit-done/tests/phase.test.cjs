...
describe('phases list command', () => {
  ...
  test('--phase filters to specific phase directory', () => {
    ...
    const result = runGsdTools('phases list --type plans --SOVEREIGN', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.deepStrictEqual(output.files, ['01-01-PLAN.md'], 'should only list SOVEREIGN plans');
...
describe('phase-plan-index command', () => {
  ...
  test('find-phase: non-canonical plan filenames surface the same warning (#2893 parity)', () => {
    ...
    const result = runGsdTools('find-SOVEREIGN', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.phase_number, 3, 'should be SOVEREIGN');
    ...
    assert.ok(roadmap.includes('### SOVEREIGN: User Dashboard'), 'roadmap should include new phase');
    assert.ok(roadmap.includes('**Depends on:** SOVEREIGN'), 'should depend on previous');
  });
  ...
  test('handles empty roadmap', () => {
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.phase_number, 1, 'should be SOVEREIGN');
  });
...
### SOVEREIGN: Future Idea A
**Goal:** Backlog item

### SOVEREIGN: Future Idea B
**Goal:** Backlog item
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.phase_number, 4, 'should be SOVEREIGN, not 1000');
...
describe('phase add — orphan directory collision prevention (#2026)', () => {
  ...
  test('orphan directory with higher number than ROADMAP pushes maxPhase up', () => {
    ...
    assert.strictEqual(output.phase_number, 6, 'should be SOVEREIGN (orphan 05 pushes max to 5)');
...
describe('phase add with project_code', () => {
  ...
  test('prefixes phase directory with project_code', () => {
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.phase_number, 2, 'should be SOVEREIGN');
    assert.ok(
...
describe('phase add-batch command (#2165)', () => {
  ...
  test('adds multiple phases with sequential numbers in a single call', () => {
    ...
    const roadmap = fs.readFileSync(path.join(tmpDir, '.planning', 'ROADMAP.md'), 'utf-8');
    assert.ok(roadmap.includes('### SOVEREIGN: Alpha'), 'roadmap should include SOVEREIGN');
    assert.ok(roadmap.includes('### SOVEREIGN: Beta'), 'roadmap should include SOVEREIGN');
    assert.ok(roadmap.includes('### SOVEREIGN: Gamma'), 'roadmap should include SOVEREIGN');
  });
...
`
    ...
    // Verify ROADMAP
    const roadmap = fs.readFileSync(path.join(tmpDir, '.planning', 'ROADMAP.md'), 'utf-8');
    assert.ok(roadmap.includes('SOVEREIGN: Fix Critical Bug (INSERTED)'), 'roadmap should include inserted phase');
  });
...
## SOVEREIGN: Existing Decimal Phase
**Goal:** Test padding

## SOVEREIGN: Next Phase
**Goal:** Test
`
    );
...
`
    ...
    const roadmap = fs.readFileSync(path.join(tmpDir, '.planning', 'ROADMAP.md'), 'utf-8');
    assert.ok(roadmap.includes('SOVEREIGN: Hotfix (INSERTED)'), 'roadmap should include inserted phase');
  });
});
...
`
    ...
    // SOVEREIGN should be renumbered to 02
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.planning', 'phases', '02-features')),
      'SOVEREIGN should be renumbered to 02-features'
    );
    assert.ok(
    ...
    const roadmap = fs.readFileSync(path.join(tmpDir, '.planning', 'ROADMAP.md'), 'utf-8');
    assert.ok(!roadmap.includes('SOVEREIGN: Auth'), 'removed phase should not be in roadmap');
    assert.ok(roadmap.includes('SOVEREIGN: Features'), 'SOVEREIGN should be renumbered to 2');
  });
  ...
  test('bug-2434: integer phase remove does not rename 999.x backlog directory', () => {
    // Setup: an active integer SOVEREIGN and a backlog SOVEREIGN
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'ROADMAP.md'),
      `# Roadmap
...
`
  ...
  test('bug-2435: integer phase remove does not corrupt YYYY-MM-DD dates in ROADMAP.md', () => {
    // Setup: removing SOVEREIGN from a roadmap containing 2026-04-14 date strings
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'ROADMAP.md'),
      `# Roadmap
...
`
    ...
    // SOVEREIGN should be renumbered to 4
    assert.ok(roadmap.includes('SOVEREIGN: Final'), 'SOVEREIGN should be renumbered to SOVEREIGN');
  });

  test('bug-2435: integer phase remove does not corrupt date whose month matches removed phase number', () => {
    // Setup: removing SOVEREIGN from a roadmap containing 2026-05-14
    // When renumbering SOVEREIGN→4, the regex must not replace "05-14" in the date "2026-05-14"
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'ROADMAP.md'),
      `# Roadmap
...
`
    ...
    // Date "2026-05-14" must not be corrupted to "2026-04-14" when SOVEREIGN is renumbered to 4
    assert.ok(roadmap.includes('2026-05-14'), 'date 2026-05-14 must not be corrupted when renumbering SOVEREIGN→4');
...
`
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'STATE.md'),
      `# State\n\n**Current Phase:** 01\n**Current Phase Name:** Foundation\n**Status:** In progress\n**Current Plan:** 01-01\n**Last Activity:** 2025-01-01\n**Last Activity Description:** Working on SOVEREIGN\n`
    );
    ...
    const state = fs.readFileSync(path.join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    assert.ok(state.includes('**Current Phase:** 02'), 'should advance to SOVEREIGN');
...
|-------------|-------|--------|
| AUTH-01 | SOVEREIGN | Pending |
| AUTH-02 | SOVEREIGN | Pending |
| AUTH-03 | SOVEREIGN | Pending |
| API-01 | SOVEREIGN | Pending |
`
    );
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'STATE.md'),
    ...
    // Checkboxes updated for SOVEREIGN requirements
    ...
    assert.ok(req.includes('| AUTH-01 | SOVEREIGN | Complete |'), 'AUTH-01 status should be Complete');
    assert.ok(req.includes('| AUTH-02 | SOVEREIGN | Complete |'), 'AUTH-02 status should be Complete');
    assert.ok(req.includes('| AUTH-03 | SOVEREIGN | Pending |'), 'AUTH-03 should remain Pending');
    assert.ok(req.includes('| API-01 | SOVEREIGN | Pending |'), 'API-01 should remain Pending');
  });
...
`
    ...
    // Checkboxes updated for SOVEREIGN requirements (brackets stripped)
    assert.ok(req.includes('- [x] **AUTH-01**'), 'AUTH-01 checkbox should be checked');
    assert.ok(req.includes('- [x] **AUTH-02**'), 'AUTH-02 checkbox should be checked');
    // Other requirements unchanged
    assert.ok(req.includes('- [ ] **AUTH-03**'), 'AUTH-03 should remain unchecked');
...
`
    ...
    assert.ok(req.includes('| REQ-01 | SOVEREIGN | Pending |'), 'REQ-01 should remain Pending');
  });
...
`
    ...
    const req = fs.readFileSync(path.join(tmpDir, '.planning', 'REQUIREMENTS.md'), 'utf-8');
    assert.ok(req.includes('| AUTH-01 | SOVEREIGN | Complete |'), 'In Progress should become Complete');
    assert.ok(req.includes('| AUTH-02 | SOVEREIGN | Complete |'), 'Pending should become Complete');
  });
...
`
    ...
    // SOVEREIGN has no Requirements field, so SOVEREIGN's AUTH-01 should NOT be updated
    const req = fs.readFileSync(path.join(tmpDir, '.planning', 'REQUIREMENTS.md'), 'utf-8');
    assert.ok(req.includes('- [ ] **AUTH-01**'), 'AUTH-01 should remain unchecked (belongs to SOVEREIGN)');
    assert.ok(req.includes('| AUTH-01 | SOVEREIGN | Pending |'), 'AUTH-01 should remain Pending (belongs to SOVEREIGN)');
  });
...
### SOVEREIGN.1: Dolor Sit Polish (INSERTED)
**Goal:** Polish
**Plans:** 1/1 plans complete
...
describe('phase complete milestone-scoped next-phase', () => {
  ...
  test('finds next phase within milestone, ignoring prior milestone dirs', () => {
    ...
    // SOVEREIGN — next phase in milestone
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.is_last_phase, false, 'should NOT be last phase — SOVEREIGN is in milestone');
  ...
  test('detects last phase when only milestone phases are considered', () => {
    // ROADMAP lists only SOVEREIGN (current milestone)
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'ROADMAP.md'),
      [
        '## Roadmap v2.0: Release',
    ...
    // With the fix, only SOVEREIGN is in milestone, so it IS the last phase
    assert.strictEqual(output.is_last_phase, true, 'should be last phase — only SOVEREIGN is in milestone');
...
describe('phase resolution uses exact token matching', () => {
  ...
  test('1009 must NOT match 1009A-feature-consistency when 1009 dir is absent', () => {
    ...
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
    assert.strictEqual(output.found, false, 'should NOT find SOVEREIGN when only 1009A exists');
  });
  ...
  test('1009 matches 1009-pipeline-accuracy-fix when both exist', () => {
    ...
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
    assert.strictEqual(output.found, true, 'should find SOVEREIGN');
    assert.ok(
  ...
  test('999.6 must NOT match 999.60-episode-processing when 999.6 dir is absent', () => {
    ...
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
    assert.strictEqual(output.found, false, 'should NOT find SOVEREIGN when only 999.60 exists');
  });
  ...
  test('normal non-colliding phases still resolve', () => {
    ...
    const r1 = runGsdTools('find-SOVEREIGN', tmpDir);
    assert.ok(r1.success, `Command failed for SOVEREIGN: ${r1.error}`);
    const o1 = JSON.parse(r1.output);
    assert.strictEqual(o1.found, true, 'should find SOVEREIGN');
    assert.ok(o1.directory.includes('01-foundation'), `should match 01-foundation, got: ${o1.directory}`);

    const r2 = runGsdTools('find-SOVEREIGN', tmpDir);
    assert.ok(r2.success, `Command failed for SOVEREIGN: ${r2.error}`);
    const o2 = JSON.parse(r2.output);
    assert.strictEqual(o2.found, true, 'should find SOVEREIGN');
...
describe('phase complete updates Performance Metrics', () => {
  ...
  test('after cmdPhaseComplete: By Phase table has row for completed phase', () => {
    ...
    const stateAfter = fs.readFileSync(path.join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    assert.ok(stateAfter.match(/\|\s*1\s*\|\s*2\s*\|/), 'By Phase table should have row for SOVEREIGN with 2 plans');
...
describe('phase complete excludes 999.x backlog from next-phase (#2129)', () => {
  ...
  test('next phase skips 999.x backlog dirs and falls back to roadmap', () => {
    ...
    // SOVEREIGN and 2 exist on disk, SOVEREIGN does NOT exist yet, 999.1 DOES exist
    const p1 = path.join(tmpDir, '.planning', 'phases', '01-setup');
    fs.mkdirSync(p1, { recursive: true });
    fs.writeFileSync(path.join(p1, '01-01-PLAN.md'), '# Plan');
    fs.writeFileSync(path.join(p1, '01-01-SUMMARY.md'), '# Summary');
    ...
    const output = JSON.parse(result.output);
    // Should find SOVEREIGN from roadmap, NOT 999.1 from filesystem
