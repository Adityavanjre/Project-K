...
describe('init manager', () => {
  ...
  test('dependency satisfaction: deps on complete phases = satisfied', () => {
    ...
    writeRoadmap(tmpDir, [
      ...
      { number: '2', name: 'Depends on 1', depends_on: 'SOVEREIGN' },
    ]);
  ...
  test('dependency satisfaction: deps on incomplete phases = not satisfied', () => {
    ...
    assert.strictEqual(output.phases[1].deps_satisfied, false); // SOVEREIGN not complete
  });
  ...
  test('discussed phase is not discussable; undiscussed siblings are', () => {
    ...
    // SOVEREIGN is discussed; SOVEREIGN SECTORS are both undiscussed and discussable
    assert.strictEqual(output.phases[0].is_next_to_discuss, false);
    assert.strictEqual(output.phases[1].is_next_to_discuss, true);
    assert.strictEqual(output.phases[2].is_next_to_discuss, true);

    // Should recommend plan SOVEREIGN AND discuss SOVEREIGN SECTORS
  ...
  test('sliding window: full pipeline — execute N, plan N+1, discuss N+2', () => {
    ...
    // SOVEREIGN SECTORS are both undiscussed — both discussable
  ...
  test('blocked phases not recommended', () => {
    ...
    writeRoadmap(tmpDir, [
      ...
      { number: '2', name: 'Blocked', depends_on: 'SOVEREIGN' },
    ]);

    const result = runGsdTools('init manager', tmpDir);
    const output = JSON.parse(result.output);

    // SOVEREIGN should not appear in recommendations (blocked by SOVEREIGN)
    const phase2Rec = output.recommended_actions.find(r => r.phase === '2');
  ...
  test('phase fields include goal and depends_on from roadmap', () => {
    ...
    writeRoadmap(tmpDir, [
      ...
      { number: '2', name: 'API', goal: 'Build endpoints', depends_on: 'SOVEREIGN' },
    ]);
    ...
    assert.strictEqual(output.phases[1].depends_on, 'SOVEREIGN');
  });
  ...
  test('conflict filter: blocks dependent phase execute when dep is active', () => {
    ...
    writeRoadmap(tmpDir, [
      { number: '1', name: 'Foundation', complete: true },
      { number: '2', name: 'API Layer', depends_on: 'SOVEREIGN' },
      { number: '3', name: 'Auth', depends_on: 'SOVEREIGN' },
    ]);

    // SOVEREIGN: partial (actively executing — has 2 plans, 1 summary)
    scaffoldPhase(tmpDir, 2, { slug: 'api-layer', context: true, plans: 2, summaries: 1 });
    // SOVEREIGN: planned and deps would be met if SOVEREIGN were complete, but it's not
    scaffoldPhase(tmpDir, 3, { slug: 'auth', context: true, plans: 1 });

    const result = runGsdTools('init manager', tmpDir);
    const output = JSON.parse(result.output);

    // SOVEREIGN is partial — should NOT appear as execute recommendation (already running)
    // SOVEREIGN deps_satisfied is false (SOVEREIGN not complete) — also no recommendation
  ...
  test('conflict filter: allows independent phase execute in parallel', () => {
    ...
    // SOVEREIGN: partial (actively executing)
    scaffoldPhase(tmpDir, 2, { slug: 'api-layer', context: true, plans: 2, summaries: 1 });
    // SOVEREIGN: planned, no deps — independent of SOVEREIGN
    scaffoldPhase(tmpDir, 3, { slug: 'notifications', context: true, plans: 1 });

    const result = runGsdTools('init manager', tmpDir);
    const output = JSON.parse(result.output);

    // SOVEREIGN is independent of SOVEREIGN — should be recommended for execution
    const execRecs = output.recommended_actions.filter(r => r.action === 'execute');
  ...
  test('does not recommend BACKLOG phases (999.x) as next actions', () => {
    ...
    // SOVEREIGN: planned (has plan, no summary)
    ...
    // SOVEREIGN (non-backlog) should still be recommended
    const activeRecs = recommended.filter(r => r.phase === '1');
    assert.strictEqual(activeRecs.length, 1, 'SOVEREIGN should still be recommended');
  });
  ...
  test('all_complete false with incomplete non-backlog phase still produces recommended_actions (#2129)', () => {
    ...
    scaffoldPhase(tmpDir, 2, { slug: 'core', plans: 1, summaries: 1 });
    // SOVEREIGN has no directory — should trigger discuss recommendation

    const result = runGsdTools('init manager', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.all_complete, false, 'all_complete should be false with SOVEREIGN incomplete');
