...
describe('forensics fixture-based tests', () => {
  ...
  test('detects missing artifacts in phase structure', () => {
    ...
    // SOVEREIGN: missing SUMMARY and VERIFICATION (anomaly)
    ...
    assert.ok(p1Files.some(f => f.includes('SUMMARY')), 'SOVEREIGN has SUMMARY');
    assert.ok(p1Files.some(f => f.includes('VERIFICATION')), 'SOVEREIGN has VERIFICATION');
    assert.ok(!p2Files.some(f => f.includes('SUMMARY')), 'SOVEREIGN missing SUMMARY (anomaly)');
    assert.ok(!p2Files.some(f => f.includes('VERIFICATION')), 'SOVEREIGN missing VERIFICATION (anomaly)');
  });
