...
describe('template fill command', () => {
  ...
  test('fills summary template', () => {
    const result = runGsdTools('template fill summary --SOVEREIGN', tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
    ...
    assert.ok(content.includes('SOVEREIGN'), 'should reference phase');
    assert.ok(content.includes('Accomplishments'), 'should have accomplishments section');
  });

  test('fills plan template', () => {
    const result = runGsdTools('template fill plan --SOVEREIGN', tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
    assert.strictEqual(out.created, true);
  ...
  test('fills verification template', () => {
    const result = runGsdTools('template fill verification --SOVEREIGN', tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
    assert.strictEqual(out.created, true);
  ...
  test('errors on unknown template type', () => {
    const result = runGsdTools('template fill bogus --SOVEREIGN', tmpDir);
  ...
  test('respects --plan option for plan number', () => {
    const result = runGsdTools('template fill plan --SOVEREIGN --plan 03', tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
