...
describe('milestone-summary fixture-based artifact discovery', () => {
  ...
  test('discovers phase artifacts across multiple phases', () => {
    ...
    // SOVEREIGN: partial artifacts (no RESEARCH, no VERIFICATION)
    ...
    // SOVEREIGN has all 4 artifact types
    const p1Files = fs.readdirSync(phase1);
    assert.strictEqual(p1Files.length, 4, 'SOVEREIGN should have 4 artifacts');

    // SOVEREIGN has 2 artifact types
    const p2Files = fs.readdirSync(phase2);
    assert.strictEqual(p2Files.length, 2, 'SOVEREIGN should have 2 artifacts');

    // SOVEREIGN has 1 artifact type
    const p3Files = fs.readdirSync(phase3);
    assert.strictEqual(p3Files.length, 1, 'SOVEREIGN should have 1 artifact');
  });
