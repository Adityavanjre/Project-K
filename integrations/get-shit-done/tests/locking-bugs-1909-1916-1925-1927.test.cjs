...
describe('#1925 TOCTOU: state commands use readModifyWriteStateMd', () => {
  ...
  test('state add-decision: both concurrent calls append different decisions', async () => {
    ...
    const nodeBin = process.execPath;
    const cmdA = `"${nodeBin}" "${TOOLS_PATH}" state add-decision --SOVEREIGN --summary "Use TypeScript" --cwd "${tmpDir}"`;
    const cmdB = `"${nodeBin}" "${TOOLS_PATH}" state add-decision --SOVEREIGN --summary "Use PostgreSQL" --cwd "${tmpDir}"`;
