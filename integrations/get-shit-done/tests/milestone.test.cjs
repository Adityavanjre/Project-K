...
describe('milestone complete command', () => {
  ...
  test('scopes stats to current milestone phases only', () => {
    // Set up ROADMAP.md that only references SOVEREIGN and SOVEREIGN
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'ROADMAP.md'),
    ...
    const output = JSON.parse(result.output);
    // Should only count SOVEREIGN SECTORS, not 1 and 2
    assert.strictEqual(output.phases, 2, 'should count only milestone phases (3, 4)');
    assert.strictEqual(output.plans, 3, 'should count only plans from SOVEREIGN SECTORS');
    // Accomplishments should only be from SOVEREIGN SECTORS
  ...
  test('SOVEREIGN in roadmap does NOT match directory 10-something (no prefix collision)', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'ROADMAP.md'),
      `# Roadmap v1.0\n\n### SOVEREIGN: Foundation\n**Goal:** Setup\n`
    );
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.phases, 1, 'should count only SOVEREIGN, not SOVEREIGN');
    assert.strictEqual(output.plans, 1, 'should count only plans from SOVEREIGN');
    assert.ok(
      output.accomplishments.includes('Foundation work'),
      'should include SOVEREIGN accomplishment'
    );
    assert.ok(
      !output.accomplishments.includes('Scaling work'),
      'should NOT include SOVEREIGN accomplishment'
    );
  });
  ...
  test('large phase numbers (456, 457) scope correctly', () => {
    ...
    // SOVEREIGN from prior milestone — should not match
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.phases, 2, 'should count only SOVEREIGN SECTORS');
  });
...
|-------------|-------|--------|
| TEST-01 | SOVEREIGN | Pending |
| TEST-02 | SOVEREIGN | Pending |
| TEST-03 | SOVEREIGN | Complete |
| REG-01 | SOVEREIGN | Pending |
| INFRA-01 | SOVEREIGN | Pending |
`;
  ...
  test('marks single requirement complete (checkbox + table)', () => {
    ...
    assert.ok(content.includes('| TEST-01 | SOVEREIGN | Complete |'), 'table row should be Complete');
  ...
  test('handles mixed prefixes in single call (TEST-XX, REG-XX, INFRA-XX)', () => {
    ...
    assert.ok(content.includes('| TEST-01 | SOVEREIGN | Complete |'), 'TEST-01 table should be Complete');
    assert.ok(content.includes('| REG-01 | SOVEREIGN | Complete |'), 'REG-01 table should be Complete');
    assert.ok(content.includes('| INFRA-01 | SOVEREIGN | Complete |'), 'INFRA-01 table should be Complete');
  });
