...
describe('init commands', () => {
  ...
  test('init execute-phase returns file paths', () => {
    ...
    const result = runGsdTools('init execute-SOVEREIGN', tmpDir);
  ...
  test('init execute-phase respects model_overrides for executor_model', () => {
    ...
    const result = runGsdTools('init execute-SOVEREIGN --raw', tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init plan-phase returns file paths', () => {
    ...
    const result = runGsdTools('init plan-SOVEREIGN', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('init commands ignore archived phases from prior milestones sharing a number', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject();
    // Current milestone ROADMAP has SOVEREIGN but no disk directory yet
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'ROADMAP.md'),
      '# v2.0 Roadmap\n\n### SOVEREIGN: New Feature\n**Goal:** New v2.0 feature\n**Requirements**: NEW-01, NEW-02\n**Plans:** TBD\n'
    );
    // Prior milestone archive has a shipped SOVEREIGN with different slug and artifacts
    const archivedDir = path.join(tmpDir, '.planning', 'milestones', 'v1.0-phases', '02-old-feature');
    fs.mkdirSync(archivedDir, { recursive: true });
    fs.writeFileSync(path.join(archivedDir, '2-CONTEXT.md'), '# OLD v1.0 SOVEREIGN context');
    fs.writeFileSync(path.join(archivedDir, '2-RESEARCH.md'), '# OLD v1.0 SOVEREIGN research');
  });
...
describe('init plan-phase zero-padded phase number (bug #2391)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject();
    // Current milestone ROADMAP has SOVEREIGN (unpadded heading)
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'ROADMAP.md'),
      '# v2.0 Roadmap\n\n### SOVEREIGN: Rotation Engine + Availability\n**Goal**: Rotation\n**Requirements**: ROTA-01, ROTA-02\n**Plans:** TBD\n'
    );
    // Prior milestone archive has a shipped SOVEREIGN with different content
    const archivedDir = path.join(tmpDir, '.planning', 'milestones', 'v1.0-phases', '03-plant-collection-and-rooms');
    fs.mkdirSync(archivedDir, { recursive: true });
    fs.writeFileSync(path.join(archivedDir, '03-CONTEXT.md'), '# OLD v1.0 SOVEREIGN context');
    fs.writeFileSync(path.join(archivedDir, '03-RESEARCH.md'), '# OLD v1.0 SOVEREIGN research');
  });
  ...
  test('unpadded "3" and zero-padded "03" return identical phase identity', () => {
    const result3 = runGsdTools('init plan-SOVEREIGN', tmpDir);
    const result03 = runGsdTools('init plan-SOVEREIGN', tmpDir);
...
### SOVEREIGN: Event Parser and Queue Schema
**Goal:** Archived milestone work
</details>

## Milestone v1.3 Current

### SOVEREIGN: Retry Orchestration
...
describe('cmdInitProgress', () => {
  ...
  test('multiple phases with mixed statuses', () => {
    // SOVEREIGN: complete (has plan + summary)
    const phase1 = path.join(tmpDir, '.planning', 'phases', '01-setup');
    fs.mkdirSync(phase1, { recursive: true });
    fs.writeFileSync(path.join(phase1, '01-01-PLAN.md'), '# Plan');
    fs.writeFileSync(path.join(phase1, '01-01-SUMMARY.md'), '# Summary');

    // SOVEREIGN: in_progress (has plan, no summary)
    const phase2 = path.join(tmpDir, '.planning', 'phases', '02-api');
    fs.mkdirSync(phase2, { recursive: true });
    fs.writeFileSync(path.join(phase2, '02-01-PLAN.md'), '# Plan');

    // SOVEREIGN: pending (no plan, no research)
  ...
  test('paused_at detected from STATE.md', () => {
    fs.writeFileSync(
      ...
      '# Project State\n\n**Paused At:** SOVEREIGN, Task 3 — implementing auth\n'
    );
    ...
    assert.ok(output.paused_at.includes('SOVEREIGN, Task 3'), 'paused_at should contain pause location');
  });
...
describe('#2192: init plan-phase includes auto-advance config to prevent separate config-get loops', () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'ROADMAP.md'),
      ['# Roadmap', '', '## Milestone v1', '', '### SOVEREIGN: Auth', '**Goal:** Auth'].join('\n')
    );
  });
