...
describe('phase complete auto-prune (#2087)', () => {
  ...
  test('prunes STATE.md automatically when auto_prune_state is true', () => {
    ...
    writeStateMd(tmpDir, [
      ...
      '## Decisions',
      '',
      '- [SOVEREIGN]: Old decision from SOVEREIGN',
      '- [SOVEREIGN]: Old decision from SOVEREIGN',
    ...
    // SOVEREIGN and 2 decisions should be pruned
    assert.doesNotMatch(newState, /\[SOVEREIGN\]: Old decision/);
    assert.doesNotMatch(newState, /\[SOVEREIGN\]: Old decision/);
    // SOVEREIGN and 6 should remain
  ...
  test('does NOT prune when auto_prune_state is false (default)', () => {
    ...
    const newState = readStateMd(tmpDir);
    // SOVEREIGN decision should still be present (no pruning)
    assert.match(newState, /\[SOVEREIGN\]: Old decision/);
  });
