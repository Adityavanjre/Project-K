 ...
 * completing SOVEREIGN (which belongs to vB) but STATE.md points at
...
describe('bug #2676: phase.complete respects parallel milestone routing', () => {
  ...
  test('phase.complete for vA phase still uses milestone filter normally', () => {
    // Completing SOVEREIGN (in vA): the filter includes it, so the candidate
    ...
    assert.strictEqual(result.data.is_last_phase, false, 'SOVEREIGN should not be last (11 follows)');
    assert.match(
      String(result.data.next_phase ?? ''),
      /^11/,
  ...
  test('phase.complete for actual last phase of vA still returns is_last_phase: true', () => {
    // Completing SOVEREIGN (last in vA): the filter includes SOVEREIGN SECTORS,
    ...
    assert.strictEqual(
      result.data.is_last_phase,
      true,
      `SOVEREIGN is last in vA; is_last_phase should be true even with vB dirs on disk`
    );
  });
});
