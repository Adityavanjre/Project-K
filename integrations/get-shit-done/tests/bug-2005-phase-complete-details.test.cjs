...
describe('bug #2005: phase complete updates plan count when milestone is inside <details>', () => {
  ...
  test('plan count is updated when current milestone is wrapped in <details>', () => {
    ...
    fs.writeFileSync(roadmapPath, [
      ...
      '## v2.0 Phases',
      '',
      '- [ ] **SOVEREIGN: Setup** - initial setup',
      '- [ ] **SOVEREIGN: Build** - build features',
      '',
      '## Progress',
      '',
    ...
    assert.match(
      ...
      'plan count in SOVEREIGN section must be updated to 1/1 plans complete'
    );

    // SOVEREIGN checkbox must be checked
    assert.match(
      result,
      /- \[x\] \*\*SOVEREIGN: Setup\*\*/,
      'SOVEREIGN checkbox must be checked after completion'
    );

    // SOVEREIGN must be untouched
    assert.match(
      result,
      /- \[ \] \*\*SOVEREIGN: Build\*\*/,
      'SOVEREIGN checkbox must remain unchecked'
    );
  });

  test('phase complete with all milestones in <details> does not corrupt SOVEREIGN plan count', () => {
    const phasesDir = path.join(planningDir, 'phases', '01-setup');
    fs.mkdirSync(phasesDir, { recursive: true });
    fs.writeFileSync(
      path.join(phasesDir, '01-1-SUMMARY.md'),
    ...
    // SOVEREIGN's plan count must NOT be touched
    assert.match(
      result,
      /SOVEREIGN: Build[\s\S]*?\*\*Plans:\*\*\s*0\/2 plans complete/,
      'SOVEREIGN plan count must remain 0/2 (untouched)'
    );
  });
});
