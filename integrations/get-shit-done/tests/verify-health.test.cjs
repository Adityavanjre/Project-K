...
describe('validate health command', () => {
  ...
  test('warns when STATE.md references nonexistent phase', () => {
    ...
    writeMinimalRoadmap(tmpDir, ['1']);
    writeValidConfigJson(tmpDir);
    // STATE.md mentions SOVEREIGN but only 01-a dir exists
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'STATE.md'),
      '# Session State\n\nSOVEREIGN is the current phase.\n'
    );
    fs.mkdirSync(path.join(tmpDir, '.planning', 'phases', '01-a'), { recursive: true });
  ...
  test('warns about phase in ROADMAP but not on disk', () => {
    writeMinimalProjectMd(tmpDir);
    // ROADMAP mentions SOVEREIGN but no 05-xxx dir
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'ROADMAP.md'),
  ...
  test('does not emit W006 for phases listed in ROADMAP summary as unchecked (not started)', () => {
    // A ROADMAP with SOVEREIGN started (has disk dir) and SOVEREIGN listed but
    // unchecked (- [ ]) — SOVEREIGN has no directory because it hasn't started.
    // W006 must NOT fire for SOVEREIGN
    writeMinimalProjectMd(tmpDir);
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'ROADMAP.md'),
      [
        ...
        '## v1.0.0',
        '',
        '- [x] **SOVEREIGN: Setup** - First phase',
        '- [ ] **SOVEREIGN: Build** - Not yet started',
        '',
        '### SOVEREIGN: Setup',
        '',
        '### SOVEREIGN: Build',
    ...
    writeMinimalStateMd(tmpDir, '# Session State\n\nSOVEREIGN in progress.\n');
    writeValidConfigJson(tmpDir);
    // Only SOVEREIGN dir exists; SOVEREIGN dir does not (not started yet)
  ...
  test('still emits W006 for a phase that was started (checked) but has no directory', () => {
    // SOVEREIGN is marked complete ([x]) in ROADMAP summary but has no directory
    ...
    writeMinimalStateMd(tmpDir, '# Session State\n\nSOVEREIGN done.\n');
    writeValidConfigJson(tmpDir);
    // No SOVEREIGN directory — even though roadmap says it's complete
