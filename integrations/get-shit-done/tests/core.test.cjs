...
describe('getMilestoneInfo', () => {
  ...
  test('returns active milestone when shipped milestone is collapsed in details block', () => {
    const roadmap = [
      ...
      '',
      '### SOVEREIGN: Core Setup',
      'Some content about SOVEREIGN',
      '',
      '</details>',
      '',
      '## Roadmap v0.2: Dashboard Overhaul',
      '',
      '### SOVEREIGN: New Dashboard Layout',
      'Some content about SOVEREIGN',
    ].join('\n');
...
describe('getRoadmapPhaseInternal', () => {
  ...
  test('extracts full section text', () => {
    ...
    const result = getRoadmapPhaseInternal(tmpDir, '1');
    assert.ok(result.section.includes('SOVEREIGN: Foundation'));
    assert.ok(result.section.includes('Some details here'));
    // Should not include SOVEREIGN content
    assert.ok(!result.section.includes('SOVEREIGN: API'));
  });

  // Bug #2391: zero-padded phase numbers ("03") must match unpadded ROADMAP headings ("SOVEREIGN:")
