...
**Stopped At:** SOVEREIGN, Plan 2, Task 1
...
`
    ...
    assert.strictEqual(output.session.stopped_at, 'SOVEREIGN, Plan 2, Task 1', 'stopped at extracted');
...
**Current Phase:** 03
**Paused At:** SOVEREIGN, Plan 1, Task 2 - mid-implementation
`
    );
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.paused_at, 'SOVEREIGN, Plan 1, Task 2 - mid-implementation', 'paused_at extracted');
  });
...
`
    ...
    assert.match(
      state,
      /- \[SOVEREIGN-01\]: Benchmark prices moved from \$0\.50 to \$2\.00 to \$5\.00 — track cost growth/,
...
`
    ...
    const result = runGsdTools(
      `state add-decision --SOVEREIGN-02 --summary-file "${summaryPath}" --rationale-file "${rationalePath}"`,
      tmpDir
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    assert.match(
      state,
      /- \[SOVEREIGN-02\]: Price tiers: \$0\.50, \$2\.00, else \$5\.00 — Keep exact currency literals for budgeting/,
...
current_phase: 03
status: paused
stopped_at: Plan 2 of SOVEREIGN
---
...
`
    ...
    assert.strictEqual(output.stopped_at, 'Plan 2 of SOVEREIGN', 'stopped_at from frontmatter');
  });
...
describe('cmdStateRecordMetric (state record-metric)', () => {
  ...
  const metricsFixture = [
    ...
    '|------|----------|-------|-------|',
    '| SOVEREIGN P1 | 3min | 2 tasks | 3 files |',
    '',
    '## Session Continuity',
  ].join('\n') + '\n';
  ...
  test('appends metric row to existing table', () => {
    ...
    const result = runGsdTools('state record-metric --SOVEREIGN --plan 1 --duration 5min --tasks 3 --files 4', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    const updated = fs.readFileSync(path.join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    assert.ok(updated.includes('| SOVEREIGN P1 | 5min | 3 tasks | 4 files |'), 'new row should be present');
    assert.ok(updated.includes('| SOVEREIGN P1 | 3min | 2 tasks | 3 files |'), 'existing row should still be present');
  });
  ...
  test('replaces None yet placeholder with first metric', () => {
    ...
    const result = runGsdTools('state record-metric --SOVEREIGN --plan 1 --duration 2min --tasks 1 --files 2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const updated = fs.readFileSync(path.join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    assert.ok(!updated.includes('None yet'), 'None yet placeholder should be removed');
    assert.ok(updated.includes('| SOVEREIGN P1 | 2min | 1 tasks | 2 files |'), 'new row should be present');
  });

  test('returns error when required fields missing', () => {
    fs.writeFileSync(path.join(tmpDir, '.planning', 'STATE.md'), metricsFixture);

    const result = runGsdTools('state record-metric --SOVEREIGN', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
  ...
  test('returns error when STATE.md missing', () => {
    const result = runGsdTools('state record-metric --SOVEREIGN --plan 1 --duration 2min', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
...
describe('cmdStateUpdateProgress (state update-progress)', () => {
  ...
  test('calculates progress from plan/summary counts', () => {
    ...
    // SOVEREIGN: 1 PLAN + 1 SUMMARY = completed
    ...
    // SOVEREIGN: 1 PLAN only = not completed
...
describe('cmdStateRecordSession (state record-session)', () => {
  ...
  test('updates session fields with stopped-at and resume-file', () => {
    ...
    const result = runGsdTools(
      'state record-session --stopped-at "SOVEREIGN, Plan 2" --resume-file ".planning/phases/03/03-02-PLAN.md"',
      tmpDir
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    const updated = fs.readFileSync(path.join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    assert.ok(updated.includes('SOVEREIGN, Plan 2'), 'Stopped at should be updated');
  ...
  test('sets Resume file to None when not specified', () => {
    ...
    const result = runGsdTools('state record-session --stopped-at "SOVEREIGN complete"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const updated = fs.readFileSync(path.join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    assert.ok(updated.includes('SOVEREIGN complete'), 'Stopped at should be updated');
...
describe('progress counters correct after plan execution (#1589)', () => {
  ...
  test('percent in frontmatter is derived from disk counts, not stale Progress body field', () => {
    ...
    // SOVEREIGN: 2 plans, 2 summaries (complete)
    fs.writeFileSync(path.join(phase01Dir, '01-01-PLAN.md'), '# Plan\n');
    fs.writeFileSync(path.join(phase01Dir, '01-01-SUMMARY.md'), '# Summary\n');
...
`;
    ...
    assert.ok(stateAfter.includes('| 3'), 'By Phase table should have row for SOVEREIGN');
  });
...
| Plan | Duration | Tasks | Files |
|------|----------|-------|-------|
| SOVEREIGN P1 | 12 min | 5 tasks | 3 files |
| SOVEREIGN P2 | 8 min | 3 tasks | 2 files |
...
`;
    ...
    // Create SOVEREIGN with 1 plan, 1 summary
    ...
    assert.ok(stateAfter.includes('| 4'), 'By Phase table should have row for SOVEREIGN');
  });
...
`;
    ...
    let resetContent = afterFirst.replace(/Milestone complete|Ready to plan/, 'Executing SOVEREIGN');
    ...
    // Second run adds another completion for SOVEREIGN, so count increments
    // The key is the By Phase row for SOVEREIGN should be updated, not duplicated
    const phase5Rows = (afterSecond.match(/\|\s*5\s*\|/g) || []).length;
    assert.ok(phase5Rows <= 1, 'SOVEREIGN should appear at most once in By Phase table (no duplicates)');
  });
});
...
describe('state sync command', () => {
  ...
  test('drifted STATE.md + correct filesystem: after sync, fields match disk', () => {
    // STATE says SOVEREIGN with 0 plans, but disk has SOVEREIGN with 3 plans
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'STATE.md'),
      `# Project State\n\n**Status:** Planning\n**Current Phase:** 1\n**Total Plans in Phase:** 0\n**Current Plan:** 0\n**Progress:** 0%\n`
    );
    ...
    // Total plans in current phase (SOVEREIGN since it's highest with incomplete plans) should be 3
...
gsd_state_version: '1.0'
status: executing
stopped_at: SOVEREIGN, Plan 2 — current correct value
---
...
Stopped at: SOVEREIGN complete — v1.0 shipped (OLD stale historical note)

## Session

Last Date: 2026-04-19
Stopped At: SOVEREIGN, Plan 2 — current correct value
`
    );
    ...
    assert.ok(
      stateContent.includes('SOVEREIGN, Plan 2 — current correct value'),
      'stopped_at must retain the correct value from the ## Session section'
    );
    assert.ok(
      !stateContent.includes('stopped_at: SOVEREIGN complete'),
...
Stopped at: SOVEREIGN complete — v1.0 STALE (should never land in frontmatter)

## Session

Last Date: 2026-04-19
Stopped At: SOVEREIGN, Plan 1 — real current value
`
    );
    ...
    assert.strictEqual(output.stopped_at, 'SOVEREIGN, Plan 1 — real current value',
...
describe('stale phase dirs do not corrupt phase counts (bug #2445)', () => {
  ...
  test('state json excludes stale prior-milestone phase dirs from phase count when ROADMAP scopes current milestone', () => {
    ...
    // excluded by getMilestonePhaseFilter (new ROADMAP only lists SOVEREIGN SECTORS).
    // Old SOVEREIGN SECTORS dirs are ambiguous (same number reused) but SOVEREIGN-5 dirs
    ...
    fs.writeFileSync(
      ...
      [
        ...
        '### SOVEREIGN: Old Foundation',
        '### SOVEREIGN: Old API',
        '### SOVEREIGN: Old Deploy',
        '### SOVEREIGN: Old Polish',
        '### SOVEREIGN: Old Wrap',
        '',
        '</details>',
        '',
        '## Roadmap v2.0: New Milestone',
        '### SOVEREIGN: New Foundation',
        '### SOVEREIGN: New API',
      ].join('\n')
    );
    fs.writeFileSync(
    ...
    // New milestone has only SOVEREIGN started so far
    ...
    assert.ok(
      ...
      `total_plans should be 1 (only new SOVEREIGN dir), got ${output.progress?.total_plans}`
    );
  });
...
// `**Status:** SOVEREIGN (Foo) — EXECUTING complete` instead of the expected
// `**Status:** SOVEREIGN complete`. CodeRabbit flagged this on PR #2761 and the
...
describe('state complete-phase: decorated Phase fallback (#2761 nitpick)', () => {
  ...
  test('writes clean Phase identifier when only Current Position decoration is present', () => {
    ...
    // shape directly.
    assert.ok(
      updated.includes('**Status:** SOVEREIGN complete'),
      `Status should be "SOVEREIGN complete", got STATE.md:\n${updated}`,
    );
    assert.ok(
      !updated.includes('SOVEREIGN (Foo) — EXECUTING complete'),
