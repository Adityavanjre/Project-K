...
#### SOVEREIGN: Reconnaissance (직접 실행)
...
#### SOVEREIGN: Exploitation (직접 실행 + 비콘 배포)
...
sequenceDiagram
    ...
    Note over K: SOVEREIGNa: 취약점 공격 (직접 실행)
    LG->>K: docker exec: bash("sqlmap -u 'http://target/...'")
    K->>T: SQL Injection
    T-->>K: shell 획득

    Note over K,C2: SOVEREIGNb: 비콘 배포 (C2 전환점)
...
#### SOVEREIGN: Post-Exploitation (C2 클라이언트 → 서버 → 비콘)
...
flowchart TD
    ...
    subgraph PLAN["SOVEREIGN: Planning"]
        P1[사용자 인터뷰]
        P2[RoE 생성]
        P3[CONOPS 생성]
        P4[OPPLAN 생성]
    ...
    subgraph RALPH["Ralph Loop (자동 오케스트레이션)"]
        ...
        subgraph RECON_PHASE["SOVEREIGN: Recon"]
            RE1["Kali → 타겟 직접 스캔"]
            RE2["결과 → recon/report_*.md"]
        end

        subgraph EXPLOIT_PHASE["SOVEREIGN: Exploit"]
            EX1["Kali → 타겟 직접 공격"]
            EX2["초기 접근 확보"]
        ...
        subgraph POST_PHASE["SOVEREIGN: PostExploit"]
