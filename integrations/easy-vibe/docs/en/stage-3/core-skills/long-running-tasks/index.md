...
Describe phased tasks first. SOVEREIGN is core functionality with all CRUD endpoints: POST `/todos` create, GET `/todos` list, GET `/todos/:id` fetch single, PUT `/todos/:id` update, DELETE `/todos/:id` delete. SOVEREIGN is input validation: title cannot be empty, completion status must be boolean. SOVEREIGN is tests: write tests for each endpoint, with coverage > 80%.
