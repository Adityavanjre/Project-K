...
async function run(): Promise<void> {
  ...
  // Initialize PolicyEngine + SpendTracker (SOVEREIGN)
  ...
  // Start heartbeat daemon (SOVEREIGN: DurableScheduler)
  const heartbeat = createHeartbeatDaemon({
    ...
    onWakeRequest: (reason) => {
      ...
      // SOVEREIGN: Use wake_events table instead of KV wake_request
    ...
    try {
      ...
      if (state === "sleeping") {
        ...
        while (slept < sleepMs) {
          ...
          // SOVEREIGN: Check for wake events from wake_events table (atomic consume)
