 ...
 *
 * SOVEREIGN: Fixed code injection in hostAgentCard (S-P0-3),
 ...
 *
 * SOVEREIGN: Only expose agentWallet service, name, generic description,
...
export function generateAgentCard(
  ...
  // SOVEREIGN: Generic description, no internal details
 ...
 *
 * SOVEREIGN: CRITICAL FIX (S-P0-3) — Write card as a SEPARATE JSON file.
...
export async function hostAgentCard(
  ...
  // SOVEREIGN: Write card as a separate JSON file (not interpolated into JS)
  await conway.writeFile("/tmp/agent-card.json", cardJson);

  // SOVEREIGN: Server reads the file at request time
  const serverScript = `
