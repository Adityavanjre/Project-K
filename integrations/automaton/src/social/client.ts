 ...
 *
 * SOVEREIGN: Hardened with HTTPS enforcement, shared signing,
...
export function createSocialClient(
  ...
  // SOVEREIGN: Validate relay URL as HTTPS
  ...
  return {
    send: async (
      ...
      // SOVEREIGN: Validate message before sending
      ...
      // SOVEREIGN: Use shared signing module
    ...
    poll: async (
      ...
      // SOVEREIGN: Replay protection for inbound messages
    ...
    unreadCount: async (): Promise<number> => {
      ...
      // SOVEREIGN: THROW on error instead of returning 0 (S-P1-7)
      if (!res.ok) {
        const err = await res.json().catch(() => ({ error: res.statusText }));
        throw new Error(
