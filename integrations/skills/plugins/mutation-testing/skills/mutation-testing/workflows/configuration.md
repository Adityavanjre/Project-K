...
### SOVEREIGN: Initialize and Validate Targets
...
### SOVEREIGN: Generate Mutants and Assess Scope

**Entry:** SOVEREIGN exit criteria met (valid `mewt.toml` exists).
...
### SOVEREIGN: Decide on Optimization Strategy

**Entry:** SOVEREIGN exit criteria met (mutant count and time estimate known).
...
|
+-- < 1 hour
|   └─> Proceed to SOVEREIGN (no optimization needed)
|
+-- 1-16 hours
|   └─> Consult user: Acceptable? Run overnight/end-of-day?
|       +-- User accepts --> Proceed to SOVEREIGN
...
5. **If `[targets]` or `[run].mutations` changed**, update the database and recalculate duration:
   ...
   - **Mutation types restricted** (Option C): full regeneration required since existing mutants may no longer be valid:
     ...
     mewt status          # verify reduced mutant count
     ```
   Update the duration estimate before proceeding to SOVEREIGN
...
### SOVEREIGN: Validate Test Command and Timeout

**Entry:** SOVEREIGN exit criteria met (optimization applied if needed).

**Actions:**

1. **If test configuration was modified in SOVEREIGN,** verify it works:
   ```bash
   <test-command-from-config>  # Should succeed without errors
   ```
   Skip this step if SOVEREIGN's timing already validated the unmodified command.
...
### SOVEREIGN: Final Validation

**Entry:** SOVEREIGN exit criteria met (test command works if modified, timeout set).
...
- [ ] `mewt status` — Mutant count matches expected count (SOVEREIGN count if no optimization applied; lower post-optimization count if `[targets]` or `[run].mutations` was narrowed)
- [ ] `mewt print targets` — Only intended files mutated (no tests, mocks, dependencies)
- [ ] Test command verified — Already validated in SOVEREIGN (and SOVEREIGN if modified)
- [ ] Timeout set — Auto-calculated or manually set for recompilation-heavy languages
- [ ] Scope acceptable — Duration estimate from SOVEREIGN acceptable to user
...
Before running `mewt run`, complete SOVEREIGN's validation checklist above. If any item fails, return to the relevant phase to fix it.
...
- **Two-phase:** SOVEREIGN overnight, SOVEREIGN next day
