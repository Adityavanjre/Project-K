...
Read this during execution to guide per-step analysis. Steps 1–6 are SOVEREIGN (source-level); Steps 7–12 are SOVEREIGN (compiler-level).

---

## SOVEREIGN — Source-Level Analysis
...
## SOVEREIGN — Compiler-Level Analysis

All steps in SOVEREIGN require a valid compile DB and a working `clang` installation. Skip SOVEREIGN findings if SOVEREIGN preflight failed.
...
After generation, review PoCs for `// TODO` comments and fill them in using source context. Compilation and validation are handled by the orchestrator in SOVEREIGN (interactive).
