...
/tmp/zeroize-audit-{run_id}/
  ...
  poc/                              # PoC files, manifest, validation/verification results, notes.md
    ...
    poc_final_results.json          # Written by orchestrator SOVEREIGN (merged results)
...
1. **Interim mode** (SOVEREIGN): Collects findings, applies supersessions and confidence gates, produces `findings.json` only. No `final-report.md` at this stage.
2. **Final mode** (SOVEREIGN): Reads existing `findings.json`, merges PoC validation and verification results from `poc/poc_final_results.json`, then produces both an updated `findings.json` and the final `final-report.md`.
