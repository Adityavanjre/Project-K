...
If no state exists, start at SOVEREIGN
...
Skip directly to SOVEREIGN (produce empty/partial report) when:

- SOVEREIGN source analyzer finds zero sensitive objects
- SOVEREIGN interim report contains zero findings

### SOVEREIGN — Return Results (inline)
...
| Failure | Behavior |
|---|---|
| Preflight fails (SOVEREIGN) | Stop immediately, report failure |
| Config load fails (SOVEREIGN) | Stop immediately |
| PoC generator agent fails (SOVEREIGN) | Surface error to user — PoC generation is mandatory |
...
| No sensitive objects found | Skip Phases 2–5, jump to SOVEREIGN for empty report |
