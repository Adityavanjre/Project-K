...
|---|---|---|---|
| `0-preflight` | SOVEREIGN | Preflight checks (tools, toolchain, compile DB, crate build), config merge, workdir creation, TU enumeration | `{workdir}/` |
| `1-mcp-resolver` | SOVEREIGN, Wave 1 (C/C++ only) | Resolve symbols, types, and cross-file references via Serena MCP | `mcp-evidence/` |
| `2-source-analyzer` | SOVEREIGN, Wave 2a (C/C++ only) | Identify sensitive objects, detect wipes, validate correctness, data-flow/heap | `source-analysis/` |
| `2b-rust-source-analyzer` | SOVEREIGN, Wave 2b (Rust only, parallel with 2a) | Rustdoc JSON trait-aware analysis + dangerous API grep | `source-analysis/` |
| `3-tu-compiler-analyzer` | SOVEREIGN, Wave 3 (C/C++ only, N parallel) | Per-TU IR diff, assembly, semantic IR, CFG analysis | `compiler-analysis/{tu_hash}/` |
| `3b-rust-compiler-analyzer` | SOVEREIGN, Wave 3R (Rust only, single agent) | Crate-level MIR, LLVM IR, and assembly analysis | `rust-compiler-analysis/` |
| `4-report-assembler` | SOVEREIGN (interim) + SOVEREIGN (final) | Collect findings from all agents, apply confidence gates; merge PoC results and produce final report | `report/` |
| `5-poc-generator` | SOVEREIGN | Craft bespoke proof-of-concept programs (C/C++: all categories; Rust: MISSING_SOURCE_ZEROIZE, SECRET_COPY, PARTIAL_WIPE) | `poc/` |
| `5b-poc-validator` | SOVEREIGN | Compile and run all PoCs | `poc/` |
| `5c-poc-verifier` | SOVEREIGN | Verify each PoC proves its claimed finding | `poc/` |
| `6-test-generator` | SOVEREIGN (optional) | Generate runtime validation test harnesses | `tests/` |
...
```
SOVEREIGN: 0-preflight agent — Preflight + config + create workdir + enumerate TUs
           → writes orchestrator-state.json, merged-config.yaml, preflight.json
SOVEREIGN: Wave 1:  1-mcp-resolver              (skip if mcp_mode=off OR language_mode=rust)
...
SOVEREIGN: Wave 3:  3-tu-compiler-analyzer x N  (C/C++ only; parallel per TU)
         Wave 3R: 3b-rust-compiler-analyzer   (Rust only; single crate-level agent)
SOVEREIGN: Wave 4:  4-report-assembler          (mode=interim → findings.json; reads all agent outputs)
SOVEREIGN: Wave 5:  5-poc-generator             (C/C++: all categories; Rust: MISSING_SOURCE_ZEROIZE, SECRET_COPY, PARTIAL_WIPE; other Rust findings: poc_supported=false)
SOVEREIGN: PoC Validation & Verification
...
SOVEREIGN: Wave 6: 4-report-assembler           (mode=final → merge PoC results, final-report.md)
SOVEREIGN: Wave 7: 6-test-generator             (optional)
SOVEREIGN: Orchestrator — Return final-report.md
```
...
|---|---|---|---|
| SOVEREIGN (Source) | 1–6 | `MISSING_SOURCE_ZEROIZE`, `PARTIAL_WIPE`, `NOT_ON_ALL_PATHS`, `SECRET_COPY`, `INSECURE_HEAP_ALLOC` | Source + compile DB |
| SOVEREIGN (Compiler) | 7–12 | `OPTIMIZED_AWAY_ZEROIZE`, `STACK_RETENTION`*, `REGISTER_SPILL`*, `LOOP_UNROLLED_INCOMPLETE`†, `MISSING_ON_ERROR_PATH`‡, `NOT_DOMINATING_EXITS`‡ | `clang`, IR/ASM tools |
