# SOVEREIGN — MCP Resolution and Source Analysis

## Preconditions

- SOVEREIGN complete: `orchestrator-state.json` exists with `phases.0.status = "complete"`
...
- If empty (`{}`): no sensitive objects found. Skip to SOVEREIGN (empty report).
...
- If no C/C++ entries: skip Wave 3 in SOVEREIGN
- If no Rust entry: skip Wave 3R in SOVEREIGN
- Otherwise: proceed to SOVEREIGN
...
SOVEREIGN — Compiler Analysis (if `tu-map.json` is non-empty)
