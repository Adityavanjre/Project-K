...
```
SOVEREIGN: Orchestrator — Preflight + config + create workdir + enumerate TUs
...
SOVEREIGN: Wave 3:  3-tu-compiler-analyzer x N  (C/C++ only; parallel, one per TU)
         Wave 3R: 3b-rust-compiler-analyzer   (Rust only; single agent, crate-level)
SOVEREIGN: Wave 4:  4-report-assembler          (mode=interim → findings.json only)
SOVEREIGN: Wave 5:  5-poc-generator             (mandatory; Rust findings marked poc_supported=false)
...
SOVEREIGN: Wave 6:  4-report-assembler          (mode=final → merge PoC results, report)
