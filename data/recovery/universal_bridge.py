...
class UniversalBridge:
    ...
    def _discover_all_nodes(self):
        ...
        discovered = {}
        cache_path = os.path.join(self.root, "logs", "sovereign_nodes.json")
        
        # In cloud/HF mode, fall back to the dynamically generated cache if available
        if os.getenv("KALI_CLOUD_MODE") == "true" and os.path.exists(cache_path):
            try:
                with open(cache_path, "r") as f:
                    import json
                    discovered = json.load(f)
                self.logger.info(f"👑 Sovereign Bridge: Dynamic cloud recovery active. Loaded {len(discovered)} nodes from cache.")
                return discovered
            except Exception as e:
                self.logger.error(f"Failed to load cached nodes on cloud: {e}")

        # Local physical discovery
        ...
        # Save active discovery to cache for cloud deployment
        if discovered:
            try:
                ...
                self.logger.error(f"Failed to save sovereign node cache: {e}")
