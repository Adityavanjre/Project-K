...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # Bypass auth if GOOGLE_CLIENT_ID is not configured (Local Dev Mode)
        # OR if KALI_SOVEREIGN_BYPASS_AUTH is set to true
        bypass_env = os.environ.get("KALI_SOVEREIGN_BYPASS_AUTH", "false").lower() == "true"
        client_id = current_app.auth_service.client_id if current_app.auth_service else None
        is_dev = not current_app.auth_service or not client_id or bypass_env
        
        # 🧠 PHASE 5 — AUTHENTICATION BYPASS VALIDATION
        # logger.info(f"AUTH_PROBE: bypass_env={bypass_env}, client_id_present={bool(client_id)}, is_dev={is_dev}")
...
def main():
    ...
    # 🧠 PHASE 2 — ROUTE MAP FORENSICS
    print("\n" + "="*60)
    print(" KALI ROUTE MAP (app.url_map)")
    print("="*60)
    for rule in app.url_map.iter_rules():
        print(f" ROUTE: {rule.rule} -> {rule.endpoint} [{rule.methods}]")
    print("="*60 + "\n")
