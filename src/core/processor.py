...
class DoubtProcessor:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if self.use_local_ai:
            ...
            self.ai_service = AIService(self.config.get("openai", {}), vector_memory=self.vector_memory)
        ...
        self.reflection_engine = ReflectionEngine(self.ai_service, self.user_dna, self.vector_memory, processor=self)
        ...
        if self.sovereign_force_local:
            self.logger.warning("🛡️ SOVEREIGN_FORCE_LOCAL ACTIVE: Bypassing External Council.")
    ...
    def process_doubt(self, query: str, context: Optional[DoubtContext] = None, source: str = "general", bypass_cache: bool = False) -> Any:
        try:
            ...
            if not bypass_cache:
                ...
                if cached:
                    ...
                    bio_state = self.biometric_service.get_physiological_state(self.sensors.get_system_metrics().get("cpu_usage", 0))
            ...
            predictions = self.predictive_engine.predict_next_steps(query, dna_profile.get("interaction_stats", {}).get("total_conversations", 0))
            ...
            if self.user_tension > 80:
                ...
                self.logger.info("KALI: High tension detected. Simplifying explanation.")
            ...
            use_council = len(query) > 150 and self.power_mode == "TURBO" and not self.sovereign_force_local
            
            if self.sovereign_force_local:
                self.logger.info("🛡️ KALI: SOVEREIGN_FORCE_LOCAL engaged. Using Local Node.")
                ...
                council_response = self.council.get_consensus(query, context=full_context, bypass_cache=bypass_cache)
                response = council_response if council_response else self.explainer.generate_explanation(
                    query, context=full_context, style=user_level, bypass_cache=bypass_cache
                )
            else:
            ...
            if "" in str(response):
                ...
                audit_res = self.review_service.review_manifest(str(response), query.splitlines()[0])
                if audit_res.get("score", 100) < 80:
                    self.logger.warning(f"KALI CodeRabbit: Audit score low ({audit_res.get('score')}). Re-correcting.")
                    response = self.council.get_consensus(f"CRITIQUE AND FIX: {query}\n\nFindings: {audit_res.get('findings')}", context=str(response))
            
            # Phase 39: RLHF Post-Processing (Alignment)
            alignment = self.rlhf_service.calculate_alignment(str(response), dna_profile.get("preferences", {}).get("top_directives", []))
            ...
            self.vector_memory.remember(f"Q: {query}\nA: {response}", collection_name="history")
            ...
            if self.message_count % 5 == 0:
                ...
                self.gsd_service.add_task(f"Evolution Milestone: Completed 5 interactions. System alignment: {alignment}%.")

            # Proactive Gap Analysis & Autonomous Manifestation
            gap_prompt = self.gap_detector.get_proactive_prompt(query)
            if gap_prompt:
                self.logger.info(f"KALI: Capability Gap Identified: {gap_prompt}. Manifesting skill.")
                ...
                response = f"{response}\n\n---\n{status_msg}"
                res["manifested_skill"] = manifest_res.get("skill_name")
            ...
            self.training_logger.log(query, str(response), source=source, context=source)
            ...
            if "manifest" in query.lower() or "build this" in query.lower():
                ...
                manifest_res_path = self.manifestor.manifest({"title": proj_name, "summary": response})
                response = f"{response}\n\n✅ **MANIFESTED**: Project path: {manifest_res_path}"

            # Final Response Assembly
            build_keywords = ["build", "manifest", "design", "make", "create", "circuit", "robot", "drone", "how to build"]
            res = {
                "text": response,
                "audio_url": audio_url,
            ...
            self.sovereign_cloud.anchor_memory_segment(res["msg_id"], {
                "query": query,
                "response": response,
                "alignment": alignment,
    ...
    def process_project_mentor(self, idea: str) -> Dict[str, Any]:
        ...
        bom = self.bom_service.generate_project_bom({
            "name": idea[:30],
            "components": research.get("steps", []) # Use steps as components if not explicitly listed
        })
        self.active_bom = bom

        # Phase 28: Fabrication Hub Blueprints & CAD
        blueprint = self.blueprint_service.generate_assembly_steps(idea[:30], bom, research.get("steps", []))
        ...
        review = self.review_service.review_manifest(json.dumps(project_plan_enhanced, indent=2), idea)
        ...
        self.sovereign_cloud.anchor_memory_segment(f"PROJ_{idea[:8]}", {
            "idea": idea,
            "bom": bom,
        ...
        try:
            ...
            if added > 0:
                self.logger.info(f"KALI DreamEngine: Synthesized {added} augmented interactions.")
        except Exception as e:
    ...
    def end_session(self):
        ...
        """
        self.logger.info(f"KALI: Ending Session {self.current_session_id}. Consolidating...")
        
        # Trigger Dream Engine
        seeds = self.dream_engine.dream()
        for seed in seeds:
            self.vector_memory.remember(f"WISDOM_SEED: {seed['insight']}", collection_name="knowledge")
            
        # Global Sync
        self.run_sync_cycle()
        
        # Phase 38: Final Cloud Anchor
        self.sovereign_cloud.anchor_memory_segment("SESSION_END", {
    ...
    def _generate_dpo_critique(self, goal: str, original_answer: str):
        ...
        improved_answer = self.ai.ask_question(
            f"GOAL: {goal}\n"
        ...
        dpo_entry = {
            ...
            "metadata": {"score": critique.get("score"), "timestamp": datetime.now().isoformat()}
        }
    ...
    def swap_skill_service(self, service_name: str, instance: Any):
        ...
        try:
            if hasattr(self, service_name):
                ...
                self.logger.info(f"[+] Skill Service '{service_name}' upgraded successfully.")
            else:
    ...
    def log_preference(self, is_positive: bool):
        ...
        self.logger.info(f"KALI DPO: Preference Recorded -> {'SUCCESS' if is_positive else 'CRITIQUE'}")

    def get_history(self):
        """Phase 1: Retrieve session history for formal testing."""
        return self.memory.get_recent_memories(limit=50, session_id=self.current_session_id)

    def clear_history(self):
        """Phase 1: Clear session history for formal testing."""
        self.memory.clear_memory(session_id=self.current_session_id)
        self.vector_memory.clear_memory(session_id=self.current_session_id) # If it exists
