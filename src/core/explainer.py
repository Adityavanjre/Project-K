...
from .processor import DoubtContext
...
class Explainer:
    ...
    def generate_explanation(
        ...
        """
        try:
            self.logger.debug(f"Generating explanation for question type: {analysis.get('type')}")
            
            # Determine explanation style based on user level
            style = self.explanation_styles.get(context.user_level, self.explanation_styles["intermediate"])
    ...
    def _create_knowledge_based_explanation(
        ...
        if analysis["type"] == "definition":
            explanation_parts.append(f"**{primary_knowledge.title.replace('What is ', '')}**")
    ...
    def _create_general_explanation(
        ...
        if analysis["domain"] != "general":
            suggestions.append(f"• Focus on {analysis['domain']} resources for domain-specific information")
...
I'm continuously learning and improving. Feel free to rephrase your question or ask about something else!"""
    ...
    def _generate_example(self, knowledge: KnowledgeItem, analysis: Dict[str, Any]) -> Optional[str]:
    ...
    def _generate_analogy(self, knowledge: KnowledgeItem, analysis: Dict[str, Any]) -> Optional[str]:
