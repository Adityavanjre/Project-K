...
class BOMService:
    ...
    def generate_project_bom(self, design_specs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        total_cost = sum(
            float(str(item.get("est_price", 0)).replace("$", "").replace(",", ""))
        ...
        bom = {
            ...
            "total_est_local": self.market_engine.get_currency_conversion(
                total_cost, "INR"
            ),
            "currency": "INR",
        ...
        logger.info(
            f"KALI Economic: BOM Generated for {bom['project']} - Total: ${total_cost:.2f}"
        )
        return bom
