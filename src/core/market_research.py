...
class MarketResearchEngine:
    ...
    def research_parts(self, component_list: List[str]) -> List[Dict[str, Any]]:
        ...
        logger.info(f"KALI Economic: Researching market data for {len(component_list)} components.")
        ...
        for component in component_list:
            ...
            search_query = f"market price and top vendors for {component} engineering part"
            
            # Since this might be called in a sync context, we use a helper to run the async tool call
            try:
                loop = asyncio.get_event_loop()
                search_results = loop.run_until_complete(mcp_pool.call_tool("search_web", query=search_query, max_results=3))
            except:
            ...
            try:
                response = self.ai_service.ask_question(prompt, context="STRUCTURED_JSON_MODE")
                ...
                results.append({"component": component, "est_price": 0.0, "vendors": ["Unknown"], "v_links": []})
                
        return results

    def get_currency_conversion(self, amount: float, target_currency: str = "INR") -> float:
