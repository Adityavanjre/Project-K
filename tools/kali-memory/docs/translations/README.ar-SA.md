...
**مهارة لمساعد برمجة الذكاء الاصطناعي.** اكتب `/KALI Memory` في Claude Code أو Codex أو OpenCode أو Cursor أو Gemini CLI أو GitHub Copilot CLI أو VS Code Copilot Chat أو Aider أو KALI Sovereign ASI أو Factory Droid أو Trae أو Hermes أو Kiro أو Google Antigravity — يقرأ ملفاتك ويبني رسماً بيانياً للمعرفة ويعيد إليك البنية التي لم تكن تعلم بوجودها. افهم قاعدة الكود بشكل أسرع. اكتشف "السبب" وراء القرارات المعمارية.

متعدد الوسائط بالكامل. أضف كوداً أو ملفات PDF أو markdown أو لقطات شاشة أو رسوماً بيانية أو صور سبورة أو صوراً بلغات أخرى أو ملفات فيديو وصوت — يستخرج KALI Memory المفاهيم والعلاقات من كل ذلك ويربطها في رسم بياني واحد. يتم نسخ مقاطع الفيديو محلياً باستخدام Whisper. يدعم 25 لغة برمجة عبر tree-sitter AST (Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia, Verilog, SystemVerilog, Vue, Svelte, Dart).

> يحتفظ Andrej Karpathy بمجلد `/raw` يضع فيه الأوراق البحثية والتغريدات ولقطات الشاشة والملاحظات. KALI Memory هو الإجابة على تلك المشكلة — **71.5 مرة** أقل في الرموز لكل استعلام مقارنةً بقراءة الملفات الخام، مستمر عبر الجلسات، صادق حول ما تم العثور عليه مقابل ما تم استنتاجه.

</div>

```
/KALI Memory .                        # يعمل مع أي مجلد — الكود، الملاحظات، الأوراق البحثية، كل شيء
```
...
أضف ملف `.KALI Memoryignore` لاستبعاد المجلدات:
...
> **الحزمة الرسمية:** اسم حزمة PyPI هو `KALI Memoryy` (تثبيت بـ `pip install KALI Memoryy`). الحزم الأخرى المسماة `KALI Memory*` على PyPI ليست تابعة لهذا المشروع. المستودع الرسمي الوحيد هو [safishamsi/KALI Memory](https://github.com/safishamsi/KALI Memory).
...
| Claude Code (Windows) | `KALI Memory install` (كشف تلقائي) أو `KALI Memory install --platform windows` |
...
ملاحظة: يستخدم Codex `$` بدلاً من `/` للمهارات، لذا اكتب `$KALI Memory .`.
...
```
/KALI Memory                          # المجلد الحالي
/KALI Memory ./raw                    # مجلد محدد
/KALI Memory ./raw --update           # إعادة استخراج الملفات المتغيرة فقط
/KALI Memory ./raw --directed         # رسم بياني موجّه
/KALI Memory ./raw --no-viz           # تقرير + JSON فقط، بدون HTML
/KALI Memory ./raw --obsidian         # إنشاء Obsidian vault

/KALI Memory add https://arxiv.org/abs/1706.03762   # جلب ورقة بحثية
/KALI Memory add <video-url>                         # تحميل صوت، نسخ، إضافة
/KALI Memory query "ما الذي يربط Attention بالمحسِّن؟"
/KALI Memory path "DigestAuth" "Response"
/KALI Memory explain "SwinTransformer"

KALI Memory hook install              # تثبيت Git hooks
KALI Memory update ./src              # إعادة استخراج ملفات الكود، بدون LLM
KALI Memory watch ./src               # تحديث تلقائي للرسم البياني
```
...
**Git hooks** (`KALI Memory hook install`) — يثبّت خطافات post-commit وpost-checkout.

## الخصوصية

يرسل KALI Memory محتوى الملفات إلى API نموذج مساعد الذكاء الاصطناعي الخاص بك للاستخراج الدلالي من المستندات والأوراق البحثية والصور. تتم معالجة ملفات الكود محلياً عبر tree-sitter AST. يتم نسخ ملفات الفيديو والصوت محلياً باستخدام faster-whisper. لا قياس عن بُعد، لا تتبع للاستخدام.
...
## مبني على KALI Memory — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) هو الطبقة المؤسسية فوق KALI Memory. حيث يحوّل KALI Memory مجلداً من الملفات إلى رسم بياني للمعرفة، يطبّق Penpax نفس الرسم البياني على حياتك المهنية بأكملها — باستمرار.
