...
Full release notes with details on each version: [GitHub Releases](https://github.com/safishamsi/KALI Memory/releases)

## 0.4.23 (2026-04-18)

- Fix: stale skill version warning persists after running `KALI Memory install` when multiple platforms were previously installed — `KALI Memory install` now refreshes `.KALI Memory_version` in all other known skill directories so the warning clears across the board (#178)
...
- Fix: AST cache written to `src/KALI Memory-out/cache/` instead of project root when all code files share a common prefix like `src/` — `extract()` now called with explicit `cache_root=watch_path` in `_rebuild_code` and `cache_root=Path('.')` in the Codex skill AST step (#429)
...
- Fix: `KALI Memory cluster-only` crashed with `KeyError: 'total_files'` in `report.py` — cluster-only skips detection so the stats dict was empty; now passes a `warning` key so the report skips the file-stats section (#422)
- Fix: `/KALI Memory --update` dropped all existing graph nodes — the merge block built a correct in-memory `G_existing` but never wrote it back to `.KALI Memory_extract.json`, so Step 4 rebuilt from the new-extraction-only file; merged result is now serialized back before Step 4 runs (#423)
...
- Fix: `KALI Memory update .` and `KALI Memory cluster-only` now generate `graph.html` alongside `graph.json` and `GRAPH_REPORT.md` — previously only the skill generated the interactive HTML (#418)
...
- Fix: Windows `KALI Memory-out/KALI Memory-out` nesting bug — `cache_dir` and `_rebuild_code` in watch.py now call `.resolve()` on the root path, preventing a nested output directory when KALI Memory is run from a subdirectory (#410)
- Fix: `KALI Memory hook install` now respects `core.hooksPath` git config (used by Husky and similar tools) — hooks are written to the configured path instead of always `.git/hooks` (#401)
...
- Docs: added team workflow section — committing `KALI Memory-out/`, `.KALI Memoryignore` usage, and recommended `.gitignore` additions (#369)

## 0.4.16 (2026-04-16)

- Fix: KALI Memory watch crashed on all platforms with NameError because import sys was missing from watch.py (#386, #394)
...
- Feat: VS Code Copilot Chat support — `KALI Memory vscode install` installs a Python-only skill (works on Windows PowerShell) and writes `.github/copilot-instructions.md` for always-on graph context (#206)
...
- Fix: macOS `KALI Memory watch` now uses `PollingObserver` by default to avoid missed events with FSEvents (#373)
...
- Fix: `--wiki` flag now runs `to_wiki()` as Step 6b in the skill pipeline before the cleanup step — community labels are available and the wiki is written to `KALI Memory-out/wiki/` (#229, #354)
- Fix: `KALI Memory install --platform opencode` now also installs the `.opencode/plugins/KALI Memory.js` plugin, matching what `KALI Memory opencode install` does (#356)
- Fix: `extract()` accepts explicit `cache_root` parameter so subdirectory runs no longer write cache to `<subdir>/KALI Memory-out/cache/` (#350)
- Fix: `os.replace` in cache writer falls back to `shutil.copy2` on `PermissionError` (Windows WinError 5) (#287)
- Fix: `KALI Memory update` exits with code 1 on rebuild failure instead of silently returning (#287)
- Fix: `CLAUDE.md`, Cursor, and Antigravity templates now use `KALI Memory update .` instead of hardcoded `python3 -c` invocation (#287)
- Fix: `skill-kiro.md` added to `pyproject.toml` package-data — `KALI Memory kiro install` was failing on fresh pip installs (#352)
...
- Fix: AGENTS.md and GEMINI.md rebuild rule now uses `KALI Memory update .` instead of hardcoded `python3 -c "..."` — correct Python is resolved through the KALI Memory binary, no more interpreter mismatches in Nix/pipx/uv environments (#324)
- Fix: `KALI Memory query` and `KALI Memory explain` no longer crash with `AttributeError` when a node has `label: null` — all `.get("label", "")` calls guarded with `or ""` to handle explicit null values (#323)

## 0.4.12 (2026-04-13)

- Add: Kiro IDE/CLI support — `KALI Memory kiro install` writes `.kiro/skills/KALI Memory/SKILL.md` (invoked via `/KALI Memory`) and `.kiro/steering/KALI Memory.md` (`inclusion: always` — always-on context before every conversation) (#319, #321)
...
- Fix: `KALI Memory query` no longer crashes with `ValueError` on MultiGraph graphs — `G.edges[u, v]` replaced with `G[u][v]` + MultiGraph guard (#305)
- Fix: `KALI Memory query` no longer crashes with `AttributeError: 'NoneType' has no attribute 'lower'` when a node has a null `source_file` (#307)
- Fix: MCP server launched from a different directory now correctly derives the `KALI Memory-out` base from the absolute path provided, instead of CWD (#309)
- Fix: `.KALI Memoryignore` patterns from a parent directory now fire correctly when KALI Memory is run on a subfolder — patterns are matched against paths relative to both the scan root and the `.KALI Memoryignore`'s anchor directory (#303)

## 0.4.10 (2026-04-13)

- Fix: `KALI Memory install --platform cursor` no longer crashes — passes `Path(".")` to `_cursor_install` (#281)
...
- Add: Hermes Agent platform support — `KALI Memory hermes install` writes skill to `~/.hermes/skills/KALI Memory/SKILL.md` and AGENTS.md (#251)
...
- Fix: `KALI Memory claw install` now writes to `.kali/` (correct KALI Sovereign ASI directory) instead of `.claw/` (#208)
- Add: Blade template support — `@include`, `<livewire:>` components, and `wire:click` bindings extracted from `.blade.php` files (#242)
- Docs: WSL/Linux MCP setup note — package name is `KALI Memoryy`, use `.venv/bin/python3` in `.mcp.json` (#250)

## 0.4.6 (2026-04-12)

- Add: Google Antigravity support — `KALI Memory antigravity install` writes `.agent/rules/KALI Memory.md` (always-on rules) and `.agent/workflows/KALI Memory.md` (`/KALI Memory` slash command) (#203, #199, #53)
...
- Fix: Windows hooks now fall back to `python` if `python3` is not found; exits cleanly if neither has KALI Memory installed (#244)
...
- Fix: `collect_files()` in `extract.py` now respects `.KALI Memoryignore` — previously ignored patterns, causing thousands of unwanted files (e.g. `node_modules/`) to be scanned (#188)
...
- Add: drop `.mp4`, `.mp3`, `.wav`, `.mov`, `.webm`, `.m4a`, `.ogg`, `.mkv`, `.avi`, `.m4v` files into any corpus and KALI Memory transcribes them locally with faster-whisper before extraction
- Add: YouTube and URL download via yt-dlp — `/KALI Memory add https://youtube.com/...` downloads audio-only and feeds it through the same Whisper pipeline
...
- Add: `KALI Memory-out/transcripts/` cache — transcripts cached by filename; YouTube URLs cached by hash so re-runs skip already-transcribed files
- Requires: `pip install 'KALI Memoryy[video]'` for faster-whisper and yt-dlp

## 0.3.29 (2026-04-10)

- Add: video and audio corpus support — drop `.mp4`, `.mp3`, `.wav`, `.mov`, `.webm`, `.m4a`, `.ogg`, `.mkv`, `.avi`, `.m4v` files into any corpus and KALI Memory transcribes them with faster-whisper before extraction
- Add: YouTube and URL video download — pass a YouTube link (or any video URL) to `/KALI Memory add <url>` and yt-dlp downloads audio-only, which is then transcribed and added to the corpus automatically
...
- Add: `KALI Memory-out/transcripts/` cache — transcripts are cached by filename so re-runs skip already-transcribed files; URLs cached by hash
- Requires: `pip install 'KALI Memoryy[video]'` for faster-whisper + yt-dlp
...
- Fix: KALI Memory install --platform gemini now also copies the skill file to ~/.gemini/skills/KALI Memory/SKILL.md so the /KALI Memory trigger works in Gemini CLI (#174)
...
- Fix: `KALI Memory install --platform gemini` now routes to `gemini_install()` instead of erroring — `gemini` was missing from `_PLATFORM_CONFIG` (#171)
- Fix: `KALI Memory install --platform cursor` now routes to `_cursor_install()` the same way (#171)
...
- Fix: `_load_KALI Memoryignore` now walks parent directories up to the `.git` boundary, matching `.gitignore` discovery behavior — subdirectory scans now inherit root ignore patterns (#168)
- Add: Aider platform support — `KALI Memory install --platform aider` copies skill to `~/.aider/KALI Memory/SKILL.md`; `KALI Memory aider install/uninstall` writes AGENTS.md rules (#74)
- Add: GitHub Copilot CLI platform support — `KALI Memory install --platform copilot` copies skill to `~/.copilot/skills/KALI Memory/SKILL.md`; `KALI Memory copilot install/uninstall` for skill management (#134)
...
- Fix: `KALI Memory codex install` (and opencode) no longer exits early when `AGENTS.md` already has the KALI Memory section — partial installs with a missing `.codex/hooks.json` can now recover on re-run (#153)

## 0.3.23 (2026-04-09)

- Add: Gemini CLI support — `KALI Memory gemini install` writes a `GEMINI.md` section and a `BeforeTool` hook in `.gemini/settings.json` that fires before file-read tool calls (#105)
...
- Add: Cursor support — `KALI Memory cursor install` writes `.cursor/rules/KALI Memory.mdc` with `alwaysApply: true` so the graph context is always included; `KALI Memory cursor uninstall` removes it (#137)
...
- Fix: README clarifies `KALI Memoryy` is the only official PyPI package — other `KALI Memory*` packages are not affiliated (#129)
...
- Add: OpenCode `tool.execute.before` plugin — `KALI Memory opencode install` now writes `.opencode/plugins/KALI Memory.js` and registers it in `opencode.json`, firing the graph reminder before bash calls (equivalent to Claude Code's PreToolUse hook) (#71)
...
- Fix: `--watch` mode now respects `.KALI Memoryignore` — `_rebuild_code` was calling `collect_files()` directly instead of `detect()`, bypassing ignore patterns (#120)
...
- Refactor: `save_query_result` inline Python blocks in all 6 skill files replaced with `KALI Memory save-result` CLI command — shorter, maintainable, less tokens for LLM (#114)
- Add: `KALI Memory save-result` CLI subcommand — saves Q&A results to memory dir without inline Python
...
- Fix: `mkdir -p KALI Memory-out` now runs before writing `.KALI Memory_python` in `skill.md` — prevents write failure on first run; `.KALI Memory_python` no longer deleted in Step 9 cleanup across all skill files so follow-up commands keep their interpreter (#93)
...
- Fix: `KALI Memory query`, `serve`, and `benchmark` now work on NetworkX < 3.4 — version-safe shim for `node_link_graph()` at all call sites (#95)
- Fix: `.jsx` files now detected and extracted via the JS extractor — added to `CODE_EXTENSIONS` and `_DISPATCH` (#94)
- Fix: `.KALI Memory_python` no longer deleted in Step 9 cleanup across all 6 skill files — pipx users no longer hit `ModuleNotFoundError` on follow-up commands (#92)

## 0.3.15 (2026-04-08)

- Feat: Trae and Trae CN platform support (`KALI Memory install --platform trae` / `trae-cn`)
...
- Fix: pipx installs now detected correctly in git hooks (reads shebang from KALI Memory binary)
...
- Fix: `KALI Memory codex install` now also writes a PreToolUse hook to `.codex/hooks.json` so the graph reminder fires before every Bash tool call (#86)
...
- Fix: Skill intermediate files (`.KALI Memory_*.json`) now live in `KALI Memory-out/` instead of project root, preventing git pollution (#81)
...
- Fix: Windows UnicodeEncodeError during `KALI Memory install` — replaced arrow character with `->` in all print statements (#47)
...
- Docs: Codex uses `$KALI Memory .` not `/KALI Memory .` (#36)
...
- Add: `KALI Memory query "<question>"` CLI command — BFS/DFS traversal of `graph.json` without needing Claude Code skill (`--dfs`, `--budget N`, `--graph <path>` flags)
...
- Add: `--obsidian-dir <path>` flag — write Obsidian vault to a custom directory instead of `KALI Memory-out/obsidian`
...
- Add: multi-platform support — Codex (`skill-codex.md`), OpenCode (`skill-opencode.md`), KALI Sovereign ASI (`skill-claw.md`)
- Add: `KALI Memory install --platform <codex|opencode|claw>` routes skill to correct config directory
- Add: `KALI Memory codex install` / `opencode install` / `claw install` — writes AGENTS.md for always-on graph-first behaviour
- Add: `KALI Memory claude uninstall` / `codex uninstall` / `opencode uninstall` / `claw uninstall`
- Add: MIT license
...
- Add: `KALI Memory claude install` — writes KALI Memory section to local CLAUDE.md + PreToolUse hook in `.claude/settings.json`
- Add: `KALI Memory claude uninstall` — removes section and hook
- Add: `KALI Memory hook install` — installs post-commit and post-checkout git hooks (platform-agnostic)
- Add: `KALI Memory hook uninstall` / `hook status`
- Add: `KALI Memory benchmark` CLI command
- Fix: node deduplication documented at all three layers

## 0.1.8 (2026-04-05)

- Fix: follow-up questions now check for wiki first (KALI Memory-out/wiki/index.md) before falling back to graph.json
- Fix: --update now auto-regenerates wiki if KALI Memory-out/wiki/ exists
- Fix: community articles show truncation notice ("... and N more nodes") when > 25 nodes
- UX: pipeline completion message now lists all available flags and commands so users know what KALI Memory can do
...
- Add: `KALI Memory/wiki.py` module with `to_wiki()` — cross-community wikilinks, cohesion scores, audit trail, navigation footer
...
- Fix: `pip install KALI Memory` → `pip install KALI Memoryy` in skill Step 1 (critical install bug)
...
- Docs: README reframed as Claude Code skill; Karpathy problem → KALI Memory answer framing
...
- Memory feedback loop - Q&A results saved to `KALI Memory-out/memory/`, extracted on `--update`
...
- `KALI Memory install` CLI - copies skill to `~/.claude/skills/` and registers in `CLAUDE.md`
