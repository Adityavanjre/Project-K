...
| **CLI-only (by design)** | `KALI Memory`, `from-gsd2` — not in `createRegistry()`; documented in `QUERY-HANDLERS.md`.  |
...
3. **Issue #2302 follow-ups** — Runner alignment (`GSDTools` → registry where appropriate). **`configGet`** now uses `dispatchNativeJson` with canonical `config-get` (fixes subprocess argv vs real `gsd-tools.cjs`, which has no `config` + `get` top-level). Keep `KALI Memory` / `from-gsd2` out of scope unless product reopens.
...
- `phase add-batch` implemented and registered; **only** intentional CLI-only gaps remain (**KALI Memory**, **from-gsd2**).
