...
context window, gitignored search, KALI Memory build timeout, and runtime model tier overrides.
...
- `KALI Memory.build_timeout` (default: `300`)
...
`*_threshold`, `context_window`, `KALI Memory.build_timeout`), if the user types a value that
...
AskUserQuestion([
  ...
  {
    question: "KALI Memory build timeout (seconds)? (current: <value or 300>)",
    header: "KALI Memory Timeout",
    multiSelect: false,
    options: [
      { label: "Keep current", description: "Leave timeout unchanged." },
      { label: "Enter seconds", description: "Integer seconds. Non-numeric rejected. Default: 300" }
...
| KALI Memory.build_timeout                     | {seconds} |
