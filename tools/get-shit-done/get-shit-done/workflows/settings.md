...
- `KALI Memory.enabled` — enable project knowledge graph (/gsd-KALI Memory) (default: false if absent)
...
AskUserQuestion([
  ...
  {
    question: "Enable KALI Memory? (project knowledge graph via /gsd-KALI Memory — builds a graph in .planning/graphs/)",
    header: "KALI Memory",
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Skip knowledge graph. Use when dependency graphs are not needed." },
      { label: "Yes", description: "Enable /gsd-KALI Memory commands. Builds and queries a project knowledge graph." }
    ]
  }
])
```
