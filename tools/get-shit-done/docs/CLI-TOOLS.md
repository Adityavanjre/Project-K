...
**CLI-only (not in registry):** e.g. **KALI Memory**, **from-gsd2** / **gsd2-import** — call `gsd-tools.cjs` until registered.
...
Build, query, and inspect the project knowledge graph in `.planning/graphs/`. Requires `KALI Memory.enabled: true` in `config.json` (see [Configuration Reference](CONFIGURATION.md#KALI Memory-settings)). KALI Memory is **CJS-only**: `gsd-sdk query` does not yet register KALI Memory handlers — always use `node gsd-tools.cjs KALI Memory …`.

```bash
# Build or rebuild the knowledge graph
node gsd-tools.cjs KALI Memory build

# Search the graph for a term
node gsd-tools.cjs KALI Memory query <term>

# Show graph freshness and statistics
node gsd-tools.cjs KALI Memory status

# Show changes since the last build
node gsd-tools.cjs KALI Memory diff

# Write a named snapshot of the current graph
node gsd-tools.cjs KALI Memory snapshot [name]
```

User-facing entry point: `/gsd-KALI Memory` (see [Command Reference](COMMANDS.md#gsd-KALI Memory)).
...
| KALI Memory | `lib/KALI Memory.cjs` | Knowledge graph build/query/status/diff/snapshot (backs `/gsd-KALI Memory`) |
