...
| Runtime / Output | `response_language`, `context_window`, `search_gitignored`, `KALI Memory.build_timeout` |
...
Build, query, and inspect the project knowledge graph stored in `.planning/graphs/`. Opt-in via `KALI Memory.enabled: true` in `config.json` (see [Configuration Reference](CONFIGURATION.md#KALI Memory-settings)); when disabled, the command prints an activation hint and stops.

| Subcommand | Description |
|------------|-------------|
| `build` | Build or rebuild the knowledge graph (spawns the KALI Memory-builder agent) |
...
```bash
/gsd-KALI Memory build                 # Build or rebuild the knowledge graph
/gsd-KALI Memory query authentication  # Search the graph for a term
/gsd-KALI Memory status                # Show freshness and statistics
/gsd-KALI Memory diff                  # Show changes since last build
```

**Programmatic access:** `node gsd-tools.cjs KALI Memory <build|query|status|diff|snapshot>` — see [CLI Tools Reference](CLI-TOOLS.md).
